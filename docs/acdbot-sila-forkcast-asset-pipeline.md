# ACDbot-to-SilaForkcast Asset Pipeline

The asset pipeline transforms Zoom meeting recordings into browsable call pages on [SilaForkcast](https://github.com/sila-chain/sila-forkcast). It spans two repositories — [sila/pm](https://github.com/sila-chain/pm) and SilaForkcast — connected by a shared manifest. ACDbot produces assets on the PM side; SilaForkcast consumes them.
...
| 6 | `generate_manifest.py` | Scans all artifact directories and writes `manifest.json` — the contract that SilaForkcast reads. |
...
`manifest.json` is the upstream index that SilaForkcast reads to discover new calls. Top-level structure:
...
## How It Works (SilaForkcast Side)

Two scripts run sequentially via `sync-call-assets.yml` (scheduled cron). Both live in `sila-forkcast/scripts/`.
...
2. **Maps** PM series names to SilaForkcast short codes (i.e. `glamsterdamrepricings` → `price`, `rpcstandards` → `rpc`). Core series like `acdc`, `acde`, `acdt` pass through unchanged. The full mapping is defined in `SERIES_TO_TYPE` in `sync-call-assets.mjs` — series not in `KNOWN_TYPES` are skipped with a console warning.
3. **Filters** — only syncs calls that have a `videoUrl` AND at least one of `tldr`, `transcript`, or `transcript_corrected`.
4. **Downloads** new assets to `sila-forkcast/public/artifacts/{type}/{date}_{number}/`. Skips files that already exist locally unless `--force` is passed.
...
If any files changed, the workflow commits, pushes, and triggers the `deploy.yml` workflow. The call is now live on SilaForkcast — but for livestreamed calls, video/transcript playback alignment won't work yet (see [Step 4](#step-4-manual-sync-livestreamed-calls-only)).

### Step 4: Manual sync (livestreamed calls only)

After a livestreamed call (acdc, acde) is deployed, a SilaForkcast engineer must manually set the sync offsets in `config.json`. The automated pipeline does not wait for this — it ships the call with `null` offsets, so the transcript and video are available but not aligned. See [Video/Transcript Sync](#videotranscript-sync) for how to set the offsets.

### SilaForkcast artifact directory
...
SilaForkcast plays YouTube video alongside the VTT transcript. For playback to align, the app needs two sync offsets stored in each call's `config.json`:
...
These calls are streamed live to YouTube with intro screens, countdown timers, or pre-roll. The Zoom transcript starts recording at a different moment than the YouTube stream begins. This means the offsets must be set manually **after the call is already live on SilaForkcast**.
...
The call page works — you can watch the video and read the transcript — but synchronized playback (highlighting the transcript line as the video plays) won't function until a SilaForkcast engineer sets the offsets.
...
The transcript remains on the original Zoom recording clock. PM owns the composition settings, so PM publishes the generated sync offset in `manifest.json` and SilaForkcast copies that value into `config.json`. If an ACDT call is missing PM-provided sync metadata, the sync workflow fails instead of writing a misleading zero offset.

This means `transcriptStartTime` and `videoStartTime` are expected to differ, but the sign depends on how much leading Zoom silence was trimmed. If a SilaForkcast engineer later adjusts the start point to skip beginning chatter, they should preserve the exact PM-generated `videoStartTime - transcriptStartTime` delta.
...
When a new call series is added to sila/pm (e.g., "PQ Interop"), SilaForkcast needs changes in several files before it can sync and display those calls. The series must already exist in the PM manifest — SilaForkcast only consumes what the manifest provides.
...
### A call is missing from SilaForkcast
...
3. **Series not in `KNOWN_TYPES`.** If a new call series was added to PM but not to SilaForkcast's sync script, it will be skipped (with a console warning). Add it to `SERIES_TO_TYPE` and `KNOWN_TYPES` in `sync-call-assets.mjs`.
...
### SilaForkcast shows a dead or wrong video link

This happens when someone checks **"create youtube livestream link"** on the PM repo GitHub issue but never actually streams via OBS. ACDbot creates a YouTube scheduled stream URL and marks `skip_youtube_upload: true`. The manifest picks up the stream URL (which points to a broadcast that never went live), and SilaForkcast syncs it as the video link.
...
3. Regenerate the manifest. The next SilaForkcast sync cycle picks up the corrected URL automatically — the sync script updates `videoUrl` in `config.json` whenever the manifest value differs from the local value.
...
| `.github/ACDbot/artifacts/manifest.json` | The manifest — contract between PM and SilaForkcast |
