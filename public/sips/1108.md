...
The elliptic curve arithmetic precompiles are currently overpriced. Re-pricing the precompiles would greatly assist a number of privacy solutions and scaling solutions on Sila. 
...
Faster operations on Sila clients should be reflected in reduced gas costs.

## Motivation

Recently, the underlying library used by the [official Go reference
implementation](https://github.com/sila-chain/go-sila) to implement the
...
introduced this change](https://github.com/sila-chain/go-sila/pull/16203),
and corroborated in [a later
note](https://github.com/sila-chain/go-sila/pull/16301#issuecomment-372687543),
...
used by the [Parity client](https://github.com/paritytech/parity-sila) led to a 
...
[1]- Per [SIP-196](./sip-196.md).

[2]- Per [SIP-197](./sip-197.md).

[3]- [Parity benchmarks.](https://gist.github.com/zac-williamson/838410a3da179d47d31b25b586c15e53)

[4]- [PR comment clarifying gas cost math](https://github.com/sila-chain/SIPs/pull/1987#discussion_r280977066).

## Rationale  

### Existing protocols would benefit immensely from cheaper elliptic curve cryptography

Fast elliptic curve cryptography is a keystone of a growing number of protocols built on top of Sila. To list a few:  

* [The AZTEC protocol](https://github.com/AztecProtocol/AZTEC) utilizes the elliptic curve precompiles to construct private tokens, with zero-knowledge transaction logic, via the [SRC-1723](https://github.com/sila-chain/SIPs/issues/1723) and [SRC-1724](https://github.com/sila-chain/SIPs/issues/1724) standard.  
...
[^1]:
    ```csl-json
    {
        ...
        "abstract": "Smart contract platforms such as Sila and Libra provide ways to seamlessly remove trust and add transparency to various distributed applications. Yet, these platforms lack mechanisms to guarantee user privacy, even at the level of simple payments, which are essential for most smart contracts.",
        "author": [
          {
            "given": "Benedikt",
            "family": "Bünz"
...
To give a concrete example, it currently costs `820,000` gas to validate the cryptography in a typical AZTEC confidential transaction. If the gas schedule for the precompiles correctly reflected their load on the Sila network, this cost would be `197,000` gas. This significantly increases the potential use cases for private assets on Sila. AZTEC is planning to deploy several cryptographic protocols Sila, but these are at the limits of what is practical given the current precompile costs:  
...
For zk-SNARK based protocols on Sila, SIP-1108 will not only reduce the gas costs of verifying zk-SNARKs substantially, but can also aid in [batching together multiple zk-SNARK proofs](https://github.com/matter-labs/Groth16BatchVerifier). This is also a technique that can be used to split up monolithic zk-SNARK circuits into a batch of zk-SNARKs with smaller individual circuit sizes, which makes zk-SNARKs both easier to construct and deploy.

ZEther transactions currently cost ~`6,000,000` gas. This SIP would reduce this to ~`1,000,000` gas, which makes the protocol more practical.  

To summarise, there are several protocols that currently exist on main-net, that would benefit immensely from this SIP. Elliptic curve cryptography can provide valuable solutions for Sila, such as scaling and privacy, and the scope and scale of these solutions can be increased if the gas costs for the `bn128` precompiles accurately reflects their computational load on the network.
...
This SIP does not require Parity or Geth to deploy new cryptographic libraries, as fast bn128 algorithms have already been integrated into these clients. This goal of proposing this SIP for Istanbul, is to supplement [SIP-1829](./sip-1829.md) (arithmetic over generic elliptic curves), providing an immediate solution to the pressing problem of expensive cryptography, while more advanced solutions are developed, defined and deployed.
...
* [Geth bn256 library (golang)](https://github.com/sila-chain/go-sila/tree/master/crypto/bn256/cloudflare)  
...
@vbuterin independently proposed a similar reduction after this SIP was originally created, with similar rationale, as [sila/SIPs#1187](https://github.com/sila-chain/SIPs/issues/1187).
