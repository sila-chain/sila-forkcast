...
With the recent advances in quantum computing, there are increased concerns for the quantum threat against Sila. Today ECDSA is the EOA signature algorithms, which is vulnerable to attacks by quantum computers. Efficient replacement algorithms use polynomial multiplication as the core operation. Once NTT and Inverse NTT are available, the remaining of the verification algorithm is trivial. Choosing to integrate NTT and InvNTT instead of a specific algorithm provides agility, as DILITHIUM or FALCON or any equivalent can be implemented with a modest cost from those operators. NTT is also of interest to speed-up STARK verifiers. This single operator would thus benefit to both the Sila scaling and post-quantum threat mitigation.
...
The gas cost model for SIP-7885 precompiles is designed to target approximately 50-55 mgas/s performance, consistent with existing precompiled contracts like ECRECOVER. Two implementations with different gas costs are available:
...
Adopting this SIP, the signature verification of Falcon can be reduced to **1500** gas, and a similar result is expected for Dilithium.
Adopting the hash function as a separate SIP would enable a gas verification cost of 2000 gas.
...
These results validate that NTT precompiles significantly reduce gas costs for post-quantum signature verification, making lattice-based cryptography more practical for Sila applications.
...
Benchmark data are available in the SIP assets:

**Pure Go Implementation (nocgo):**

- [NTT Precompile Benchmark Results](../assets/sip-7885/op-geth/nocgo/benchmark_results/BenchmarkPrecompiledNTT)
- [ECRECOVER Benchmark Results](../assets/sip-7885/op-geth/nocgo/benchmark_results/BenchmarkPrecompiledEcrecover)

**liboqs Implementation (cgo):**

- [NTT Precompile Benchmark Results](../assets/sip-7885/op-geth/cgo/benchmark_results/BenchmarkPrecompiledNTT)
- [ECRECOVER Benchmark Results](../assets/sip-7885/op-geth/cgo/benchmark_results/BenchmarkPrecompiledEcrecover)

## Reference Implementation

- [Python reference implementation](../assets/sip-7885/pythonref)
- [Solidity reference implementation](../assets/sip-7885/solidity/src/ZKNOX_NTT.sol)
- OP-Geth implementations in the assets of this SIP:
  - [Pure Go (nocgo)](../assets/sip-7885/op-geth/nocgo)
  - [liboqs (cgo)](../assets/sip-7885/op-geth/cgo)
