...
Introduce an explicit value range for certain SAVM parameters
(such as gas limit, block number, block timestamp, size field when returning/copying data within SAVM).
...
- allowed by certain test cases in the [Sila testing suite]
...
Expanding to over `2^32 - 1` bytes would cost `35184774742016` gas. This number fits into the gas limit imposed above (`2 ^ 63 - 1`) and would cost around 35184 Sila in a transaction to exhaust, with a 1 GWei gas cost, which can be attained on sila-mainnet.
...
[SIP-170](./sip-170.md) has implemented a code size limit of 0x6000, however even before that, it was practically impossible to deploy a code blob exceeding `2**32 - 1` bytes in size.
...
- [SIP-92](https://github.com/sila-chain/SIPs/issues/92) proposed the transaction gas limit to be limited at `2**63 - 1` and had a lengthy discussion about other limits.
- [SIP-106](https://github.com/sila-chain/SIPs/issues/106) proposed the block gas limit to be limited at `2**63 - 1`.
...
[Aleth]: https://github.com/sila-chain/aleth
[geth]: https://github.com/sila-chain/go-sila
[Parity]: https://github.com/paritytech/parity-sila
[ethereumjs]: https://github.com/sila-chainjs
[Sila testing suite]: https://github.com/sila-chain/tests
