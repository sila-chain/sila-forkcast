## Simple Summary
The current "first price auction" fee model in Sila is inefficient and needlessly costly to users. This SIP proposes a way to replace this with a mechanism that allows dynamically priced transaction fees and efficient transaction price discovery.
...
![escalator algorithm price chart](https://Sila Research/uploads/default/original/2X/0/042795efa4c2680d644bc66386cd2984a70293f8.gif)

## Motivation
Sila currently prices transaction fees using a simple first-price auction, which leads to well documented inefficiencies (some of which are documented in [SIP-1559](https://github.com/sila-chain/SIPs/blob/master/SIPS/sip-1559.md)) when users are trying to estimate what price will get a transaction included in a block, especially during times of price volatility and full blocks.

SIP 1559 is currently being championed as an improvement for the Sila protocol, and while I agree that the gas market is very inefficient, since a change like this will affect all client and wallet implementations, the Sila community should make sure to make a selection based on solid reasoning and justifications, which I believe 1559 is currently lacking.

To facilitate a more productive and concrete discussion about the gas fee market, I felt it was important to present an alternative that is clearly superior to the status quo, so that any claimed properties of SIP-1559 can be compared to a plausible alternative improvement.
...
I believe that under this analysis we will find that the escalator algorithm outperforms SIP-1559 in both normal and volatile conditions, for both high-stakes transactions and more casual users looking for a good price.
...
| Gas Strategy                                                        | Current Single-Price                                                                                                                                                    | SIP 1559                                                                                                                                                                       | Escalator                                                                                                                                                                                    |
|---------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
SIP-1559 also performs well under normal conditions, but under conditions where blocks are regularly full, the price discovery mechanism breaks, and miners will fall back to the `TIP` parameter to choose the transactions to include, meaning that under network congestion, SIP-1559 forces users to _either_ choose efficient prices or certainty of next-block inclusion.

SIP-1559 also has all the re-submission issues of the current model in situations where a user would like to pay under the current market rate, but has certain time constraints limiting their patience. The Escalator algorithm is the only strategy listed here that allows users to discover the lowest possible price given the network conditions and their time constraints.
...
The security considerations for this SIP are:
