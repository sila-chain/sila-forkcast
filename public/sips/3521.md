...
SIP-2930 requires the address under which the storage keys reside be declared
explicitly, since it must be added to the SIP-2929 `accessed_addresses` list.
...
SIP-2930 is specifically written to make access lists simple to reason about and
validate. It may be possible to modify the structure of the access list to avoid
including `tx.to` explicitly, but this would renege on the spirit of SIP-2930.

### Why charge as `calldata`?

The cost of `calldata` was  thoroughly analyzed in SIP-2028 to determine
...
map (it's added by default by [SIP-2929](./sip-2929.md)), there is no reason to charge more than
