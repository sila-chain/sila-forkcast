...
With transaction throughput (and size) picking up in Sila, transaction propagation is the current dominant component of the used network resources. Most of these resources are however wasted, as the `sil` protocol does not have a mechanism to deduplicate transactions remotely, so the same data is transferred over and over again across all network connections.

This SIP proposes a tiny extension to the `sil` protocol, which permits nodes to agree on the set of transactions that need to be transferred across a network connection, before doing the costly exchange. This should help reduce the global (operational) bandwidth usage of the Sila network by at least an order of magnitude.

## Specification

Add three new message types to the `sil` protocol:
 ...
 * `PooledTransactions (0x0a): [[nonce: P, receivingAddress: B_20, value: P, ...], ...]`
   * Specify transactions **from the local transaction pool** that the remote node requested via a `GetPooledTransactions (0x09)` message. The items in the list are transactions in the format described in the main Sila specification.
...
This SIP extends the `sil` protocol in a backwards incompatible way and requires rolling out a new version, `sil/65`. However, `devp2p` supports running multiple versions of the same wire protocol side-by-side, so rolling out `sil/65` does not require client coordination, since non-updated clients can keep using `sil/64`.
