...
Describes the motivation and rational for splitting the SIP repositories into an
SIP repository, targeting core sila changes and an SRC repository, targeting
application layer specifications.

## Motivation

Long ago when the SIPs repository was created, there was a vision of a single
home for all standards related to Sila. The community was small and most
...
For years, we've considered separating the repository. This would allow SRC and
SIP specifications to evolve more naturally due to the independence. But it's
...
Now that the Consensus Layer is also utilizing the SIP process, the cracks are
becoming more visible. There are changes we could make to the process that might
benefit them more, but because we also need to ensure the quality of SRCs, we
are restricted.

There are also many more efforts to catalyze applications around the SRC
process. Attempts have been made to develop working groups and review groups for
certain SRC "categories" (a distinction that doesn't even technically exist
...
SIP, as it is the motivating point of this SIP that the divergent needs of the
community will require highly divergent methods.

1. All SRCs and Interface-category SIPs are removed from this repository and
   migrated to a new repo. The history should be intact so that repo should be
   forked of this one with the non-SRCs removed. 
2. The new SRCs repository goes live and includes the changes from the script.
3. Setup ercs.sila.org subdomain and update the CI to point to the SRCs
   repo.
4. Set up a redirect for SRCs on sips.sila.org to go to the new website.
5. Create a unified document for editors to assign SIP/SRC numbers. SIPs and
   SRCs will no longer be based on an initial PR number but on a number
   incremented by the SIP editors of their respective repositories. SIPs will be
   assigned even numbers and SRCs will be assigned odd numbers. The exact timing
   of this migration is a policy decision of the editors.

The SIP repository will be associated with core protocol changes, specifically
the kind that would be discussed in one of the AllCoreDevs calls; whereas the
SRC repository will be affiliated with all remaining areas such as smart
...
This association is to persist across any other process changes the SIP editors
...
SIP repository and other sub-groupings are associated with the SRC repository.
Any such process change are out of scope of this SIP and are independent of the
structural changes to the repositories specified in this SIP.

There may be further structural changes to repository layouts to accommodate
more sub-groupings. Such proposals are out of scope of this SIP.

## Rationale

There are two major communities served by the SIP process that are highly
...
improperly implemented part of a new spec could cause the sila sila-mainnet to
split, possibly costing millions to billions of value lost to node operators as
well as community members using the services offered by the Sila protocols.
...
protocol SIP adjusting the gas cost for transaction data needs to be rolled out
...
Sila clients, and in fact would also need to be rolled out by service
provides that provide RPC services for Sila networks. Wallets can use early
support as a differentiating factor in their appeal to community members.

To address this divergence the AllCoreDevs call has adopted a lifecycle for SIPs
different from the Draft -> Review -> Last Call -> Final lifecycle of the SIP
repository. It would best be described as Draft -> Eligible for Inclusion ->
Considered for Inclusion -> Testnet -> SilaMainnet. The SIPs also get slotted for a
...
to adequately judge if an SIP is complete and sound. Considering that SIPs
...
When examining how the core protocol and SRC communities would approach the
...
sometimes both. There is no SIP that would be shipped in sila-mainnet that would not
first be extensively considered by one of these two groups.

SRC proposals have no such standing groups. Wallet impacting changes may go
...
The Working Group model, however, would be a critical social signal for the SRC
...
While working groups are excellent for the SRC community, it is overhead for the
...
`sip-editors.yml` file. This allows for different groups of editors to review
...
probably be done anyway, whether or not the SIP repository splits.

### Alternative: Replace SIP Editors with AI Chatbots
...
### Objection: This splits the sila community

One objection is that splitting the repository would result in the community no
longer being able to say "we are all of us Sila Magicians."
...
operating separate from SIPs have allowed them to strengthen the sila
community.
...
### Objection: This should be an  [SIP-1](./sip-1.md) proposal

Since this is directly impacting the SRC process it should be documented
in  [SIP-1](./sip-1.md) first.
...
only confuse the existing process, adding special cases for one class of SIPs
that don't apply to another. It is precisely this kind of problem the proposed
split is aiming to change.

This is also valid grounds for a Meta category SIP, as how many and which
repository to put a proposal in is core to the "procedures, guidelines, \[and\]
changes to the decision-making process".

Some process changes that can be expected in a Core Protocol SIP may include:

* Changing the work flow to add the Eligible for Inclusion/Considered for
  Inclusion stages to a pre-last-call SIP.
* Adding test net and sila-mainnet steps to the lifecycle
* Adding a "fork" header to the RFCs section, for SIPs that are (or will be)
  implemented in a specific fork
* Changing the testing section to a header link to reference tests

Some process change SRC may want to adopt:

* A strong working group model and adding an optional "forming working group"
  step editors may require.
* Add an "outdated" or "replaced" lifecycle step for SIPs that are abrogated by
  future specs.
* Deputize single-sip reviewers for specific SIPs
...
SIP process changes related to this. Bundling the changes is unnecessary and
such structure and process changes should be handled independently.

To accommodate this objection this SIP has been revised to only address
...
Old SRC links pointing to the old url `https://sips.sila.org/` will continue
to work. Redirect instructions will be put into place to redirect to the new SRC
repos for their corresponding location.

### Stray Proposals

SRC community members may continue to post new SRCs in the SIP proposal. Editors
will be able to redirect them to the new repository. SRCs that do not respond to
editor requests would not be merged anyway.

## Security Considerations

This proposal only addresses the SIP and SRC proposal process and is not
