...
Execution proofs enable consensus layer (CL) nodes to perform stateless validation of execution payloads. Altruistic _proof-generating nodes_ generate execution proofs and broadcast them over the CL p2p network. _Proof-verifying nodes_ consume gossiped execution proofs and check payload validity by verifying those proofs. Verification is _stateless_ and constant time with respect to the payload's gas limit and EL state size, so a validator's payload-verification requirements are decoupled from the linear cost of re-execution. The mechanism is fully opt-in and does not change consensus validity rules, allowing the proving stack to mature in production; a separate, future SIP could subsequently propose making execution proofs mandatory once they have matured. This SIP does not introduce incentives for proof-generating nodes; for that reason, they are considered altruistic and the mechanism is opt-in.
...
This SIP introduces an opt-in path with the following properties:
...
This SIP uses the following proof-system terms:
...
- **Prover.** A node that runs the execution-validation computation and generates an execution proof for the result. In this SIP, provers are opt-in, altruistic proof-generating validators.
...
- **Private input.** Data supplied to the guest by the prover but not revealed to verifiers. In this SIP, the serialised `StatelessInput` is private input: it contains the data needed to run stateless payload validation.
...
- **Public input.** The data committed to by the proof and visible to verifiers. In this SIP, the public input binds the proof to a specific `NewPayloadRequest`, chain configuration, and validation result.
...
- **Server-Sent Events (SSE).** An HTML standard for one-way, long-lived HTTP event streams from server to client. In this SIP, SSE is used in two places: by the beacon node to push `Block` events to the prover, and by the proof node to push proof-completion events to the prover.

### Specification references

The normative specifications for this SIP are maintained alongside the rest of the protocol in the consensus-specs and execution-specs repositories. The sections below summarise the changes; the canonical sources are:

- [Consensus Layer](https://github.com/sila-chain/consensus-specs/tree/031e521f0a82b0e475ecfc13f79dd251a6284fc2/specs/_features/eip8025)
- Execution Layer:
    - [Stateless validation interfaces](https://github.com/sila-chain/execution-specs/blob/85fc20ca5937719a854472a87cb48d01ef1dffca/src/sila/forks/amsterdam/stateless.py)
    - [Guest program](https://github.com/sila-chain/execution-specs/blob/85fc20ca5937719a854472a87cb48d01ef1dffca/src/sila/forks/amsterdam/stateless_guest.py)
    - [Host-side input assembly](https://github.com/sila-chain/execution-specs/blob/85fc20ca5937719a854472a87cb48d01ef1dffca/src/sila/forks/amsterdam/stateless_host.py)
    - [Execution witness construction](https://github.com/sila-chain/execution-specs/blob/85fc20ca5937719a854472a87cb48d01ef1dffca/src/sila/forks/amsterdam/stateless_host_exec_witness.py)
    - [SSZ serialisation schema](https://github.com/sila-chain/execution-specs/blob/85fc20ca5937719a854472a87cb48d01ef1dffca/src/sila/forks/amsterdam/stateless_ssz.py)
    - [Conformance tests](https://github.com/sila-chain/execution-specs/tree/85fc20ca5937719a854472a87cb48d01ef1dffca/tests/amsterdam/eip8025_optional_proofs)

### Consensus Layer

This SIP introduces a new `execution_proof` gossip topic on the
...
- **Proof-generating mode.** An active validator that opts in to act as
  a _prover_ — generates execution proofs and broadcasts them on the
...
- **Proof-verifying mode.** A node that consumes gossiped proofs and
  ...
  fork-choice dependent on it. A subsequent SIP could propose promoting
...
![Execution-proof flow between proof-generating and proof-verifying nodes via the `execution_proof` gossip topic](../assets/sip-8025/proof-flow.png)
...
[SIP-7732](./sip-7732.md), which defers execution validation until the next
...
- **Verification** (`verify_execution_proof`) is called on every incoming
  ...
  verified. The value of `k` will be pinned before this SIP transitions
...
SIP-8025 extends `process_block` to forward execution payload processing to
...
- **`ExecutionProofStatus`** — request and response share the same
  ...
  [`Status v2`](https://github.com/sila-chain/consensus-specs/blob/031e521f0a82b0e475ecfc13f79dd251a6284fc2/specs/fulu/p2p-interface.md#status-v2)
  handshake.
...
Discovery is extended with a new Sila Node Record (ENR) key, `eproof`, encoded as a `uint8`. A
...
SilaFulu via the
[`cgc` ENR field](https://github.com/sila-chain/consensus-specs/blob/031e521f0a82b0e475ecfc13f79dd251a6284fc2/specs/fulu/p2p-interface.md#the-discovery-domain-discv5)
...
![Prover lifecycle: SSE-driven flow between beacon node, prover, and proof node](../assets/sip-8025/prover-lifecycle.png)
...
The prover role is scoped to the optional-proof phase. If a future SIP
...
This SIP introduces a stateless execution guest and prover-side logic that
...
[SIP-4844](./sip-4844.md). The typed deposit, withdrawal, and consolidation
requests are specified by [SIP-6110](./sip-6110.md),
[SIP-7002](./sip-7002.md), and [SIP-7251](./sip-7251.md), respectively.
Builder deposit and exit requests are specified by
[SIP-8282](./sip-8282.md).
The `block_access_list` field in `ExecutionPayload` is the block-level access
list defined by [SIP-7928](./sip-7928.md).
...
SilaAmsterdam guest does not compare the payload timestamp with fork activation
...
identifies the schema revision. SilaAmsterdam revision 1 uses schema ID `0x1501`.
...
STATELESS_INPUT_SCHEMA_FORK_INDEX = ProtocolFork.SilaAmsterdam
...
The progressive containers and lists follow [SIP-7688](./sip-7688.md).
...
This SIP is fully opt-in and does not change consensus validity rules. Validators that do not enable either mode see no change to their behaviour, their bandwidth, or their attestation duties. Nodes that opt in additionally subscribe to the proof gossip topic, advertise themselves via the `eproof` ENR field, and may run a prover; this affects only their local resource profile.

## Test Cases

Conformance tests are maintained in the canonical specification repositories:

- [Execution Layer](https://github.com/sila-chain/execution-specs/tree/85fc20ca5937719a854472a87cb48d01ef1dffca/tests/amsterdam/eip8025_optional_proofs)
- [Consensus Layer](https://github.com/sila-chain/consensus-specs/tree/031e521f0a82b0e475ecfc13f79dd251a6284fc2/specs/_features/eip8025)

## Reference Implementation

- [Execution Layer](https://github.com/sila-chain/execution-specs/tree/85fc20ca5937719a854472a87cb48d01ef1dffca/src/sila/forks/amsterdam)
