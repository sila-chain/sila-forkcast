...
Since [SIP-170](./sip-170.md) was implemented, there has been a size limit of `24576` (`0x6000`) on contract code. We propose to also limit the size of executable code to `2x` the above limit, i.e. `49152` (`0xc000`).
...
Historically, this was exploited in June 2017, precipitating the 1.6.5-patch release of [geth](https://github.com/sila-chain/go-sila/releases/tag/v1.6.5)
...
In all these (and future) cases, the SAVM should fail with Out Of Gas error if the code has a length more than `max_initcode_size`.
...
For client implementations, this SIP makes attacks based on jumpdest-analysis or hashing of code less problematic, so should increase the robustness of clients. 

For layer 2, this SIP introduces failure-modes where there previously were none. There _could_ exist factory-contracts which deploy multi-level contract hierarchies, such that the code for multiple contracts are included in the initcode of the first contract. The author(s) of this SIP are not aware of any such contracts. 
