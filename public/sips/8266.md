...
A frame transaction is in **expiring-nonce mode** if `tx.nonce == EXPIRING_NONCE_SENTINEL`. Otherwise the transaction follows [SIP-8141](./sip-8141.md) unchanged.
...
The sender-nonce check from SIP-8141 (`tx.nonce == state[tx.sender].nonce`) is skipped.

### Nonce consumption

SIP-8141's payment-approval transition currently increments the sender's account nonce. In expiring-nonce mode that increment is replaced with the following sequence, performed atomically on the unique successful payment-scoped `APPROVE`:
...
Nodes MAY admit multiple pending expiring-nonce transactions per sender; SIP-8141's one-pending-frame-transaction-per-sender guidance does not apply. Instead, the node MUST reserve each pending transaction's maximum cost (`TXPARAM(0x06)`) against its gas payer's available balance, applying SIP-8141's `reserved_pending_cost(payer)` accounting, and admit a new transaction only if the payer's available balance covers it.

### Activation

This SIP MUST activate at or after [SIP-8141](./sip-8141.md). At activation, on the first execution payload with `timestamp >= FORK_TIMESTAMP` and before any transaction in that payload runs, clients MUST install `NONCE_RING_CODE` at `NONCE_RING` with nonce `1` and empty storage, preserving any pre-existing balance. The same address-selection and existing-account-handling requirements as SIP-8141's `EXPIRY_VERIFIER` activation apply.
...
### Composition with SIP-8250

If both this SIP and [SIP-8250](./sip-8250.md) ship, the sentinel collapses into SIP-8250's keyed-nonce framing as a reserved key `nonce_key == 2**256 - 1`; `NONCE_RING`'s storage can live in `NONCE_MANAGER` under a distinct slot prefix; and the mechanism in this SIP is otherwise unchanged. This composition is non-normative.

## Backwards Compatibility

Transactions with `tx.nonce != EXPIRING_NONCE_SENTINEL` behave exactly as in SIP-8141. Existing wallets, RPC methods, and verifiers continue to work unchanged. Verifiers that previously assumed `TXPARAM(0x01)` is the sender's account nonce MUST either reject expiring-nonce transactions or be updated to handle the sentinel.
...
`NONCE_RING` storage is system-managed; ordinary direct calls revert. Any balance held at `NONCE_RING` is outside the scope of this SIP and is not recoverable by protocol logic defined here.
