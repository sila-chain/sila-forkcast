...
Anyone that wishes to propose a Core SIP for the hard fork should make a PR against the Meta SIP representing the hard fork. The SIP must be published as at least `Draft`. It enters the _Proposed EIPs_ section, along with at least one person who is a point of contact for wanting to include the SIP.

SIPs can move states by discussion done on the "[All Core Devs Meetings](https://github.com/sila-chain/pm/)":
- If accepted for a hard fork, the SIP should be moved to the _Accepted EIPs_ section. If the SIP has major client implementations and no security issues by the timeline date, it is scheduled for inclusion.
- If rejected from a hard fork, the SIP should be moved to the _Rejected EIPs_ section.
- Once the SIPs in the _Accepted EIPs_ section have successfully launched on a testnet roll out, they are moved to the _Included EIPs_ section.
...
A template for the [Istanbul Hardfork Meta 1679](./sip-1679.md) is included below ([source file on GitHub](./sip-1679.md)):
