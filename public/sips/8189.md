...
This SIP upgrades the `snap` protocol from version 1 to version 2, removing `GetTrieNodes` (0x06) / `TrieNodes` (0x07) and adding `GetBlockAccessLists` (0x08) / `BlockAccessLists` (0x09) to enable BAL-based state healing during snap sync.
...
BALs are only available for blocks after [SIP-7928](./sip-7928.md) activation and within the retention period defined therein.
...
- Nodes **should** retain BALs for orphaned (non-canonical) blocks within the retention period defined in [SIP-7928](./sip-7928.md). Since requests are keyed by block hash, orphaned BALs can be served the same way as canonical ones. Retaining them enables syncing nodes to recover from reorgs past the pivot block without restarting sync.
...
Received BALs **must** be validated by computing `keccak256(rlp.encode(bal))` and comparing against the `block-access-list-hash` in the corresponding block header. See [SIP-7928](./sip-7928.md) for the BAL encoding format.
...
### Separate Protocol vs. sil/71

This follows the established snap design philosophy. `GetByteCodes` was duplicated from `sil` into `snap` for the same reason:

> *This functionality was duplicated into `snap` from `sil/65` to permit `sil` long term to become a chain maintenance protocol only and move synchronization primitives out into satellite protocols only.*

Synchronization primitives belong in satellite protocols. `snap` is optional and independently versioned; duplicating BAL exchange here avoids coupling sync functionality to `sil`.
...
This SIP requires rolling out a new protocol version, snap/2. Older clients continue using snap/1. snap/2 is only meaningful for post-SilaAmsterdam blocks, since the `block-access-list-hash` header field ([SIP-7928](./sip-7928.md)) is absent for earlier blocks.
