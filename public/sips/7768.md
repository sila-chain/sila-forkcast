...
A technique is introduced where an externally-owned account having no Sila can send transactions and pay tips using a new "free-for-all" bucket and using their own `origin.tx`. This requires no client changes and is compatible with existing ecosystem parts.

## Motivation

There is much interest in third-party-pay transactions on Sila and competing networks.

Other proposals require changes to the Sila client, that transactions be sent to the network (i.e. `tx.origin`) using a separate account and/or other additional things.
...
1. Sign a transaction (from any origin) to send Sila to Alice representing the current gas price times the current block size.
2. (Optional) Prepare and sign a transaction to the free-for-all account, to preload any necessary responses.
3. Start an execution context and include this send-Sila transaction and all of Alice's transactions.
...
5. Sign a transaction to (from any origin) to take security of the consideration from the free-for-all account and include this transaction in the execution context.
6. Evaluate the total gas spent.
7. Rollback the execution context. And repeat steps 1 through 4 with these changes:
   1. Step 1: use the actual required gas amount (in Sila).
...
8. Use some local business logic to compare the Sila spent in step 1 (second iteration) versus the consideration received in step 4 and classify the result as favorable or not.
...
This SIP proposes to use a well-known contract account deployment for this purpose. And here is the required interface:
...
This approach can be useful for end users who do not want to or are not able to add Sila to their account.

This approach allows to use the correct `origin.tx` which may be required for important transactions like SRC-721 `setApprovalForAll`. 
...
* Update [SIP-1559](./sip-1559) so that transactions with gasPrice = 0 are legal, but only if the commensurate amount of gas will be burnt by the block preparer in that same block.
