...
This SIP proposes a procedure to convert, on each block, a fixed number of key-values from the existing Merkle Patricia Tree (MPT) to the Verkle Tree (VKT).

## Motivation

The accounts state is too large to wait for transactions to organically move all of them to the VKT through the Overlay Tree. Thus, we need a strategy to convert all the state within a reasonable time. The state conversion completion allows removing the Overlay Tree abstraction introduced in [SIP-7612](./sip-7612.md) and to use directly the VKT for all state access.
...
def state_convert(state: State, stride: int):
    ...
    while n < stride and not state._conversion_finished:
        ...
        # This covers the 28 SIP-7610 accounts, but would also cover all
...
### Deletion of [SIP-7610](./sip-7610.md) accounts

SIP-7610 mentions the existence of 28 accounts that have:
...
### Not counting [SIP-161](./sip-161.md) accounts for `CONVERSION_STRIDE` limit

The `CONVERSION_STRIDE` parameter tries to limit the load of effective writes. These special accounts are skipped since we try to perform a bulk [SIP-158](./sip-158.md) deletion of the remaining accounts.
...
- `transition-post-genesis` branch in `github.com/gballet/go-sila` implements this when setting `--override.overlay-stride` to a non-zero value on the command line.
