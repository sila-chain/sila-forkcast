...
This SIP decomposes the flat `21,000` intrinsic transaction cost into explicit primitives, each priced to match the resources a transaction actually consumes. The costs of plain SIL transfers and contract creation transactions remain equal, while SIL transfers to new accounts and [7702](./sip-7702.md)-related transactions
...
| `TRANSFER_LOG_COST` | 1,756 | [SIP-7708](./sip-7708.md) transfer log: `GAS_LOG + 3 × GAS_LOG_TOPIC + 32 × GAS_LOG_DATA_PER_BYTE` |
...
| `ACCOUNT_WRITE` | 8,000 | [SIP-8038](./sip-8038.md) | First write to an account leaf |
| `CREATE_ACCESS` | 11,000 | [SIP-8038](./sip-8038.md) | Contract-deployment account access and write |
| `REGULAR_PER_AUTH_BASE_COST` | 7,816 | [SIP-8037](./sip-8037.md) | Per-authorization base: calldata, ECDSA recovery, cold authority access, and warm writes |
...
The two state-gas products change whenever `CPSB` changes. Regular-gas and state-gas semantics are defined by [SIP-8037](./sip-8037.md).
...
- each [SIP-7702](./sip-7702.md) authorization is charged `REGULAR_PER_AUTH_BASE_COST` in regular gas.
...
If the transaction goes out-of-gas in this pre-execution phase, the sender pays for all gas consumed, and all state changes are reverted, including any [SIP-7702](./sip-7702.md) delegations. By contrast, if the transaction goes out-of-gas after entering the first SAVM frame, the already applied delegations during pre-execution phase stay in place, per [SIP-7702](./sip-7702.md), and all runtime charges remain consumed.
...
For each authorization, after the gas charges of that authorization are processed, the authority is added to `accessed_addresses`, per [SIP-7702](./sip-7702.md).
...
- if the recipient is an [SIP-7702](./sip-7702.md) delegated account, additionally charge the delegation-target access in regular gas: `COLD_ACCOUNT_ACCESS` if cold, `WARM_ACCESS` if warm.
...
| SIL transfer to existing EOA | `TX_BASE_COST + COLD_ACCOUNT_ACCESS + TX_VALUE_COST + TRANSFER_LOG_COST` / 0 | 0 / 0 | 21,000 / 0 |
| SIL transfer to contract | `TX_BASE_COST + COLD_ACCOUNT_ACCESS + TX_VALUE_COST + TRANSFER_LOG_COST` / 0 | execution / 0 | 21,000 + execution / 0 |
| No-transfer to 7702 delegated | `TX_BASE_COST + COLD_ACCOUNT_ACCESS` / 0 | `COLD_ACCOUNT_ACCESS` + execution / 0 | 18,000 + execution / 0 |
| SIL transfer to 7702 delegated | `TX_BASE_COST + COLD_ACCOUNT_ACCESS + TX_VALUE_COST + TRANSFER_LOG_COST` / 0 | `COLD_ACCOUNT_ACCESS` + execution / 0 | 24,000 + execution / 0 |
| SIL transfer to self, sender 7702 delegated | `TX_BASE_COST` / 0 | `COLD_ACCOUNT_ACCESS` + execution / 0 | 15,000 + execution / 0 |
| SIL transfer creating new account | `TX_BASE_COST + COLD_ACCOUNT_ACCESS + TX_VALUE_COST + TRANSFER_LOG_COST` / 0 | 0 / `STATE_BYTES_PER_NEW_ACCOUNT × CPSB` | 21,000 / 183,600 |
...
- **[SIP-7702](./sip-7702.md) (set EOA code).** `TX_BASE_COST` is unchanged even when the sender temporarily assumes code; clients must not perform a disk code load to classify the sender, since SIP-7702 provides the code inline. When `tx.to` is a delegated account, resolving the delegation loads the target's code, charged `COLD_ACCOUNT_ACCESS` on first touch or warm thereafter, following the standard SIP-2929 `accessed_addresses` model. Setting a delegation (writing the 23-byte pointer) is distinct from resolving one (reading the target's code); the resolution charge is genuine work and is not prepaid by the authorization. Authorization cost itself is decomposed: `REGULAR_PER_AUTH_BASE_COST` — which already includes the authority's cold access — is charged per authorization at the intrinsic phase, while the state-dependent charges (`STATE_BYTES_PER_NEW_ACCOUNT × CPSB` for a non-existent authority, `ACCOUNT_WRITE` for the first write to the authority within the transaction, and `STATE_BYTES_PER_AUTH_BASE × CPSB` for net-new delegation bytes) are charged at runtime during authorization processing, replacing the worst-case `PER_EMPTY_ACCOUNT_COST` intrinsic charge and refund.
- **[SIP-7708](./sip-7708.md) (SIL transfers emit a log).** Every non-zero-value transfer to a different account emits a transfer log, priced explicitly as `TRANSFER_LOG_COST` rather than absorbed into the base.
- **[SIP-7928](./sip-7928.md) (block-level access lists).** `tx.sender` is accessed at the intrinsic phase and `tx.to` at the runtime phase, so both are included in the BAL even when the transaction reverts.
...
`COLD_ACCOUNT_ACCESS` is taken directly from [SIP-8038](./sip-8038.md), since the recipient touch is the same cold-account access priced there, and `TRANSFER_LOG_COST` is fixed by the [SIP-7708](./sip-7708.md) log shape (see [Transfer log cost](#transfer-log-cost)). `TX_BASE_COST` and `TX_VALUE_COST` are the residual parameters this SIP introduces.
...
A self-transfer (`tx.sender == tx.to`) charges neither the recipient touch nor the value cost: the account is already accessed and written as the sender, and SIP-7708 emits no transfer log when sender and recipient coincide. Only `TX_BASE_COST` applies.
...
Blocks of pure SIL transfers should be tested across all EL clients (e.g. on Perfnet) to confirm the pricing matches measured runtimes at the chosen throughput anchor.
