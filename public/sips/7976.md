...
- Maintain the same exemption for transactions with significant SAVM computation
...
Equivalently, the floor cost is `64` gas per calldata byte (both zero and non-zero), since `TOTAL_COST_FLOOR_PER_TOKEN * 4 = 64`. The token abstraction is retained only for consistency with [SIP-7623](./sip-7623.md).
...
The formula for determining the gas used per transaction changes from [SIP-7623](./sip-7623.md)'s implementation to:
...
With [SIP-7623](./sip-7623.md)'s implementation, data-heavy transactions cost 10/40 gas per zero/non-zero byte, reducing the maximum possible EL payload size to approximately 1.07 MB (`45_000_000/40`). This SIP further reduces this to approximately 0.67 MB (`45_000_000/64`).

By increasing calldata costs from 10/40 to 64/64 gas per byte for data-heavy transactions, this SIP provides:

- **Enhanced block size reduction**: Maximum data-heavy payload size drops by ~37%.
- **Maintained user experience**: Regular users engaging in DeFi, token transfers, and other SAVM-heavy operations remain unaffected
...
An empirical analysis summarizing the impact of this SIP can be found in [this](../assets/sip-7976/eip7976_empirical_analysis.md) report.
...
Testing for this SIP should verify the correct application of the new calldata cost floor of 64/64 gas per byte:

1. **Data-heavy transactions**: Verify transactions with minimal SAVM execution pay the floor cost of 64 gas per calldata byte
...
As the maximum possible block size is further reduced compared to [SIP-7623](./sip-7623.md), no additional security concerns are introduced beyond those already addressed in the original proposal.
