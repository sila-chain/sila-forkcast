...
Update blob maximum and target verification from [SIP-4844](./sip-4844.md).

The execution layer no longer verifies the blob maximum and receives the target dynamically from the consensus layer.

## Motivation

Following SIP-4844, the execution layer (EL) maintains a hard-coded blob target value and blob maximum value. Given the relationship
...
This SIP also changes how the EL sources the current blob target value for two reasons:

1) Gain more flexibility over the value, rather than the static `TARGET == MAX // 2` relation in SIP-4844.
...
The data facility introduced via SIP-4844 adds blobs to Sila blocks, which are simply fixed sets of data that can be
included in the canonical chain but have no execution semantics (cf. `calldata` in an Sila transaction).

The protocol specifies a maximum allowed blob count per block to prevent DoS vectors via the abuse of this data facility.
The protocol also maintains an [SIP-1559](./sip-1559.md)-like "target" value for an intended running average amount of blob throughput per
unit time. Blob usage is compared against this target to influence a "blob base fee" to administer allocation of this
resource to users of the Sila protocol.

Both of these values are currently hard-coded in the EL after SIP-4844 and the blob maximum is separately hard-coded in
the CL following SIP-4844. This SIP proposes a set of changes to uncouple these values across the CL and EL to make development
...
by SIP-4844 is unnecessary.

#### Target amount of blobs per block

The target is currently specified as a fixed value in relation to the blob count. The Sila community intends to increase
...
value should be. To address this lack of information, this SIP proposes the CL sends the current target value to the EL
...
Upon activation of this SIP, execution clients **MUST** extend the header schema with an
additional 64-bit field: the `target_blobs_per_block`. This value is set to the current target blob count. The Engine API
is modified along with this SIP to provide the `target_blobs_per_block` with each payload and implementations can use this
...
Upon activating this SIP (i.e. before processing any transactions),
the verification of the blob maximum as given in SIP-4844 can be skipped. Concretely, this means any logic relating
to `MAX_BLOB_GAS_PER_BLOCK` as given in SIP-4844 can be deprecated.
...
Otherwise, the specification of SIP-4844 is not changed. For example, blob base fee accounting and excess blob gas tracking occur in the exact same way.
