...
Introduce code validation at contract creation time for EOF formatted ([SIP-3540](./sip-3540.md))
...
Currently existing contracts require no validation of correctness and SAVM implementations can decide
...
Moreover, SAVM implementations may require fewer paths to decide which instruction is valid in
...
This feature is introduced on the same block SIP-3540 is enabled, therefore every EOF1-compatible bytecode MUST be validated according to these rules.

1. Previously deprecated instructions `CALLCODE` (0xf2) and `SELFDESTRUCT` (0xff), as well as instructions deprecated in SIP-3540, are invalid and their opcodes are undefined. (**NOTE** there are more instructions deprecated and rejected in EOF, as specced out by separate SIPs)
...
Allowing implicit zero immediate data for `PUSH` instructions introduces inefficiencies to SAVM implementations without any practical use-case (the value of a `PUSH` instruction at the code end cannot be observed by SAVM). This SIP requires all immediate bytes to be explicitly present in the code.
...
The `BLOCKHASH` instruction is well replaced by the system contract introduced in [SIP-2935](./sip-2935).
...
This change poses no risk to backwards compatibility, as it is introduced at the same time SIP-3540 is. The validation does not cover legacy bytecode (code which is not EOF formatted).

## Test Cases

### Contract creation

Each case should be tested by submitting an EOF container to EOF contract creation (as specced out in a separate SIP). Each case should be tested with code placed in code sections at different indices.
...
```python
# The ranges below are as specified by Execution Specs for SilaShanghai.
...
# Drop the opcodes deprecated and rejected in here and in SIP-3540
rejected_in_eof = [
...
See [Security Considerations of SIP-3540](./sip-3540.md#security-considerations).
