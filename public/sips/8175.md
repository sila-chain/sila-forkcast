...
This SIP introduces a new [SIP-2718](./sip-2718.md) transaction type with [SIP-1559](./sip-1559.md) gas fields, typed `capabilities` (CALL and CREATE) that define the transaction's operations, a separate `signatures` list for authentication, a `fee_auth` field that executes account code to sponsor gas, and four new opcodes — `RETURNETH`, `SIG`, `SIGHASH`, and `TX_GAS_LIMIT` — for fee delegation and in-SAVM transaction introspection.

## Motivation

Each Sila upgrade has introduced new transaction types for new capabilities: [SIP-1559](./sip-1559.md) for priority fees, [SIP-4844](./sip-4844.md) for blobs, and [SIP-7702](./sip-7702.md) for authorizations. Both [SIP-4844](./sip-4844.md) and [SIP-7702](./sip-7702.md) extend and reuse mostly the fields from [SIP-1559](./sip-1559.md), yet each required an entirely new transaction type. This leads to linear growth of transaction types with overlapping gas-payment semantics. This SIP proposes a single extensible transaction format where new features are added as typed capabilities without defining new transaction types.

Transaction sponsorship — where a third party pays gas on behalf of the sender — has been a long-sought feature. [SIP-8141](./sip-8141.md) proposes a solution using execution frames, new opcodes (`APPROVE`, `TXPARAM`), and per-frame gas budgets. This SIP achieves sponsorship with a simpler `fee_auth` field and four focused opcodes.
...
A new [SIP-2718](./sip-2718.md) transaction is introduced where the `TransactionType` is `COMPOSABLE_TX_TYPE` and the `TransactionPayload` is the RLP serialization of:
...
The fields `chain_id`, `nonce`, `max_priority_fee_per_gas`, `max_fee_per_gas`, and `gas_limit` follow the same semantics as [SIP-1559](./sip-1559.md).

The `fee_auth` field is either empty (zero bytes) or a 20-byte address. When non-empty, it designates the account whose code is executed to provide SIL covering the transaction fee (see [Fee Delegation via fee_auth](#fee-delegation-via-fee_auth)).

The `capabilities` field is an RLP list of typed capabilities. Each capability is an RLP list whose first element is the capability type identifier. The transaction MUST contain at least one capability. See [Capability types](#capability-types) for the defined types. Future SIPs MAY define additional capability types for this transaction format.
...
| `value` | SIL value in wei to transfer |
| `data` | Calldata bytes |
...
| --- | --- |
| `cap_type` | `0x02` |
| `value` | SIL value in wei to endow the new contract |
...
The `s` value MUST be less than or equal to `secp256k1n/2`, as specified in [SIP-2](./sip-2.md). The signer address is recovered via `ecrecover(sig_message, y_parity, r, s)`.
...
When `fee_auth` is empty, gas is charged directly from the sender's balance, following standard [SIP-1559](./sip-1559.md) behavior.
...
Returns SIL from the current execution context to the parent (calling) context.
...
- Credits `value` wei to the parent calling context. If the parent context is a contract call, the SIL is credited to the caller's account balance. If the parent context is the protocol-level `fee_auth` prelude, the SIL is credited to the transaction-scoped fee escrow.
- If `ADDRESS` has insufficient balance, execution **reverts**.
- `RETURNETH` is valid in any execution context (not restricted to `fee_auth`), including descendant calls. When used in nested calls within `fee_auth`, the SIL propagates upward: each `RETURNETH` credits the immediate parent, and the top-level `fee_auth` frame's `RETURNETH` credits the fee escrow.
...
Rather than defining a new transaction type per feature (blobs, authorizations, sponsorship), each potentially combined with each other, this SIP defines a single extensible format. New features are expressed as capabilities within the existing type.
...
Requiring `fee_auth` to escrow `gas_limit * max_fee_per_gas` before main execution avoids the chicken-and-egg problem of paying for execution with the proceeds of that execution. Surplus is refunded to `fee_auth` after settlement, mirroring [SIP-1559](./sip-1559.md) reserve-and-refund discipline.
...
`RETURNETH` provides a safe, explicit mechanism for code to return SIL to the parent calling context. When used within the `fee_auth` prelude, SIL propagates up to the protocol-managed fee escrow. When used in regular calls, SIL is credited to the caller's account balance. This general-purpose design avoids the need for `SELFDESTRUCT` or value-carrying calls back to a system address, and enables composable SIL flows beyond fee delegation.

### `SIG` and `SIGHASH` opcodes

These opcodes enable in-SAVM signature introspection. The `fee_auth` code loads signatures via `SIG`, obtains the base hash via `SIGHASH`, reconstructs `sig_message`, and verifies signatures using `ecrecover`. This allows arbitrary sponsor authorization without off-chain coordination beyond collecting signatures.
...
This SIP introduces a new transaction type and does not modify the behavior of existing transaction types. No backward compatibility issues are expected.
...
The `fee_auth` execution is bounded by `gas_limit` and runs before the main transaction. If `fee_auth` reverts or credits insufficient SIL, the transaction is invalid and produces no state changes. The upfront escrow requirement (`gas_limit * max_fee_per_gas`) ensures the protocol never under-collects fees.

### `fee_auth` griefing

A `fee_auth` contract could consume gas without crediting sufficient SIL, making the transaction invalid. Block builders can simulate `fee_auth` before inclusion to avoid wasting block space. Mempool nodes SHOULD simulate the `fee_auth` prelude before accepting the transaction.

### `RETURNETH` restrictions

`RETURNETH` credits SIL to the immediate parent calling context — either the caller's account balance or the protocol fee escrow at the top-level `fee_auth` frame. It cannot send SIL to arbitrary addresses. It is invalid in `STATICCALL` contexts. If the enclosing call reverts, the credit is rolled back, preventing double-spending.

### Transaction propagation

Composable transactions with `fee_auth` introduce validation complexity similar to [SIP-7702](./sip-7702.md) and [SIP-8141](./sip-8141.md). Nodes SHOULD keep at most one pending Composable transaction per sender in the public mempool. `fee_auth` simulation during mempool validation SHOULD be bounded in gas and restricted in state access patterns to limit DoS surface.
