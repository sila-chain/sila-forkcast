...
This proposal creates a precompiled contract that performs signature verifications using the Falcon-512 signature scheme by given parameters of the message hash, signature, and public key. This allows any EVM chain -- principally Sila roll-ups -- to integrate this precompiled contract easily.
...
* An EVM-friendly version where the hash function is efficiently computed in the Sila Virtual Machine.
...
The hash-to-point function computes extendable Output from a hash Function (XOF). This SIP provides two instantiations of a XOF:
    * SHAKE256 is the XOF provided in NIST submission, a sponge construction derived from SHA256. Extracting bytes using SHAKE256 calls the `Keccak_f` permutation:
        ...
        While this construction is standardized, it is expensive when computed in the Sila Virtual Machine because `Keccak_f` has no EVM opcode.
    * Keccak-PRNG is a XOF that is build from a counter-mode PRNG based on Keccak256. Generating new chunks of bytes requires an incrementing counter.
        ...
        Precompile of `Keccak256` are available in the Sila Virtual Machine, making this XOF very efficient in the EVM.
