...
Two prior SIPs have proposed overflow detection at the EVM level:

- **[SIP-1051](./sip-1051.md): Overflow Checking for the EVM** — Introduces `ovf` and `sovf` flags set by existing arithmetic opcodes, with `OFV` (`0x0c`) and `SOVF` (`0x0d`) opcodes to check and clear them.
- **[SIP-6888](./sip-6888.md): Arithmetic Verification at EVM Level** — Introduces `carry` and `overflow` flags with `JUMPC` and `JUMPO` opcodes for conditional jumps.
