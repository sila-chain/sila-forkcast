...
The `sil` protocol defines various request and response commands that are used to exchange data between Sila nodes. For example, to ask a peer node for a specific set of headers, a node sends it the [`GetBlockHeaders`](https://github.com/sila-chain/devp2p/blob/40ab248bf7e017e83cc9812a4e048446709623e8/caps/sil.md#getblockheaders-0x03) command.

*Citing from the [`GetBlockHeaders` spec definition](https://github.com/sila-chain/devp2p/blob/40ab248bf7e017e83cc9812a4e048446709623e8/caps/sil.md#getblockheaders-0x03):*
...
The node that receives the `GetBlockHeaders` command should answer it with the [`BlockHeaders`](https://github.com/sila-chain/devp2p/blob/40ab248bf7e017e83cc9812a4e048446709623e8/caps/sil.md#blockheaders-0x04) response command accordingly.

*Citing from the [`BlockHeaders` spec definition](https://github.com/sila-chain/devp2p/blob/40ab248bf7e017e83cc9812a4e048446709623e8/caps/sil.md#blockheaders-0x04):* 

>`[blockHeader_0, blockHeader_1, ...]`

>Reply to `GetBlockHeaders`. The items in the list (following the message ID) are block
headers in the format described in the main Sila specification, previously asked for
...
This SIP proposes to change the `GetBlockHeaders` and the `BlockHeaders` command to include a `request_id`.
...
The selected request/response pair serves as an example for many similar request/response pairs in the `sil` networking protocol.

## Motivation

The lack of request identifiers in the request / response paris of the `sil` protocol puts unnecessary burden of code complexity into every Sila client. It also makes the communication slightly less efficient. Another argument can be made that the addition of request identifiers makes the protocol more aligned with the `les` protocol which **does** already defines request identifiers for each request / response pair.

## Specification

Change the following message types in the `sil` protocol:

* `GetBlockHeaders (0x03)`
   * **Current (sil/65):** `[block: {P, B_32}, maxHeaders: P, skip: P, reverse: P in {0, 1}]`
   * **Then (sil/66)**: `[request_id: P, [block: {P, B_32}, maxHeaders: P, skip: P, reverse: P in {0, 1}]]`
* `BlockHeaders (0x04)`
   * **Current (sil/65):** `[blockHeader_0, blockHeader_1, ...]`
   * **Then (sil/66)**: `[request_id: P, [blockHeader_0, blockHeader_1, ...]]`
* `GetBlockBodies (0x05)`
   * **Current (sil/65):** `[hash_0: B_32, hash_1: B_32, ...]`
   * **Then (sil/66)**: `[request_id: P, [hash_0: B_32, hash_1: B_32, ...]]`
* `GetPooledTransactions (0x09)`:
   * **Current (sil/65)**`[hash_0: B_32, hash_1: B_32, ...]`
   * **Then (sil/66)**`[request_id: P, [hash_0: B_32, hash_1: B_32, ...]]`
* `PooledTransactions (0x0a)`:
   * **Current (sil/65)**`[[nonce: P, receivingAddress: B_20, value: P, ...], ...]`
   * **Then (sil/66)**`[request_id: P, [[nonce: P, receivingAddress: B_20, value: P, ...], ...]]`
* `BlockBodies (0x06)`
   * **Current (sil/65):** `[hash_0: B_32, hash_1: B_32, ...]`
   * **Then (sil/66)**: `[request_id: P, [hash_0: B_32, hash_1: B_32, ...]]`
* `GetNodeData (0x0d)`
   ...
   * **Then (sil/66)**: `[request_id: P, [hash_0: B_32, hash_1: B_32, ...]]`
* `NodeData (0x0e)`
   * **Current (sil/65):** `[value_0: B, value_1: B, ...]`
   * **Then (sil/66)**: `[request_id: P, [value_0: B, value_1: B, ...]]`
* `GetReceipts (0x0f)`
   * **Current (sil/65):** `[blockHash_0: B_32, blockHash_1: B_32, ...]`
   * **Then (sil/66)**: `[request_id: P, [blockHash_0: B_32, blockHash_1: B_32, ...]]`
* `Receipts (0x10)`
   * **Current (sil/65):** `[[receipt_0, receipt_1], ...]`
   * **Then (sil/66)**: `[request_id: P, [[receipt_0, receipt_1], ...]]`
...
This SIP extends the `sil` protocol in a backwards incompatible way and requires rolling out a new version, `sil/66`. However, `devp2p` supports running multiple versions of the same wire protocol side-by-side, so rolling out `sil/66` does not require client coordination, since non-updated clients can keep using `sil/65`.

This SIP does not change the consensus engine, thus does *not* require a hard fork.
