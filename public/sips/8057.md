...
The mechanism builds on block-level access lists ([SIP-7928](./sip-7928.md)), which are committed in prior blocks. This allows a newly synced node to correctly price the first block it validates without replaying historical execution.
...
This proposal narrows that gap in a deterministic and consensus-safe way. It introduces a smoothly decaying, per-item discount based solely on *publicly observable prior-block activity* recorded in block-level access lists ([SIP-7928](./sip-7928.md)). That achieves three effects:
...
In short, 8057 aligns Sila's gas economics with real execution cost, using already-available data (BALs) to make temporal locality explicit, deterministic, and beneficial.
...
Discount caps (this SIP treats maximum as full cold surcharge, minimum as zero):
...
This SIP applies the discount to the **first access in a transaction** of:
...
Access lists per [SIP-2930](./sip-2930.md) remain effective and, when present, pre-warm the listed addresses and storage keys at the start of the transaction. For items that are pre-warmed by an access list, this temporal discount does not apply.
...
![Gas discount percentage graph](../assets/sip-8057/gas_discount_percentage.png)
...
Subsequent accesses to the same `ITEM` within the **same transaction** are warm as per existing rules. This SIP does not introduce block-level warming across transactions.

### Initial sync and pricing with block-level access lists

This SIP requires block-level access lists in blocks. Each block carries commitments to the sets of accounts and storage keys accessed during that block.
...
Builder rotation and similar operational concerns are out of scope for this SIP.
...
This SIP does not introduce intra-block warming across transactions. If a block-level warming proposal is active, its semantics are unchanged. The temporal discount defined here applies only to the first access to an item in a transaction based on prior-block history.

## Backwards Compatibility

This SIP changes gas charging and therefore requires a hard fork. It only decreases or equalises costs for affected opcodes and does not introduce new failure modes for existing contracts. Warm and cold cost upper bounds do not increase.
