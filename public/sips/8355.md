...
Sila needs a post-quantum signature verifier that a contract can call.
...
[SIP-8051](./sip-8051.md) also proposes ML-DSA verification, but two of its
choices narrow what can be verified. This proposal differs from it in these two
ways.

First, this SIP covers NIST security levels III and V in addition to level II.
SIP-8051 specifies only ML-DSA-44, which targets 128-bit classical security. An
...
total input size, where SIP-8051 fixes the message at a 32-byte pre-hash. ML-DSA
...
as a zero address, and secp256r1 verification ([SIP-7951](./sip-7951.md))
...
The layout also matches how a witness arrives. Under [SIP-8141](./sip-8141.md)
...
Accepting a message of any length, rather than the fixed 32 bytes of SIP-8051,
...
SIP-8051 tradeoff deliberately. SIP-8051 accepts a 20512-byte key with the
...
The base costs are higher than the 4500 gas SIP-8051 charges because these
...
in a separate [file](../assets/sip-8355/test-vectors.json); the table below
...
message. An authentication contract built on [SIP-8141](./sip-8141.md) satisfies
