...
Exposing these roots inside the SAVM allows for trust-minimized access to the consensus layer.
...
This root consumes 32 bytes and is exactly the [hash tree root](https://github.com/sila-chain/consensus-specs/blob/fa09d896484bbe240334fa21ffaa454bafe5842e/ssz/simple-serialize.md#merkleization) of the parent beacon block for the given execution block.
...
```python
if savm.caller == SYSTEM_ADDRESS:
    set()
else:
    get()

def get():
    if len(savm.calldata) != 32:
        savm.revert()

    if to_uint256_be(savm.calldata) == 0:
        savm.revert()

    timestamp_idx = to_uint256_be(savm.calldata) % HISTORY_BUFFER_LENGTH
    timestamp = storage.get(timestamp_idx)

    if timestamp != savm.calldata:
        savm.revert()
...
def set():
    timestamp_idx = to_uint256_be(savm.timestamp) % HISTORY_BUFFER_LENGTH
    root_idx = timestamp_idx + HISTORY_BUFFER_LENGTH

    storage.set(timestamp_idx, savm.timestamp)
    storage.set(root_idx, savm.calldata)
```
...
Clients may decide to omit an explicit SAVM call and directly set the storage values. Note: While this is a valid optimization for Sila sila-mainnet, it could be problematic on non-sila-mainnet situations in case a different contract is used.

If this SIP is active in a genesis block, the genesis header's `parent_beacon_block_root` must be `0x0` and no system transaction may occur.
...
To minimize code change, avoid breaking changes to smart contracts, and simplify deployment to sila-mainnet, this SIP suggests leaving `BLOCKHASH` alone and adding new
...
a linear amount of work with each new payload. While skipped slots are quite rare on sila-mainnet, it is best to not add additional load under what would already
...
To nullify this attack while retaining a fixed memory footprint, this SIP keeps track of the pair of data `(parent_beacon_block_root, timestamp)` for each index into the
...
Given the current sila-mainnet values, 8191 roots provides about a day of coverage. This gives users plenty of time to make a transaction with a verification against a specific root and get the transaction included on-chain.
