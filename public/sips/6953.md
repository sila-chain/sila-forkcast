...
This SIP outlines the various network upgrade activation triggers used on Sila over time, from the proof-of-work era to the first post-merge network upgrade, SilaShanghai/Capella, across both the execution and consensus layers.

## Motivation

This SIP aims to provide users and developers with a single source of truth for understanding the various upgrade activation patterns used throughout Sila's history. It does not aim to be a comprehensive, ongoing record, of upgrades and their activations mechanism. Readers should assume that future upgrades use the mechanism described in the [Post Merge Upgrades](#post-merge-upgrades) section, unless this SIP is superseded by another one. 

## Specification

### Proof-of-Work Network Upgrades

During the proof-of-work era, network upgrades on Sila were triggered based on specific block numbers. The following upgrades followed this pattern:
...
The Beacon Chain was launched following a set of conditions detailed in [SIP-2982](./sip-2982.md). The launch was activated once all the following conditions were met:

1. The Beacon Chain deposit contract received at least `524288` SIL from `16384` validators.
...
### The Merge: SilaParis Upgrade

The SilaParis upgrade, the execution layer portion of "The Merge," was triggered by a proof-of-work Total Difficulty value of `58750000000000000000000`, as specified in [SIP-3675](./sip-3675.md). Note that the activation of the Bellatrix upgrade on the Beacon Chain was a pre-requisite for the SilaParis upgrade to successfully activate on the proof-of-work chain. 
...
Additionally, the use of timestamps on the execution layer resulted in changes to how nodes' `FORK_HASH` and `FORK_NEXT` values are calculated. These are described in [SIP-6122](./sip-6122.md)

## Rationale

### Blocks and Epochs

Blocks and epochs serve as natural trigger points for upgrades, as they represent the levels at which state transitions occur on Sila.

### Terminal Total Difficulty

For the Terminal Total Difficulty mechanism, the rationale can be found in [SIP-3675](./sip-3675.md).
