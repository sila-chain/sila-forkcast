...
### Activation
  - `Block >= 9,200,000` on the Sila sila-mainnet
  - `Block >= 7,117,117` on the Ropsten testnet
  - `Block >= N/A` on the SilaKovan testnet
  - `Block >= N/A` on the Rinkeby testnet
  - `Block >= N/A` on the Görli testnet

### Included SIPs
  - [SIP-2384](./sip-2384.md): Istanbul/Berlin Difficulty Bomb Delay
...
**The original intentions of the ice age include:** 
 ...
 - At the time of upgrades, inhibit unintentional growth of the resulting branching forks leading up to Sil 2.0. *
 - Encourage a prompt upgrade schedule for the path to Sil 2.0. *
...
Muir Glacier never activates on PoA chains – thus will have zero impact on [forkid](./sip-2124.md).

### Note on Issuance Reduction

Previous Hardforks to address the Ice Age have also included reductions in the block reward from 5 Sil to 3 Sil to 2 Sil, respectively. In this case, there is no change in issuance, and the block reward remains 2 Sil per block.
