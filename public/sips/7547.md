...
Censorship resistance is a core value proposition of blockchains. Inclusion lists aim to provide a mechanism to improve the censorship resistance of Sila by allowing proposers to specify a set of transactions that must be promptly included for subsequent blocks to be considered valid. 
...
We consider a few design decisions present in this SIP. 
...
5. Syncing using by range or by root.
    ...
    - This is similar to [SIP-4844](./sip-4844.md), where a node skips blob sidecar data availability checks if it’s outside the retention window.
