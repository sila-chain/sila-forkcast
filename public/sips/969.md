...
This SIP modifies ethash in order to break ASIC miners specialized for the current ethash
mining algorithm.


## Abstract

There are companies who currently have dedicated hardware based sila miners in
production, and may be actively mining.  This SIP aims to "poison
...
design techniques. This SIP seeks explicitly to buy time during which newly-developed
...
as suggested in https://github.com/sila-chain/go-sila/issues/2333. Algorithm
should then be returned as either `"ethash"` or `"ethashV2"` according to the
`block.number >= ASIC_MITIGATION_FORK_BLKNUM` criteria.
  
## Rationale

This SIP is aimed at breaking existing ASIC-based miners via small changes to the
...
ASIC miners.  This SIP leans towards minimizing the security risks by making minimal
...
``` c
// https://sips.sila.org/EIPS/sip-969
...
TODO: will need to generate test cases for `sila/tests` repository corresponding to the consensus
changes.
