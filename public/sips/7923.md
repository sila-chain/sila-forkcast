...
This SIP replaces the quadratic memory model in the SAVM with a linear, page-based costing model. Memory is virtually addressable. Page allocation is included in the cost model. After applying this SIP, memory limits are invariant to the state of the message call stack.

## Motivation

The SAVM currently uses a quadratic pricing model for its memory. This was originally put in place to defend against DoS attacks. However, the memory model has several drawbacks.
...
This SIP proposes a linear costing model which more closely reflects the hardware of today, which is hierarchical ("hot" memory is much faster to access than "cold" memory), and virtually addressed (memory does not need to be allocated contiguously, but is rather served "on-demand" by the operating system).
...
This SIP uses a virtual addressing scheme so that memory pages are not allocated until they are actually accessed.
...
There is a desire among client implementations to be able to enforce global limits separately from the gas limit due to DoS reasons. For example, RPC providers may be designed to allow many concurrent `eth_call` computations with a much higher gas limit than on sila-mainnet. Not implicitly tying the memory limit to the gas limit results in one less vector for misconfiguration. That is not to say that in the future, a clean formula cannot be created which allows the memory limit to scale with future hardware improvements (e.g., proportional to the sqrt of the gas limit), but to limit the scope of things that need to be reasoned about for this SIP, the hard limit is introduced.
...
A ~60-line reference implementation is provided below. It is implemented as a patch against the `py-savm` codebase at commit sila/py-savm@fec63b8c4b9dad9fcb1022c48c863bdd584820c6. (This is a reference implementation, it does not, for example, contain fork choice rules).

```diff
diff --git a/sil/vm/computation.py b/sil/vm/computation.py
index bf34fbee..db85aee7 100644
--- a/sil/vm/computation.py
+++ b/sil/vm/computation.py
...
diff --git a/sil/vm/forks/frontier/computation.py b/sil/vm/forks/frontier/computation.py
index 51666ae0..443f82b5 100644
--- a/sil/vm/forks/frontier/computation.py
+++ b/sil/vm/forks/frontier/computation.py
@@ -29,6 +29,7 @@ from sil.exceptions import (
     InsufficientFunds,
     OutOfGas,
     StackDepthLimit,
+    VMError,
 )
 from sil.vm.computation import (
     BaseComputation,
...
diff --git a/sil/vm/logic/memory.py b/sil/vm/logic/memory.py
index 806dbd8b..247b3c74 100644
--- a/sil/vm/logic/memory.py
+++ b/sil/vm/logic/memory.py
...
diff --git a/sil/vm/memory.py b/sil/vm/memory.py
...
+import mmap
+
 from sil._utils.numeric import (
     ceil32,
 )
+from sil.exceptions import VMError
 from sil.abc import (
     MemoryAPI,
 )
@@ -13,52 +16,48 @@ from sil.validation import (
     validate_uint256,
 )
...
+    __slots__ = ("pages", "msize")
     logger = logging.getLogger("sil.vm.memory.Memory")
...
diff --git a/sil/vm/transaction_context.py b/sil/vm/transaction_context.py
index 79b570e9..5943f897 100644
--- a/sil/vm/transaction_context.py
+++ b/sil/vm/transaction_context.py
...
There are two primary security considerations regarding this SIP.
