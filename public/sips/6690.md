...
This SIP proposes new SAVM modular arithmetic opcodes which support operations on odd or power-of-two moduli between 3 and 2**768-1
...
Many cryptographic operations are heavily-bottlenecked by modular arithmetic.  To expand the range of cryptographic primitives that can be implemented efficiently as SAVM contracts, we propose new modular arithmetic opcodes designed for efficiency.
...
The execution state of an SAVM call frame is modified to include a mapping of `id` (a number 0-256) to a field context.  A field context comprises a modulus and an allocated space of virtual registers to perform modular arithmetic operations on.
...
New load/store opcodes copy values to and from SAVM memory and the virtual registers allocated by the active field context.
...
* Assert that the byte range`[modulus_offset, modulus_offset+modulus_size]` falls within SAVM memory.
* Load the byte range from SAVM memory, interpreting it as a big endian `modulus`.
...
* Charge SAVM memory expansion cost to expand memory by `alloc_count * element_size` bytes:.
...
Note: serialization format for values in SAVM memory: big-endian padded to `active_ctx.element_size` bytes.
...
**Description**: copies values from registers in the currently-active field context to SAVM memory.
...
* Assert that the range `[dest, dest + count * active_ctx.element_size]` falls entirely within SAVM memory.
...
**Description**: copies values from SAVM memory into registers of the currently-active field context.
...
* Else if modulus of the active context is odd:
    * charge `count * operation_cost` gas, where `operation_cost` is looked up from the multiplication cost table, given `active_ctxt.element_size`.
* Assert that `[source, source + count * active_context.element_size]` falls entirely within SAVM memory.  Interpret it as `count` number of values, asserting that each is less than the modulus and storing them in the registers `[dest, dest + count]`.

### SAVM Memory Expansion Cost Modification

When expanding SAVM memory or allocating a new field context via `SETMODX`, expansion cost will now consider the size of all allocated virtual registers in the current call frame.

## Rationale

### Separation of SAVM Memory and EVMMAX Virtual Register Space

It is assumed that optimized implementations will not store values in SAVM-compatible big-endian serialization format, but instead convert them to an internal working representation.  The costs in the spec explicitly reflect the choice of Montgomery form as an optimal internal representation.
...
For power-of-two moduli, `LOADX`/`STOREX` are assumed to copy values directly between SAVM/EVMMAX memories without measurable conversion cost to convert between SAVM big-endian serialization and whatever internal representation is used.
...
There is an implementation of this SIP in an open PR to Go Sila.
