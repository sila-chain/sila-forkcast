...
This SIP defines Sila Node Records, an open format for p2p connectivity information.

# Motivation

Sila nodes discover each other through the node discovery protocol. The purpose of
...
schemes are defined by further SIPs. The "v4" scheme is backwards-compatible with the
...
- To sign record `content` with this scheme, apply the keccak256 hash function (as used by
  the SAVM) to `content`, then create a signature of the hash. The resulting 64-byte
