...
SIP, but the choice remains open.

## Motivation

Sila's long-term goal is to let blocks be proved with validity proofs so chain
verification is as simple and fast as possible. Part of this work consists of proving the
state read during SAVM execution.
...
to touch a fresh account (the [SIP-2930](./sip-2930.md) access-list address cost. A
cold account access under [SIP-2929](./sip-2929.md) is `2600`), `12*480` is the
branch to that account, and `64k` is the [SIP-7954](./sip-7954.md) 64 KiB code size
...
### Notable changes from SIP-7864

[SIP-7864](./sip-7864.md) specifies a flat unified binary tree where every key is
...
- A 4-bit zone prefix is added to every stem. In SIP-7864 all of an account's
...
- Overflow code (chunks at index 128 and above) moves to zone `0x1` and is keyed
  ...
  leaves. SIP-7864 keys all code per account, storing a copy for every contract.
- Overflow storage (slots 64 and above) moves to the storage zone, where a stem is
  the storage high bit, a 60-bit address prefix, and a 187-bit suffix. SIP-7864
  uses `key_hash(address || tree_index)[:31]` with no per-account bucket boundary.
- `MAIN_STORAGE_OFFSET` is removed. SIP-7864 offsets main storage by `256**31` to
...
- Non-storage stems are a 4-bit zone followed by a 244-bit hash. The hash is
  truncated to 244 bits rather than SIP-7864's 248, since the zone takes the high
  4 bits of the stem.
...
![Image](../assets/sip-8297/diagram.png)
...
Partitioned Binary Tree (PBT) adopts [SIP-4762](./sip-4762.md)'s access-event framework with two required modifications:

1. Content-addressed code. [SIP-4762](./sip-4762.md) keys code-chunk access events per account at `(CODE_OFFSET + i) // STEM_SUBTREE_WIDTH`. PBT moves chunks ≥ 128 to a leaf shared by all contracts with the same `code_hash`. Access events for overflow code MUST be keyed by the `(zone, stem, sub-index)` tree-key, not by `(address, chunk)`, so that a shared chunk is charged once per block regardless of which contract triggers the access and so the witness contains one copy. Header chunks (0–127) remain per-account.

2. Branch-cost calibration. `WITNESS_BRANCH_COST (1900)` is calibrated in [SIP-4762](./sip-4762.md) to ≈13.2 gas/byte assuming a ~144-byte average branch for the SIP-7864 depth profile. PBT's branches are deeper, so we need to adjust the values.

## Rationale

This SIP defines a binary tree that starts empty. Only new state changes are stored
in it. The MPT continues to exist but is frozen, setting up a later hard fork that
migrates MPT data into the binary tree ([SIP-7748](./sip-7748.md)).
...
2. **Keccak**: already in Sila, well-studied, less efficient to prove.
3. **Poseidon2**: strong in-circuit performance, security analysis ongoing through
   the Sila Foundation (EF) cryptography initiative, needs extra specification for field encoding.
...
mechanism itself is left to a separate SIP.
...
1. Gas costs for code chunk access can affect application economics. This can be
   mitigated by raising the gas limit alongside this SIP.
2. The tree structure change makes in-SAVM proofs of historical state stop working.

The change is invisible to the SAVM. Contracts address storage by 256-bit slot
numbers through `SLOAD` and `SSTORE` and never see tree keys. Key derivation runs
inside the client, below the SAVM, exactly as the MPT already hashes slot keys and
