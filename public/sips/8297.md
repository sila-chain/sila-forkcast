...
to touch a fresh account, the [SIP-2930](./sip-2930.md) access-list address cost
(a cold access under [SIP-2929](./sip-2929.md) costs `2600`); `12*480` is the
...
![Tree structure diagram](../assets/sip-8297/diagram.png)
...
this SIP's choice of merkelization hash (see "Backwards Compatibility").
...
No key defined by this SIP resolves to any other sub-index; the remaining
sub-indices are reserved for future header fields (see "Storage layout").

### Delegation

An account whose code is an [SIP-7702](./sip-7702.md) delegation indicator, the
...
([SIP-3541](./sip-3541.md)), so an account holds one only by delegation.
...
Deleting an account ([SIP-161](./sip-161.md) state clearing, or
`SELFDESTRUCT` in the transaction that created the account, per
[SIP-6780](./sip-6780.md)) MUST remove its header leaves and its storage
...
storage (i.e., the that condition [SIP-7610](./sip-7610.md) checks before contract
...
specified in [SIP-8347](./sip-8347.md). Migration happens off the
...
This SIP defines the tree; it does not define how existing state is converted to it. Migration is specified in [SIP-8347](./sip-8347.md): the MPT state is
...
exactly while some account has that contract code, and [SIP-161](./sip-161.md)
...
as in [SIP-7864](./sip-7864.md). The only account whose `BASIC_DATA` is all
zero is the empty account of [SIP-161](./sip-161.md), with zero nonce, zero
...
with the recorded commitment. The mechanism itself is left to a separate SIP.
...
This SIP does not change the gas schedule.
