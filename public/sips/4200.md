...
Three new SAVM jump instructions are introduced (`RJUMP`, `RJUMPI` and `RJUMPV`) which encode destinations as signed immediate values. These can be useful in the majority of (but not all) use cases and offer a cost reduction.

## Motivation

A recurring discussion topic is that SAVM only has a mechanism for dynamic jumps. They provide a very flexible architecture with only 2 (!) instructions. This flexibility comes at a cost however: it makes analysis of code more complicated and it also (partially) resulted in the need to have the `JUMPDEST` marker.
...
This functionality does not preclude the SAVM from introducing other forms of control flow later on. `RJUMP`/`RJUMPI` can efficiently co-exists with a higher-level declaration of functions, where static relative jumps should be used for intra-function control flow.
...
We introduce three new instructions on the same block number [SIP-3540](./sip-3540.md) is activated on:
...
The goal was not to completely replace the current control flow system of the SAVM, but to augment it. There are many cases where dynamic jumps are useful, such as returning to the caller.
...
This change poses no risk to backwards compatibility, as it is introduced at the same time SIP-3540 is. The new instructions are not introduced for legacy bytecode (code which is not EOF formatted).
