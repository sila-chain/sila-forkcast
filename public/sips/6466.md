...
This SIP defines a migration process of [SIP-2718](./sip-2718.md) Recursive-Length Prefix (RLP) receipts to [Simple Serialize (SSZ)](https://github.com/sila-chain/consensus-specs/blob/b3e83f6691c61e5b35136000146015653b22ed38/ssz/simple-serialize.md)
...
This SIP defines a universal receipt format based on SSZ to address these concerns.
...
| Name | SSZ equivalent |
| - | - |
| [`Root`](https://github.com/sila-chain/consensus-specs/blob/b3e83f6691c61e5b35136000146015653b22ed38/specs/phase0/beacon-chain.md#custom-types) | `Bytes32` |
| [`ExecutionAddress`](https://github.com/sila-chain/consensus-specs/blob/b3e83f6691c61e5b35136000146015653b22ed38/specs/bellatrix/beacon-chain.md#custom-types) | `Bytes20` |
...
The `logs_bloom`, intermediate state `root` (Homestead scheme), and `cumulative_gas_used` (pre [SIP-8116](./sip-8116.md)) fields are not present in SSZ receipts.

### Execution block header changes

The [execution block header's `receipts-root`](https://github.com/sila-chain/devp2p/blob/bc76b9809a30e6dc5c8dcda996273f0f9bcf7108/caps/sil.md#block-encoding-and-validity) is transitioned from MPT to SSZ.
...
`from`, `gasUsed`, and `contractAddress` are already provided via JSON-RPC and are left unchanged. For `BasicReceipt` and `SetCodeReceipt`, `contractAddress` will be `null` as these receipt types do not contain the `contract_address` field. `logIndex` indicates the log index position in the individual receipt (rather than the entire block), as defined in [SIP-8116](./sip-8116.md).

### Consensus `ExecutionPayload` changes

When building a consensus `ExecutionPayload`, the [`receipts_root`](https://github.com/sila-chain/consensus-specs/blob/b3e83f6691c61e5b35136000146015653b22ed38/specs/deneb/beacon-chain.md#executionpayload) is now based on the `Receipt` type, changing the type of `receipts_root` from an MPT [`Hash32`](https://github.com/sila-chain/consensus-specs/blob/b3e83f6691c61e5b35136000146015653b22ed38/specs/phase0/beacon-chain.md#custom-types) to an SSZ [`Root`](https://github.com/sila-chain/consensus-specs/blob/b3e83f6691c61e5b35136000146015653b22ed38/specs/phase0/beacon-chain.md#custom-types).
...
All receipts share the same Merkle tree shape with a stable [generalized index (gindex)](https://github.com/sila-chain/consensus-specs/blob/b5c3b619887c7850a8c1d3540b471092be73ad84/ssz/merkle-proofs.md#generalized-merkle-tree-index) assigned to each field. Future transaction features can introduce additional receipt fields or drop existing fields without breaking verifiers.

### Verifier improvements

Committing to `from`, `contract_address` and `authorities` in the receipt allows efficient verification without the expensive `ecrecover` mechanism. This allows future SIPs to change how these addresses are computed without breaking verifiers, e.g., when future signature schemes are introduced.
