...
Empirical analysis reveals that approximately 27,869 bytecodes existed in full-synced node databases with no live account pointing to them (as of the SilaCancun fork). A database lookup `CodeExists(hash)` would yield different results on different nodes, causing different gas costs and breaking consensus.
...
This proposal leverages the existing SIP-2930 access list structure without any modifications. No new fields or protocol changes are required.
...
- The cost of reading `codeHash` for access-listed addresses is already covered by SIP-2929/2930 access costs (intrinsic access-list cost and cold→warm state access charges).
...
for tuple in tx.access_list:
    warm(tuple.address)  # per SIP-2930/SIP-2929 rules
...
Builds on SIP-2930 access lists and SIP-2929 access costs, requiring minimal protocol changes.
...
- No changes to transaction structure - uses existing SIP-2930 access lists
...
- The cost of reading `codeHash` is already covered by SIP-2929/2930
