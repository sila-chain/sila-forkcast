...
Operations in the Legacy SAVM trim off the top 12 bytes of an address operand before evaluation. This
SIP changes the handling of those opcodes within EOF so that no trimming occurs and the top twelve
bytes need to be zero or an exceptional halt is raised.

## Motivation

There have been proposals to extend Sila Addresses from 160 bits to 256, such as one that would
use the extra bits for state expiry (such as the sila magicians forum topic "Increasing the
address size from 20 to 32 bytes"). One issue ground the work to a halt: SAVM opcodes that accept
addresses trim all but the lowest 20 bytes out from the operand before processing. SAVM Reference
tests verify this behavior in the 'stBadOpcode/invalidAddr.json' tests.

The SAVM Object Format presents an opportunity to remove this address masking in a backwards
...
in [SIP-7069](./sip-7069.md). That leaves only one operation, `BALANCE`, with issues.
...
operation specified in [SIP-2929](./sip-2929.md). This means if the account is not
...
all other operands described in [SIP-2929](./sip-2929).
...
It is anticipated that future SIPs will alter the behavior in a way that will not result in an
...
Future SIPs should be mindful when adding new operation with an address operand or parameter to how
it interacts with EOF and legacy contracts. Authors should ensure that such additions remain in
harmony with this SIP.
...
This SIP only defines a revert behavior for previously stripped addresses. Compilers will need to be
