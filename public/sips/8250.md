...
Replaces the single sender nonce of an [SIP-8141](./sip-8141.md) frame transaction with `(nonce_keys, nonce_seq)`: a bounded set of nonce keys sharing one sequence number. `nonce_keys == [0]` aliases the legacy account nonce; each non-zero key selects an independent protocol-managed nonce sequence stored in a `NONCE_MANAGER` system contract. Transactions whose non-zero key sets do not overlap are replay-independent. This helps privacy applications use one shared sender for many users without forcing every transaction through the same nonce sequence.
...
Keyed nonces let each spend pick its own nonce domain, for example one derived from a privacy nullifier. Transactions on disjoint non-zero key sets are replay-independent. This removes the protocol-level nonce obstacle to future keyed-aware mempools that admit concurrent transactions from the same sender. This SIP preserves SIP-8141's limit of one pending frame transaction per sender in the public mempool.

Tying nonce consumption to SIP-8141's payment-approval step also gives single-use-key applications, such as nullifiers, an atomic spent-once guarantee: if validation requires every selected key to be unused, successful inclusion makes all selected keys used, regardless of whether later frames revert.

## Specification

The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "NOT RECOMMENDED", "MAY", and "OPTIONAL" in this document are to be interpreted as described in RFC 2119 and RFC 8174.

This specification is a delta against [SIP-8141](./sip-8141.md).
...
`STATE_BYTES_PER_STORAGE_SET` and `CPSB` are defined by [SIP-8037](./sip-8037.md). SIP-8037 currently sets `STATE_BYTES_PER_STORAGE_SET = 64` and `CPSB = 1530`, so each fresh key costs `97920` state gas.
...
`NONCE_MANAGER` MUST be selected so that no code or storage exists at the address on every intended activation network at fork-configuration finalization. If the selected address has non-empty code or non-empty storage on any intended activation network at fork-configuration finalization, a different address MUST be selected. This SIP does not define a normal activation transition that clears non-empty storage or overwrites non-empty code at `NONCE_MANAGER`.

### Transaction payload

This SIP replaces the `nonce` field in the SIP-8141 `FRAME_TX_TYPE` payload with two consecutive fields, `nonce_keys` followed by `nonce_seq`. No other payload field is changed. Applied to the SIP-8141 payload, the result is:
...
`nonce_keys` is a list of `uint256` values; `nonce_seq` is a `uint64`. Each nonce key and `nonce_seq` MUST be encoded as canonical minimal-length RLP integers. The frame layout and all other SIP-8141 transaction fields are unchanged.

The SIP-8141 signature hash procedure applies after this replacement. The signing payload contains `nonce_keys` and `nonce_seq` at the position previously occupied by `nonce`.
...
`nonce_calldata` is priced exactly as SIP-8141 prices frame and signature data under [SIP-7623](./sip-7623.md):

- add `nonce_calldata_cost` to `standard_gas_limit`;
- add `nonce_calldata_tokens` to `calldata_tokens`.

The SIP-8141 definitions of `calldata_floor_gas`, `max_gas`, `gas_used`, and fee settlement remain unchanged and use these updated quantities.

Decoders MUST reject a `FRAME_TX_TYPE` transaction to which this SIP applies if any of the following is true:

- the payload does not match the SIP-8141 schema after replacing `nonce` with consecutive fields `nonce_keys` and `nonce_seq`;
...
This check occurs at the same stage as SIP-8141's existing nonce check, before any frame executes.
...
SIP-8141's payment-approval transition currently increments the sender's account nonce. This SIP replaces that increment with:
...
For `nonce_keys == [0]`, `increment_account_nonce(sender)` increments the sender's current account nonce; it does not set the account nonce to `tx.nonce_seq + 1`. This preserves SIP-8141 behavior if an earlier frame in the same transaction has already changed the sender's legacy nonce, for example by account deployment or by executing `CREATE` or `CREATE2` at `tx.sender`.
...
If the enclosing SIP-8141 frame reverts or halts exceptionally, clients MUST revert the transition. Once that frame succeeds, a later frame failure or atomic batch rollback MUST NOT revert it unless the transaction is invalid under SIP-8141. Clients MUST discard all effects of an invalid transaction.
...
Keyed-nonce reads and writes performed by stateful validity and `consume_nonce_set` are protocol bookkeeping: they do NOT add `NONCE_MANAGER` or its slots to [SIP-2929](./sip-2929.md) `accessed_addresses` or `accessed_storage_keys`, are NOT charged under [SIP-2200](./sip-2200.md) `SSTORE` pricing, and do NOT warm the address or slot for later user-level access.

### Transaction introspection

`TXPARAM(0x01)` returns `tx.nonce_seq`. SIP-8141 assigns `TXPARAM` indices through `0x0C`, including `0x0B = len(signatures)`. This SIP adds four non-conflicting indices:
...
This SIP MUST activate at or after [SIP-8141](./sip-8141.md).
...
For the SIP-8141 public mempool rules:

- replace the pending transaction identity `(sender, nonce)` with `(sender, nonce_keys, nonce_seq)`;
- replace every SIP-8141 public mempool dependency on the sender nonce with `current_nonce_seq(sender, nonce_key)` for each `nonce_key` in `nonce_keys`.
...
For transactions under this SIP, SIP-8141 Structural Rule 6 also allows a `VERIFY` frame to use state gas when a payment-scoped `APPROVE` creates a keyed nonce slot. That state gas draws from the frame's `limits.state` and does not count toward `MAX_VERIFY_GAS`. The existing `MAX_VERIFY_STATE_GAS` cap on the sum of state limits in the validation prefix is unchanged.

All other SIP-8141 public mempool rules, including the limit of one pending frame transaction per sender, remain unchanged.
...
`nonce_keys == [0]` aliases the legacy account nonce so SIP-8141's existing replay behavior is preserved. Account deployment, `CREATE`, and `CREATE2` continue to affect account nonces by ordinary EVM rules. Only payment-approval nonce-bumping is replaced.

`nonce_keys` are `uint256` values because privacy protocols often derive keys from 32-byte nullifiers, commitments, or hash outputs. [SRC-4337](./sip-4337.md) uses the same key/sequence model but has only one 32-byte nonce field, so it packs a 24-byte key and an 8-byte sequence into that field. This SIP uses explicit fields: 32-byte nonce keys and an 8-byte `nonce_seq`. That keeps SRC-4337's 64-bit sequence width, avoids truncating nullifier-derived labels, and makes SRC-4337-style 24-byte keys a subset of this SIP's key space.

This SIP intentionally uses one `nonce_seq` shared by all selected `nonce_keys`, rather than a vector of `(nonce_key, nonce_seq)` pairs. A shared sequence preserves the scalar replay semantics of SIP-8141 and keeps transaction introspection simple. Strictly increasing `nonce_keys` give each selected key set one canonical transaction encoding and one canonical hash. Applications that require single-use capabilities, including privacy nullifiers and expiring authorizations, can derive distinct nonce keys and use `nonce_seq == 0`. Applications that require atomic advancement of unrelated nonce domains at unrelated sequence numbers are outside the scope of this SIP and may be addressed by a future extension.
...
The first use of a keyed nonce creates one storage slot, so it pays SIP-8037's state gas for one new slot. Later uses update the same slot and do not pay the creation charge again.

This SIP does not relax SIP-8141's public-mempool one-pending-frame-transaction-per-sender guidance, but it removes the protocol-level obstacle to doing so. A future policy MAY admit multiple pending frame transactions for the same sender on disjoint non-zero key sets, subject to its own dependency-tracking and replacement rules.

## Backwards Compatibility

`nonce_keys == [0]` preserves SIP-8141 replay behavior. Legacy account objects, non-frame transaction types, and `eth_getTransactionCount` are unchanged.
...
If activated after SIP-8141, pre-fork frame transactions become invalid at the fork boundary and any authorization bound to the pre-fork signature hash MUST be regenerated.

## Security Considerations

Replay protection is scoped to `(sender, nonce_keys, nonce_seq)`, with validity requiring every selected key to have the selected sequence. Disjoint non-zero key sets remove only the replay-ordering dependency; transactions on disjoint keys may still conflict on sender or payer balance, contract storage, paymaster state, or any other shared state. This SIP provides replay-domain separation, not confidentiality of key selection: `nonce_keys` are visible in the payload and committed by `compute_sig_hash(tx)`.
...
Nonce consumption persists through later-frame reverts and SIP-8141 atomic-batch rollback because it is part of payment approval. Single-use applications SHOULD minimize post-approval revert paths.
...
Ordinary direct calls to `NONCE_MANAGER` revert. However, the account may still receive SIL through force-send mechanisms where applicable. Any balance held at `NONCE_MANAGER` is outside the scope of this SIP and is not recoverable by protocol logic defined here.
