...
This SIP specifies a series of new `RETURN` opcodes which allow the user to specify which data location to return from
...
inefficiencies already occur when attempting to return data already placed in storage. This SIP attempts to rectify
...
This SIP introduces 3 new opcodes as well as renaming/aliasing an existing one.
...
The `MRETURN` opcode is a rename of `RETURN`, whereby sequential bytes in memory are returned. It will operate exactly as it currenty does as of the SilaCancun hard-fork, and its gas cost will remain the same.
...
SIP-3855: Introducing `PUSH0` opcode to optimize away alternative methods of pushing 0 onto the stack.

SIP-6: Renaming `SUICIDE` to `SELFDESTRUCT` without changing functionality.
...
activated in future SAVM version once EOF has been fully implemented.
