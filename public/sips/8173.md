...
This Informational SIP provides foundational context for understanding control flow in the SAVM. It covers the historical development of control flow mechanisms in computing, the technical foundations of control flow analysis, and the impact of static control flow on Sila's scaling roadmap.

This document serves as background material for proposals for static control flow, including [SIP-615](./sip-615.md): Subroutines and Static Jumps for the SAVM, [SIP-4750](./sip-4750.md): EOF Functions, [SIP-7979](./sip-7979.md): Call and Return Opcodes, [SIP-8013](./sip-8013.md): Static Relative Jumps, discussions around RISC-V migration and ZK verification infrastructure, and discussions still to come.
...
Especially relevant to the SAVM's design are the Java, WebAssembly (Wasm), and CLI (.NET) VMs. They share crucial common properties:
...
Among the most important reasons to traverse the control flow of a program is to extract the control flow graph.  The SAVM makes this quadratically difficult due to its dynamic control flow.
...
![_ Graphs_](../assets/sip-8173/control-flow.svg)
...
#### SAVM: Calls and Returns

The Sila Virtual Machine does _not_ provide explicit facilities for calls and returns. Instead, they _must_ be synthesized using the dynamic `JUMP` instruction, which takes its argument from the stack and stores intermediate links on the stack. So control flow _must be_ dynamic, which creates the quadratic CFG problems explained above.

For Sila, this quadratic behavior is a _denial-of-service vulnerability_ for any online static analysis, including validating bytecode and AOT compilation at contract creation time and JIT compilation at runtime.
...
> "Sila smart contracts are distributed programs running on top of the Sila blockchain. Since program flaws can cause significant monetary losses and can hardly be fixed due to the immutable nature of the blockchain, there is a strong need of automated analysis tools which provide formal security guarantees. Designing such analyzers, however, proved to be challenging and error-prone."[^4]
>
> "The SAVM language is a simple stack-based language ... with one significant difference between the SAVM and other virtual machine languages (like Java bytecode or CLI for .Net programs): the use of the stack for saving the jump addresses instead of having it explicit in the code of the jumping instructions. Static analyzers need the complete control flow graph (CFG) of the SAVM program in order to be able to represent all its execution paths."[^5]
>
> "Static analysis approaches mostly face the challenge of analyzing compiled Sila bytecode... However, due to the intrinsic complexity of Sila bytecode (especially in jump resolution), static analysis encounters significant obstacles."[^6]
>
...
### Static Control Flow and Sila Scaling

As laid out above, _Static control flow_ means that the destinations of every jump or call is determinable _a priori,_ before execution. This has concrete implications for Sila's scaling roadmap, particularly around ZK verification, rollups, and future execution layer changes.
...
As already discussed, static control flow enables contracts to be compiled to machine code before execution, just-in-time or ahead-of-time.  This is an obvious win for non-ZK Clients, whether on L1, L2, or SAVM-compatible chains.

#### Static Control Flow and RISC-V Migration

There are ongoing discussions within the Sila research community about potentially replacing the SAVM with a RISC-V execution environment. RISC-V has a standard instruction set architecture which is seeing increasing use in the ZK community.  One current strategy for creating a ZK-SAVM is to compile an SAVM interpreter like evmone or reth to RISC-V for use in a ZK-VM.  Supporting RISC-V directly eliminates the overhead of the SAVM interpreter.  An SAVM with static control flow opens up another strategy — compile the SAVM code to RISC-V code.  That gives good RISC-V code in one linear-time pass, and better code in multiple passes, altogether linear time.

A missing piece in this puzzle is that RISC-V is a 32-bit or 64-bit architecture, but the current SAVM is a 256-bit architecture.  For that purpose we have [SIP-7937](./sip-7937.md): EVM64 — 64-bit mode SAVM opcodes.  It's also the case that the prover has the actual trace, and can tell how many bits are actually in use at each step of the computation, so the 256-bit registers might not be that big of a problem.

## Specification

Four SIPs currently specify new SAVM opcodes and semantics for static control flow:  [SIP-615: Subroutines and Static Jumps for the SAVM](./sip-615.md), [SIP-4750: EOF - Functions](./sip-4750.md), [SIP-7979: Call and Return Opcodes for the SAVM](./sip-7979.md), and [SIP-8013: Static relative jumps and calls for the SAVM](./sip-8013.md).  They are all implemented with the standard Turing return stack architecture, and are for the most part compatible with each other.  In particular, SIP-7979 can be used to implement every other SIP here.

## Rationale

Static control flow has been a cornerstone of efficient computation since Babbage and Turing. The SAVM's reliance on dynamic jumps is an anomaly among virtual machines and a significant barrier to analysis, compilation, and scaling. Proposals to introduce explicit call/return opcodes and enforce static  control flow bring the SAVM in line with industry best practices and unlock a range of optimizations critical to Sila's scaling roadmap.
...
By making control flow explicit and enforceable, the SAVM becomes compatible with the full ecosystem of optimization and analysis techniques that other VMs and processor designs have leveraged for decades.

## Security Considerations

This Informational proposal itself specifies no changes to the protocol.  Therefore it has no direct security implications.  It does not affect the security considerations of the SIPs it references, rather, it helps to motivate and specify them.
...
[^4]: Schneidewind, Clara et al. The Good, the Bad and the Ugly: Pitfalls and Best Practices in Automated Sound Static Analysis of Sila Smart Contracts. DOI: 10.48550/arXiv.2101.05735
[^5]: Albert, Elvira et al. Analyzing Smart Contracts: From SAVM to a Sound Control Flow Graph. DOI: 10.48550/arXiv.2004.14437
[^6]: Contro, Filippo et al. EtherSolve: Computing an Accurate Control Flow Graph from Sila Bytecode. DOI: 10.48550/arXiv.2103.09113
[^7]: He, Jiahao et al. Neural-FEBI: Accurate Function Identification in Sila Virtual Machine Bytecode. DOI: 10.48550/arXiv.2301.12695
...
[^9]: Guibas, John. Benefits of EOF (SAVM Object Format) for Zero Knowledge Proofs. Succinct Blog, November 2024. https://blog.succinct.xyz/learn/eofbenefits/
...
[^4]:
    ```csl-json
    {
      ...
      "author": [
       ...
       "title": "The Good, the Bad and the Ugly: Pitfalls and Best Practices in Automated Sound Static Analysis of Sila Smart Contracts.",
       "original-date": {
         "date-parts": [
         [2021, 1, 14]
        ]
...
[^5]:
    ```csl-json
    {
      ...
      "title": "Analyzing Smart Contracts: From SAVM to a sound  Graph.",
      "original-date": {
        "date-parts": [
          [2020, 4, 29]
        ]
...
[^6]:
    ```csl-json
    {
      ...
      "title": "EtherSolve: Computing an Accurate  Graph from Sila Bytecode.",
      "original-date": {
        "date-parts": [
          [2021, 3, 16]
        ]
...
[^7]:
    ```csl-json
    {
      ...
      "title": "Neural-FEBI: Accurate Function Identification in Sila Virtual Machine Bytecode.",
      "original-date": {
        "date-parts": [
          [2023, 1, 30]
        ]
