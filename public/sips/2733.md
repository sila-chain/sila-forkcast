...
## Motivation
Onboarding new users to Sila has been notoriously difficult due to the need
for new users to acquire enough sila to pay for their transactions. This
...
authenticate the message. This SIP aims to provide a flexible framework for
...
An important byproduct of this SIP is that it also facilitates bundling
transactions for single users.

## Specification
Introduce a new [SIP-2718](./sip-2718.md) transaction type where `id = 2`.
...
SIP has been written to reduce as much validation complexity as possible.
