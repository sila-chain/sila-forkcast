...
It depends on the two new opcodes proposed by [SIP-4200](./sip-4200.md) to support static jumps:
...
In concert with [SIP-3540](./sip-3540.md) and [SIP-3670](./sip-3670.md) it ensures, at initialization time, that valid code will not execute invalid instructions or jump to invalid locations, will not underflow stack, will maintain consistent numbers of inputs and outputs for subroutines, and will have bounded stack height in the absence of recursion.
...
Unlike these machines, the Sila Virtual Machine _does not_ provide subroutine operations. Instead, they must be synthesized using the dynamic `JUMP` instruction, which takes its destination on the stack. Further, the SAVM provides _only_ dynamic jumps, impeding the static analysis we need.
...
Static jumps, conditional jumps, and subroutines are sufficient and efficient in space and time, as shown by historical experience and as we will show for the SAVM below.

### Safe control-flow.

The SAVM has unusually high requirements for safety.  Not only do many smart contracts control inordinately large amounts of valuable Sila, but once placed on the blockchain any defects are visible to attackers and cannot be repaired.  We propose to statically validate important safety constraints on code at initialization time.

### Static control-flow.

The SAVM's dynamic jumps cause two major problems. First, the need to synthesize static jumps and subroutines with dynamic jumps wastes space and gas with needlessly complex code, as we will show below.

Worse, jumps that can dynamically branch to any destination in the code can cause quadratic "path explosions" when traversing the flow of control. For Sila, this is a denial-of-service vulnerability that prevents us, at initialization time, from validating the safe use of SAVM code and from compiling SAVM code to machine code.

We _need_ static control-flow to validate program safety and to compile SAVM bytecode to machine code -- in time and space linear in the size of the code.
...
* _The description above lays out the semantics of these instructions in terms of a `return stack`.  But the actual state of the `return stack` is not observable by SAVM code or consensus-critical to the protocol.  (For example, a node implementer may code `RJUMPSUB` to unobservably push `PC` on the `return stack` rather than `PC + 1`, which is allowed so long as `RETURNSUB` observably returns control to the `PC + 3` location.)_

### Validity

_Execution_ is defined in the Yellow Paper as a sequence of changes in the SAVM state.  The conditions on valid code are preserved by state changes.  At runtime, if execution of an instruction would violate a condition the execution is in an exceptional halting state.  The Yellow Paper defines six such states.
...
This is a purely semantic specification, placing no constraints on the syntax of code sections beyond being a sequence of opcodes and immediate data – a subroutine is not a contiguous sequence of bytecode, it is a subgraph of the bytecode's control-flow graph.  The SAVM is a simple state machine.  We only promise that valid code will not, as it were, jam up the gears of the machine.

By avoiding syntactic constraints we allow for optimizations like tail call elimination, multiple-entry subroutines, moving "cold" code out of line, and other ways of reducing redundancy and keeping "hot" code in cache.   Since we wish to support one-pass compilation of SAVM code to machine code it is crucial that the SAVM code be as well optimized as possible up front.
...
There are a few major designs for a subroutine facility, two of which are considered here.  The others are mostly not appropriate for the SAVM, such as the Wheeler Jump -- self-modifying code that writes return addresses into called subroutines.
...
*2. Keep return addresses on the data stack.*  This design is often used by register machines, including those from CDC, IBM, DEC, Intel, and ARM.  The registers are used primarily for computation, and the stack maintains call frames for return addresses, arguments, and local variables.  On the SAVM there are no registers for computation, so using the stack for both purposes can cause the sort of inefficiencies we see below.  Pascal p-code does use this design, but as part of a complex calling convention with dedicated registers.
...
* It improves efficiency:
  * it uses native arithmetic rather than 256-bit SAVM instructions for the return address,
...
These changes affect the semantics of existing SAVM code: bytes that would have been interpreted as valid jump destinations may now be interpreted as immediate data.  Since this proposal depends on the Sila Object Format to signal the change this is not a practical issue.
...
For simplicity's sake we assume that all jumpdest analysis and prior validation has been done, including SIP-3540, SIP-3670, and SIP-4200, so EOF headers and sections are well-formed, and there are no invalid instructions or jumps.  In practice, all passes of validation can be folded into a single loop no recursion.
...
These changes introduce new flow control instructions.  They do not introduce any new security considerations. This SIP is intended to improve security by validating a higher level of safety for SAVM code deployed on the blockchain.  The validation algorithm must be quasi-linear in time and space to not be a denial of service vulnerability.  The algorithm here makes one linear-time pass of the bytecode, and uses a stack of continuations that cannot exceed the number of `RJUMPI` instructions in the code.
