...
EOFv1 as scoped in [SIP-7692] removes code introspection capabilities from the SAVM, preventing EOF from reading raw code values such as code delegation designations set by [SIP-7702]. There are a number of use cases where reading the delegation designation of [SIP-7702] would allow contracts to be more proactive about user experience issues.
...
EOF code which contains this instruction prior to the fork activating this instruction is considered invalid. Beginning with the first block this SIP is activated in, this instruction is added to the set of valid EOFv1 instructions.
...
- if `target_address` has any of the high 12 bytes set to a non-zero value (i.e. it does not contain a 20-byte address), then halt with an exceptional failure
  - Notice: Future expansion of the SAVM address space may enlarge the number of valid addresses. Do not rely on this step always halting with the current restrictions.
...
- If `loaded_code` indicates a delegation designator (for example, prefixed with `0xef0100` as defined in [SIP-7702]) push the address of the designation onto the stack.
  - Notice: if [SIP-7702] delegation designations are updated in a future fork (such as allowing chained delegations), then this section is expected to comply with any such hypothetical changes. 
...
Note: This operation straddles the line between [SIP-7702] code reading and code executing instructions.  The operation steps work as a code reading instruction, but the value returned is the resolved address as through it is a code executing instruction.

## Rationale

This SIP is very similar to [SIP-7761], which introduces account type introspection. Its rationale is included by reference as they all apply to this situation.
...
`EXTCODEADDRESS` at `0xea` can be introduced in a backwards compatible manner into EOFv1 (no bump to version), because `0xea` has been rejected by EOF validation prior to the activation of this SIP, and there are no EOF contracts on-chain with an `0xea` instruction which would have their behavior altered.

## Security Considerations

[SIP-7702] code delegation is a new feature that has not been made accessible to sila-mainnet yet. SIP authors will keep abreast of any developments and reflect on their impact to this proposed instruction. 
