...
This document describes a scheme for authenticated, updateable Sila node
lists retrievable via DNS.

## Motivation

Many Sila clients contain hard-coded bootstrap node lists. Updating those
lists requires a software update. The current lists are small, giving the client
little choice of initial entry point into the Sila network. We would like to
...
also be useful to Sila peering providers because their customers can
configure the client to use the provider's list.

## Specification

A 'node list' is a list of 'node records' [as defined by SIP-778](./sip-778.md)
