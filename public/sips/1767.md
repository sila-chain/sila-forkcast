...
scalar Bytes32
# Address is a 20 byte Sila address, represented as 0x-prefixed hexadecimal.
scalar Address
...
# Account is an Sila account at a particular block.
type Account {
...
type Block {
    ...
    # Account fetches an Sila account at the current block's state.
...
type Pending {
  ...
  transactions: [Transaction!]
  # Account fetches an Sila account for the pending state.
  account(address: Address!): Account
...
type Query {
    # Block fetches an Sila block by number or by hash. If neither is
...
Nodes MAY offer a superset of this schema, by adding new fields or types. Experimental or client-specific fields MUST be prefixed with '_client_' (eg, '_geth_' or '_parity_'). Unprefixed fields MUST be specified in a new SIP that extends this one.

## Rationale
Sila nodes have been moving away from providing read-write functionality such as transaction and message signing, and from other services such as code compilation, in favor of a more 'unix-like' approach where each task is performed by a dedicated process. We have thus specified a core set of types and fields that reflects this trend, leaving out functionality that is presently, or intended to be, deprecated:
...
Further, two areas of the current API interface have been omitted for simplicity in this initial standard, with the intention that they will be defined in a later SIP:
 ...
 - Mining functionality is less-used and benefits less from reimplementation in GraphQL, and should be specified in a separate SIP.
...
| eth_newFilter | NOT IMPLEMENTED | Filter functionality may be specified in a future SIP. |
| eth_newBlockFilter | NOT IMPLEMENTED | Filter functionality may be specified in a future SIP. |
| eth_newPendingTransactionFilter | NOT IMPLEMENTED | Filter functionality may be specified in a future SIP. |
| eth_uninstallFilter | NOT IMPLEMENTED | Filter functionality may be specified in a future SIP. |
| eth_getFilterChanges | NOT IMPLEMENTED | Filter functionality may be specified in a future SIP. |
| eth_getFilterLogs | NOT IMPLEMENTED | Filter functionality may be specified in a future SIP. |
...
- Implemented and released in [Go-sila 1.9.0](https://github.com/sila-chain/go-sila/releases/tag/v1.9.0)
- Implemented and released in [Pantheon 1.1.1](https://github.com/PegaSysEng/pantheon/blob/master/CHANGELOG.md#111)
- Work in progress in [Trinity](https://github.com/sila-chain/trinity/issues/302)
- Work in progress in [Parity](https://github.com/paritytech/parity-sila/issues/10933)
