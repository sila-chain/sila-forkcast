...
In particular, at address zero there is approximately 17 billion USD in tokens and sila, according to silascan.
...
It is a common mistake to send [SRC-20](./sip-20.md) tokens to the token address instead of the intended recipient.
...
Many scammers have fraudulently received tokens and SIL via trust-trading.
Their victims currently have no way to recover their funds.
`SUDO` fixes this.

Large amounts of users have accidentally locked up tokens and sila by losing their private keys.
...
Finally, there are many tokens and sila sitting in smart contracts locked due to a bug.
We could finally close SIP issue #156.
We cannot currently reclaim sila from stuck accounts.
`SUDO` fixes this.
...
If the call ended with `REVERT`, `INVALID`, or an SAVM assertion, `0` is pushed.
