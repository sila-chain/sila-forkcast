...
Provide an efficient SAVM instruction for copying memory areas.

## Motivation

Memory copying is a basic operation, yet implementing it on the SAVM comes with overhead.
...
the reduction by [SIP-2929](./sip-2929.md) made it slightly more economical.
...
- at least 757 gas pre-SIP-2929 using the identity precompile
- at least 157 gas post-SIP-2929 using the identity precompile
- at least 96 gas using unrolled `MLOAD`/`MSTORE` instructions
- 27 gas using this SIP
...
With [SIP-2929](./sip-2929.md) the call overhead using the identity precompile was reduced from 700 to 100 gas.
This is still prohibitive for making the precompile a reasonable alternative again.

## Backwards Compatibility

This SIP introduces a new instruction which did not exist previously. Already deployed contracts using this instruction could change their behaviour after this SIP.
...
A full suite of tests can be found in the execution spec tests: [MCOPY suite](https://github.com/sila-chain/execution-spec-tests/tree/c0065176a79f89d93f4c326186fc257ec5b8d5f1/tests/cancun/eip5656_mcopy).
