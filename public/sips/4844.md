...
accessed by SAVM execution, but whose commitment can be accessed.
The format is intended to be fully compatible with the format that will be used in full sharding.

## Motivation

Rollups are in the short and medium term, and possibly in the long term, the only trustless scaling solution for Sila.
...
Rollups are significantly reducing fees for many Sila users: Optimism and Arbitrum frequently provide fees that are ~3-8x lower than the Sila base layer itself,
...
This SIP provides a stop-gap solution until that point by implementing the _transaction format_ that would be used in sharding,
...
Compared to full data sharding, this SIP has a reduced cap on the number of these transactions that can be included, corresponding to a target of ~0.375 MB per block and a limit of ~0.75 MB.
...
| `HASH_OPCODE_GAS` | `3` |
| [`MIN_EPOCHS_FOR_BLOB_SIDECARS_REQUESTS`](https://github.com/sila-chain/consensus-specs/blob/4de1d156c78b555421b72d6067c73b614ab55584/configs/sila-mainnet.yaml#L148) | `4096` |
...
Throughout this proposal we use cryptographic methods and classes defined in the corresponding [consensus 4844 specs](https://github.com/sila-chain/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/deneb).

Specifically, we use the following methods from [`polynomial-commitments.md`](https://github.com/sila-chain/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/deneb/polynomial-commitments.md):

- [`verify_kzg_proof()`](https://github.com/sila-chain/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/deneb/polynomial-commitments.md#verify_kzg_proof)
- [`verify_blob_kzg_proof_batch()`](https://github.com/sila-chain/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/deneb/polynomial-commitments.md#verify_blob_kzg_proof_batch)
...
We introduce a new type of [SIP-2718](./sip-2718.md) transaction, "blob transaction", where the `TransactionType` is `BLOB_TX_TYPE` and the `TransactionPayload` is the RLP serialization of the following `TransactionPayloadBody`:

```
[chain_id, nonce, max_priority_fee_per_gas, max_fee_per_gas, gas_limit, to, value, data, access_list, max_fee_per_blob_gas, blob_versioned_hashes, y_parity, r, s]
```

The fields `chain_id`, `nonce`, `max_priority_fee_per_gas`, `max_fee_per_gas`, `gas_limit`, `value`, `data`, and `access_list` follow the same semantics as [SIP-1559](./sip-1559.md).
...
We introduce blob gas as a new type of gas. It is independent of normal gas and follows its own targeting rule, similar to SIP-1559.
...
The `sila/consensus-specs` repository defines the following consensus layer changes involved in this SIP:
...
Blob transactions have two network representations. During transaction gossip responses (`PooledTransactions`), the SIP-2718 `TransactionPayload` of the blob transaction is wrapped to become:
...
- `tx_payload_body` - is the `TransactionPayloadBody` of standard SIP-2718 [blob transaction](#blob-transaction)
...
For body retrieval responses (`BlockBodies`), the standard SIP-2718 blob transaction `TransactionPayload` is used.
...
This SIP introduces blob transactions in the same format in which they are expected to exist in the final sharding specification.
...
Previous SIPs were all a solution of category (i); this SIP is a solution of category (ii).

The main tradeoff in designing this SIP is that of implementing more now versus having to implement more later:
do we implement 25% of the work on the way to full sharding, or 50%, or 75%?

The work that is already done in this SIP includes:
...
This SIP also sets the stage for longer-term protocol cleanups. For example, its (cleaner) gas base fee update rule could be applied to the primary basefee calculation.
...
Like SIP-1559, it's a self-correcting formula: as the excess goes higher, the `base_fee_per_blob_gas` increases exponentially, reducing usage and eventually forcing the excess back down.
...
Hence, it has a similar effect to the existing SIP-1559, but is more "stable" in the sense that it responds in the same way to the same total usage regardless of how it's distributed.
...
The values for `TARGET_BLOB_GAS_PER_BLOCK` and `MAX_BLOB_GAS_PER_BLOCK` are chosen to correspond to a target of 3 blobs (0.375 MB) and maximum of 6 blobs (0.75 MB) per block. These small initial limits are intended to minimize the strain on the network created by this SIP and are expected to be increased in future upgrades as the network demonstrates reliability under larger blocks.

## Backwards Compatibility

### Blob non-accessibility

This SIP introduces a transaction type that has a distinct mempool version and execution-payload version,
...
[SIP-5793](./sip-5793.md) will give further fine-grained control to nodes by extending the `NewPooledTransactionHashes` announcement messages to include the transaction type and size.
...
Execution layer test cases for this SIP can be found in the [`eip4844_blobs`](https://github.com/sila-chain/execution-spec-tests/tree/1983444bbe1a471886ef7c0e82253ffe2a4053e1/tests/cancun/eip4844_blobs) of the `sila/execution-spec-tests` repository. Consensus layer test cases can be found [here](https://github.com/sila-chain/consensus-specs/tree/2297c09b7e457a13f7b2261a28cb45777be82f83/tests/core/pyspec/eth2spec/test/deneb). 

## Security Considerations

This SIP increases the bandwidth requirements per beacon block by a maximum of ~0.75 MB.
...
The _sustained_ load of this SIP is much lower than alternatives that reduce calldata costs, even if the calldata is limited,
