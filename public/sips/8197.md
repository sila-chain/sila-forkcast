...
CATX (Cryptographically Agile Transactions) is an [SIP-2718](./sip-2718.md)
...
Current Sila transactions tightly couple body and signature by embedding
...
|----------------------|------------------------------------------------------|--------|
| `CA_TX_TYPE`         | [SIP-2718](./sip-2718.md) transaction type           | TBD    |
| `ECDSA_SIG_TYPE`     | ECDSA signature identifier                           | `0x00` |
| `SET_CODE_AUTH_TYPE` | [SIP-7702](./sip-7702.md) authorization type (MAGIC) | `0x05` |

This SIP defines the transaction format and encoding rules. The assignment of
`sig_type` values to specific cryptographic algorithms will be governed by other
SIPs. The ECDSA signature type (`0x00`) is specified here as the existing
signature algorithm—in the event of conflict, the signature type registry SIP
takes precedence.
...
|--------|---------------------------|-----------|------------------------------------------------------------------------------------------------------------------------------------------------------|
| `0x01` | [SIP-2930](./sip-2930.md) | 1         | `[chain_id, nonce, gas_price, gas_limit, to, value, data, access_list]`                                                                              |
| `0x02` | [SIP-1559](./sip-1559.md) | 1         | `[chain_id, nonce, max_priority_fee_per_gas, max_fee_per_gas, gas_limit, to, value, data, access_list]`                                              |
| `0x03` | [SIP-4844](./sip-4844.md) | 1         | `[chain_id, nonce, max_priority_fee_per_gas, max_fee_per_gas, gas_limit, to, value, data, access_list, max_fee_per_blob_gas, blob_versioned_hashes]` |
| `0x04` | [SIP-7702](./sip-7702.md) | 1         | `[chain_id, nonce, max_priority_fee_per_gas, max_fee_per_gas, gas_limit, to, value, data, access_list, authorization_list]`                          |
...
The referenced SIPs govern the meaning and validation of the fields in their
respective payload bodies, except as noted below.

#### [SIP-7702](./sip-7702.md) Authorization List
...
processing follows SIP-7702 semantics.
...
transactions ([SIP-8141](./sip-8141.md)), will require multiple signatures
...
**ECDSA (0x00)**: Uses the legacy Sila address derivation for backwards
compatibility:
...
Gas costing rules for each payload type are governed by their respective SIPs.
ECDSA signatures impose no additional cost beyond what the referenced SIPs
already specify.

SIPs defining new signature types should specify a gas penalty or discount
...
This SIP intentionally limits its scope to the transaction format—encoding,
...
dedicated registry SIP such as [SIP-7932](./sip-7932.md). The ECDSA type is
...
variable frame counts in SIP-8141) while ensuring all required signatures are
present.

### Trailing vs. Embedded Signatures

The trailing signature table is for signatures that commit to the entire
transaction. SIP-7702 authorization signatures commit to a substructure (the
...
SIP-7702 authorizations use `SET_CODE_AUTH_TYPE` (`0x05`) as their type byte,
which is the same MAGIC value used in SIP-7702. This means the authorization
signing hash `keccak256(0x05 || rlp([chain_id, address, nonce]))` is unchanged
from SIP-7702, and like the outer transaction, the signed data can be obtained
...
authorization without re-serialization. This also means existing ECDSA SIP-7702
...
prefix only their own type byte (e.g., `0x02` for SIP-1559) before the RLP
...
This SIP uses the chained commitment structure first described
in [SIP-8175](./sip-8175.md). This commits each signature to all prior
...
This is critical for payload types like SIP-8141 Frame transactions where
...
approach is consistent with [SIP-7932](./sip-7932.md).
...
types. [SIP-2930](./sip-2930.md)/[SIP-1559](./sip-1559.md)/[SIP-4844](./sip-4844.md)/[SIP-7702](./sip-7702.md)
...
corresponding non-CATX transaction types and vice versa. A non-CATX SIP-1559
...
Because the SIP-7702 authorization signing hash is identical between standard
