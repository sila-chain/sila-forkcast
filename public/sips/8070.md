...
This proposal introduces the sparse blobpool, a construction that brings cell-level, custody-aligned sampling in the Execution Layer (EL). For every new type 3 (blob-carrying) transaction, an EL node fetches full blob payloads only with probability p = 0.15, and otherwise it merely samples the blobs, using the same custody assignment as its Consensus Layer (CL) counterpart. For validators, this means downloading as little as 1/8 of the data (8 out of 64 cells), so that the average bandwidth consumption compared to the (current) full blobpool is 0.15 + 0.85/8 ~ 0.25, a ~4x reduction. The choice of p = 0.15 balances reducing bandwidth consumption with guaranteeing the full propagation of txs, by ensuring that for each blob tx there exists a large connected backbone of nodes that have the full blob payload. At an individual node level, p = 0.15 translates to 98.6% probability of least 3/50 neighbours holding the full blob payload, only 0.03% chance of total unavailability. The sampling performed with probability 1 - p = 0.85 enables streamlined data availability checks during block validation, as well as enhancing the availability of the data.

## Motivation

As Blob Parameter Only (BPO) forks progressively increase throughput, the full-replication nature of today's EL blobpool will begin dominating bandwidth utilization, causing us to hit [SIP-7870](./sip-7870.md) limits. Furthermore, this traffic will compete, and potentially starve, block and attestation propagation, risking instability and liveness issues. This behavior has already been observed in Fusaka devnets, where the average bandwidth consumption _of a full node_ is dominated by the EL blobpool, since column propagation on the CL benefits from sampling.

![](../assets/sip-8070/bandwidth-breakdown-fusaka-devnets.png)
...
We introduce a new devp2p protocol version, `sil/72` extending the existing `sil/71` protocol.
...
- old schema (`sil/71`): `[types: B, [size_0: P, size_1: P, ...], [hash_0: B_32, hash_1: B_32, ...]]`
- new schema (`sil/72`): `[types: B, [size_0: P, size_1: P, ...], [hash_0: B_32, hash_1: B_32, ...], cell_mask: B_16]`
...
- old schema (`sil/71`): `[request_id: P, [tx_0, tx_1, ...]]`, where type 3 transactions are encoded as `0x03 || rlp([tx_payload_body, wrapper_version, blobs, commitments, cell_proofs])`
- new schema (`sil/72`): `[request_id: P, [tx_0, tx_1, ...]]`, where type 3 transactions are encoded as `0x03 || rlp([tx_payload_body, wrapper_version, [], commitments, cell_proofs])`
...
- `sil/72`: `[[hash_0: B_32, hash_1: B_32, ...], cell_mask: B_16]`

**New message type `Cells` (`0x15`).** Used to respond to a `GetCells` requests.

- `sil/72`: `[[hash_0: B_32, hash_1: B_32, ...], cells: [[cell_0_0: B_2048, cell_0_1: B_2048, ...], [cell_1_0: B_2048, cell_1_1: B_2048, ...]], cell_mask: B_16]`
...
By default block builder **SHOULD** permanently act in eager mode: it acts as a provider (`p = 1.0`) for every type 3 transaction it intends to include, fetching the full blob payload. This guarantees that the builder holds complete blob data for all transactions in the payload. 

A builder **MAY** choose to act as a provider with `p = 0.15` probability, and switch to eager mode 2-3 slots prior to block building. But this is not recommended as this creates a clear pattern for the builder's networking behavior and compromises its privacy, increasing the risk of DoS attacks against it.

### Engine API extensions

**Method `engine_forkchoiceUpdatedV4`**

This SIP extends `engine_forkchoiceUpdatedV4` as defined in the SilaAmsterdam Engine API specification by adding an optional `custodyColumns` parameter, allowing the CL to inform the EL of its current blob column custody set.
...
1. CL MUST include a `custodyColumns` value after startup or when its columns custody set changes. It MUST also resend it after any Engine API interruption or reconnection.
2. EL MUST return an Ok response if the request is well-formed. All subsequent sampling requests MUST adopt the new custody set. Queued sampling requests MAY be patched to reflect the new custody set.
...
- `proofs`: `Array of DATA` - Array of `KZGProof` as defined in [SIP-4844](./sip-4844.md), 48 bytes each (`DATA`)
...
- **Sampling requirement**: Minimum `SAMPLES_PER_SLOT = 8` columns per node as per SilaPeerDAS specification
...
This SIP changes the `sil` protocol and requires rolling out a new version, `sil/72`. Supporting multiple versions of a wire protocol is possible. Rolling out a new version does not break older clients immediately, since they can keep using protocol version `sil/71`.

This SIP does not change consensus rules and does not strictly require a hard fork. We are assessing the gradual rollout possibilities. In the meantime, it is RECOMMENDED that this SIP be deployed within the context of a hard fork.
