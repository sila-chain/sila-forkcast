...
Ultimately, frame transactions realize the original vision of account abstraction: an account simply becomes an address with code. It leverages the SAVM to support arbitrary *user-defined* definitions of validation and gas payment.
...
- `max_priority_fee_per_gas` -- the SIP-1559 priority fee per gas the transaction will pay.
- `max_fee_per_gas` -- the maximum SIP-1559 fee the transaction is willing to pay, per gas.
- `max_fee_per_blob_gas` -- the maximum SIP-4844 fee per blob gas the transaction is willing to pay. Must be `0` if `blob_versioned_hashes` is empty.
...
The `signatures` list contains signatures that may be referenced by `VERIFY` frames and by ordinary SAVM execution. Every protocol-validated signature in this list must validate successfully before any frame is executed. If any signature is malformed, or any protocol-validated signature is invalid, the whole transaction is invalid.
...
The raw `signature` byte strings of protocol-validated schemes are intentionally not introspectable by SAVM code to allow future aggregation schemes. Contracts can inspect only the metadata of protocol-validated signature entries through transaction introspection. The raw `signature` byte strings of `ARBITRARY` entries are introspectable by SAVM code through `SIGPARAM`'s copy operation. This means that they cannot be aggregated in the future.
...
The explicit 32-byte zero digest is invalid. This reserves the zero stack value as the SAVM-visible representation of the transaction signing hash case.
...
```python
if len(savm.calldata) != EXPIRY_DATA_LENGTH:
    revert()

expiry_timestamp = int.from_bytes(savm.calldata, "big")
if savm.timestamp > expiry_timestamp:
    revert()

stop()
```

If the check passes, the frame succeeds with no return data and no logs. The frame consumes gas according to normal SAVM execution rules.

Clients may omit the explicit SAVM execution and directly perform the expiry check above, provided the externally observable result is identical to executing the canonical contract. Note: While this is a valid optimization for Sila sila-mainnet, it could be problematic on non-sila-mainnet situations in case a different contract is used.
...
Where `calldata_cost` is calculated per standard [SIP-7623](./sip-7623.md) rules.
...
The `effective_gas_price` is calculated per [SIP-1559](./sip-1559.md) and `blob_fees` is calculated as per [SIP-4844](./sip-4844.md).
...
The `APPROVE` instruction exits the current SAVM call frame successfully and updates the transaction-scoped approval context based on the `scope` operand.
...
the standard SAVM memory expansion cost.
...
This instruction gives access to signature-scoped metadata. The raw `signature` bytes of protocol-validated schemes are intentionally not accessible from the SAVM. `ARBITRARY` signature bytes are validated during SAVM execution and therefore can be accessed with the copy operation.
...
For `param == 0x04`, this instruction copies bytes from an `ARBITRARY` signature's raw `signature` bytes into memory. Its gas cost is calculated exactly as for `CALLDATACOPY`, including the fixed cost of 3, the per-word copy cost, and the standard SAVM memory expansion cost. It takes five values from the stack: `signatureIndex` on top, `param` second from top, followed by `length`, `dataOffset`, and `memOffset`. No stack output value is produced.
...
Although `TIMESTAMP` is generally banned during validation-prefix execution, it is permitted when executing the canonical expiry verifier runtime code at `EXPIRY_VERIFIER`; clients may optimize the frame by performing the same deadline check without explicit SAVM execution.
...
The access list was introduced to address a particular backwards compatibility issue that was caused by SIP-2929. The risk-reward of using an access list successfully is high. A single miss, paying to warm a storage slot that does not end up getting used, causes the overall transaction cost to be greater than had it not been included at all.
...
Thanks to the default code, EOAs today can use frame transactions to reap many benefits of account abstraction, including sending sponsored transactions, paying gas in SRC-20 tokens, batch transactions, and more.
