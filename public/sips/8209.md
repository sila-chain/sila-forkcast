...
This SIP builds on top of Frame Transactions to introduce a mechanism of front-running protection using a simple commit-reveal scheme for transaction frame payloads.
...
Current Sila transactions expose their full contents in the mempool before they are ordered or included in a block. This puts users in a vulnerable position and enables privileged participants, often known as "MEV searchers", to perform attacks against users, such as front-running, sandwiching, back-running, censorship, and others.
...
Simply by introducing a delay between transaction inclusion and revelation of transaction payload, we make these attackers lose most of their advantage over regular users. Existing proposals to address this issue introduce encryption to the P2P mempool directly; see [SIP-8184](./sip-8184.md) and [SIP-8105](./sip-8105.md).

Such advanced mechanisms, when operating in sub-slot timelines with in-protocol encryption, may achieve some better properties; however, the complexity of such solutions may prevent them from being adopted by a rapidly scaling Sila blockchain.

## Specification

### Definitions

- **Frame Transaction**: A transaction format defined in [SIP-8141](./sip-8141.md) that contains one or more frames.
...
A COMMIT frame is a new mode in the [SIP-8141](./sip-8141.md) frame transaction format:
...
- `gas_limit = 0`: The frame itself does not execute SAVM code.
...
A reveal frame is broadcast to the mempool as a separate [SIP-8141](./sip-8141.md) frame transaction:
...
2. For each COMMIT frame, `COMMIT_GAS_RESERVATION` is charged to the gas payer at the **maximum possible next-block base fee**, calculated as `max_next_base_fee = base_fee * 1.125` (see [SIP-1559](./sip-1559.md)).
3. The transaction's intrinsic gas cost ([SIP-8141](./sip-8141.md) `FRAME_TX_INTRINSIC_COST`), including the calldata costs for the commitment hash data, is charged normally to the gas payer.
...
2. No SAVM execution occurs - no state changes beyond the gas payment.
...
Inclusion of REVEAL transactions is enforced through fork-choice by the slot's regular attestation committee, following the same model as [SIP-7805](./sip-7805.md) (FOCIL) inclusion list enforcement.
