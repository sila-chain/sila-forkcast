...
This SIP modifies the 'sil' p2p protocol to support Block-level Access Lists (BALs). It adds two new messages, `GetBlockAccessLists` (0x12) and `BlockAccessLists` (0x13), enabling peers to request and serve BALs for synchronization and parallel execution.

## Motivation

[SIP-7928](./sip-7928.md) introduces block-level access lists that record all state locations accessed during block execution. These lists enable parallel disk reads, parallel transaction execution, and executionless state updates.
...
The `block-access-list-hash` field contains `keccak256(rlp.encode(block-access-list))` as defined in [SIP-7928](./sip-7928.md). This field MUST be present in headers after the SilaAmsterdam fork and absent for earlier blocks.
...
See [SIP-7928](./sip-7928.md) for complete BAL generation rules and inclusion semantics.
...
BALs are only available for blocks after [SIP-7928](./sip-7928.md) activation and within the retention period defined therein. Requests for unavailable BALs return empty entries.
...
3. **Retention policy**: BALs may be pruned per [SIP-7928](./sip-7928.md), while block bodies are retained longer.

### Message IDs

Message IDs 0x12 and 0x13 follow sequentially from the existing sil/69 messages (BlockRangeUpdate is 0x11).

### Response Size Limit

The 2 MiB soft limit is consistent with other sil protocol responses (blocks, receipts, headers) and accommodates multiple BALs per response. Responses containing a single BAL that exceeds 2 MiB are still valid, as the soft limit governs when to stop appending additional items, not the maximum size of an individual item.

## Backwards Compatibility

This SIP changes the sil protocol and requires rolling out a new version, sil/71. Supporting multiple protocol versions is standard practice. Rolling out sil/71 does not break older clients, as they can continue using sil/69 or sil/70.

This SIP requires the SilaAmsterdam hard fork for the header field extension. The new messages are only meaningful for post-SilaAmsterdam blocks.
