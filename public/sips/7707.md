...
This SIP reduces the gas cost of access list data, incentivizing the inclusion of complete and valid access lists in transactions to improve data load efficiency for execution layer clients.

## Motivation

While [SIP-2930](./sip-2930.md) introduced `accessLists` as a mechanism for `SLOAD`
pre-warming  to reduce gas costs by informing the SAVM upfront about which storage slots a transaction will access,
...
We shall update [SIP-2930](./sip-2930.md)
parameters:
...
[SIP-2930](./sip-2930.md) logic
of "covering the bandwidth costs", which was not described in detail and
is potentially outdated. 

It should be noted that this is not the first time [SIP-2930](./sip-2930.md) additions have been proposed. In [SIP-3521](./sip-3521.md), a reduction was already proposed, but it focused only on `ACCESS_LIST_ADDRESS_COST`.
...
This SIP makes a minor update to
[SIP-2930](./sip-2930.md) with
