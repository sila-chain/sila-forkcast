...
Add a precompile that verifies multiple KZG openings `(z_i, y_i)` for a single [SIP-4844](./sip-4844.md) blob commitment, returning the same 64-byte output as the existing point-evaluation precompile.

## Motivation

[SIP-4844](./sip-4844.md) provides a point-evaluation precompile at `0x0A` costing 50,000 gas per opening. Contracts needing multiple blob values (e.g., fraud proofs) must pay this cost repeatedly. A batch interface verifies *k* openings in one call, reducing overhead.
...
It verifies multiple KZG openings `(z_i, y_i)` for a single SIP-4844 blob commitment in one call. The verification uses BLS12-381 pairing operations internally to check the multi-point opening proof. Gas cost is defined by `MULTI_POINT_EVALUATION_BASE_GAS + MULTI_POINT_EVALUATION_PER_POINT_GAS * n` where `n` is the number of evaluation points.
...
On success, return 64 bytes identical to [SIP-4844](./sip-4844.md):
...
Add a precompile at `MULTI_POINT_EVALUATION_PRECOMPILE_ADDRESS` that verifies multiple KZG openings `(z_i, y_i)` for a single SIP-4844 blob commitment.
...
- Preserves [SIP-4844](./sip-4844.md) versioned-hash model and return format.
...
No changes to [SIP-4844](./sip-4844.md) transactions, `BLOBHASH`, or the existing precompile at `0x0A`.

## Security Considerations


- Reject non-canonical field elements (`>= BLS_MODULUS`).
- Implementations MUST use the same trusted setup and subgroup checks as [SIP-4844](./sip-4844.md).
