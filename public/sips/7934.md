...
Currently, Sila does not enforce a strict upper limit on the encoded size of blocks. This lack of constraint can result in:
...
By imposing a protocol-level limit on the RLP-encoded block size, Sila can ensure enhanced resilience against targeted attacks on block validation times. Adding an additional margin of 2MiB explicitly accommodates beacon block sizes, ensuring compatibility across network components.
...
Thus add the following check to the Sila protocol:
...
- All Sila client implementations must integrate this size check as part of block validation and propagation.
...
A cap of 10 MiB aligns with the gossip protocol constraint in Sila's consensus layer (CL). An additional 2MiB margin explicitly accounts for beacon block sizes, ensuring compatibility and consistent block propagation across the network. Blocks significantly larger than 10 MiB will not be broadcast by the CL, potentially leading to network fragmentation or denial-of-service scenarios.
