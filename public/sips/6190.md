...
[SIP-6188](./sip-6188.md) and [SIP-6189](./sip-6189.md) must be used for this SIP to function correctly.
...
The base gas cost of `SELFDESTRUCT` is set to `5000`. The gas cost of `SELFDESTRUCT` is increased by `5000` for each alias contract that forwarded to the contract being self-destructed. Finally, the [SIP-2929](./sip-2929.md) gas cost increase is applied.

## Rationale

This SIP is designed to be a step towards supporting `SELFDESTRUCT` in Verkle trees while making the minimum amount of changes.

The `5000` base gas cost and additional alias contracts represents the cost of setting the account nonce and first storage slot. The [SIP-2929](./sip-2929.md) gas cost increase is preserved for the reasons mentioned in said SIP's Rationale.

The nonce of `2^64-1` was chosen since it is the nonce protected by [SIP-6188](./sip-6188.md). The account code of `0x1` was chosen since it was the code specified in [SIP-6189](./sip-6189.md).
...
This SIP requires a protocol upgrade, since it modifies consensus rules.
