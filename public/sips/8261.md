...
4. Block-level "voting" via the ±1/1024 elasticity rule introduced in [SIP-1559](./sip-1559.md), allowing the gas limit to drift up or down per block.
...
- **Implicit governance.** Setting the gas limit on sila-mainnet is currently an opaque social process performed by individual validators and large staking operators. This makes it difficult for client teams and researchers to commit to safe upper bounds that are guaranteed to be respected.
- **Asymmetry with blob parameters.** [SIP-7892](./sip-7892.md) already established BPO ("Blob Parameter Only") hard forks as the canonical, low-overhead path for scaling blob capacity. Blob `target`, `max`, and `baseFeeUpdateFraction` are now hard-fork-scheduled and consensus enforced. Block gas remains the only major capacity dial still set by social consensus among validators.
...
The goal of this SIP is to make the gas limit behave like every other hard-fork parameter: a single value, agreed at fork time, enforced by consensus, changeable only through a fork (a normal fork, or a lightweight Gas Parameter Only fork modeled on [SIP-7892](./sip-7892.md)).
...
Let `FORK_TIMESTAMP` denote the activation timestamp of the fork that includes this SIP. The rules below apply to any execution payload whose `timestamp >= FORK_TIMESTAMP`, and to any beacon block whose slot maps to an epoch at or after the corresponding consensus-layer fork epoch.
...
The chain configuration is extended with a `gasLimitSchedule` object and per-fork `<fork_name>Time` activation timestamps, following the convention established by [SIP-7840](./sip-7840.md) and [SIP-7892](./sip-7892.md):
...
`gpo<index>` ("Gas Parameter Only") forks follow the same naming convention and lifecycle as BPO forks defined in [SIP-7892](./sip-7892.md). Activation timestamps are required only for forks at or after the activation fork of this SIP.
...
A new `GAS_LIMIT_SCHEDULE` field is added to consensus layer configuration, mirroring the `BLOB_SCHEDULE` mechanism in [SIP-7892](./sip-7892.md). Entries represent gas limit changes that take effect at the start of the listed epoch:
...
This replaces the legacy [SIP-1559](./sip-1559.md) elasticity rule. Specifically, the constraints
...
Starting at the engine API version released alongside this SIP:
...
Testnets and devnets MUST include a `gasLimitSchedule` entry for genesis if their genesis is at or after this SIP's activation. For testnets that were live before the activation fork, the genesis entry is unnecessary; only the activation fork's entry is required.
...
### Relationship to SIP-1559

[SIP-1559](./sip-1559.md) introduced the elasticity multiplier and the ±1/1024 gas-limit adjustment rule. The base-fee mechanism (target = `gas_limit / elasticity_multiplier`, max = `gas_limit`) is unchanged in spirit; it now uses the scheduled `gas_limit` as its input. The per-block adjustment rule is removed because the gas limit no longer varies block-to-block within a fork.

### Relationship to SIP-7825

[SIP-7825](./sip-7825.md) caps per-transaction gas. This SIP caps (and fixes) per-block gas. The two are complementary: SIP-7825 prevents single-transaction DoS within a block; this SIP prevents the block-level capacity itself from drifting away from a tested safe value.

## Backwards Compatibility

This change is consensus-breaking. Specifically:

- Blocks valid under the [SIP-1559](./sip-1559.md) elasticity rule but whose `gas_limit` differs from the scheduled value are invalid after `FORK_TIMESTAMP`.
...
def validate_gas_limit(block: Block, parent: Block, config: ChainConfig) -> None:
    if block.header.timestamp < config.activation_timestamp(THIS_EIP_FORK):
        # Legacy SIP-1559 elasticity rule
...
**Fork coordination risk.** This SIP creates a hard dependency between EL and CL gas-limit schedules. A divergence (EL says 90M, CL says 75M) will split the network at the fork boundary. The same risk exists today for BPO blob schedules; the same mitigations apply: schedule consistency checks at startup, devnet rehearsals, and clear off-chain coordination of `gpo<index>` schedules.

**Upgrade pressure.** Removing per-validator gas-limit voting removes one informal mechanism for the staking community to signal concerns about capacity. SIP authors and core devs SHOULD treat this as a reason to maintain visible, structured channels for capacity discussion (e.g., All Core Devs calls, public test results) ahead of any `gpo<index>` fork.
