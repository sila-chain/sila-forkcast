...
2. Remove the `SSTORE` refund in all cases except for one specific case: if the _new value_ and _original value_ of the storage slot both equal 0 but the _current value_ does not (those terms being defined as in [SIP-1283](https://sips.sila.org/EIPS/sip-1283)), refund `SSTORE_REFUND_GAS` gas.
...
It also preserves two key goals of SIP 3298:
...
### With SIP-3403 partial refunds 

If refunds were to be partially removed, as specified [here](https://github.com/sila-chain/SIPs/pull/3403/), this would be the comparative table. **This table also assumes touched storage slots were already 'hot'**.
