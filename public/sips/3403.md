...
2. Remove the `SSTORE` refund in all cases except for one specific case: if the _new value_ and _original value_ of the storage slot both equal 0 but the _current value_ does not (those terms being defined as in [SIP-1283](https://sips.sila.org/SIPS/sip-1283)), refund `SSTORE_REFUND_GAS` gas.
