...
This SIP defines an extension for eth_getLogs to provide logs for events that are not associated with a given transaction, such as block rewards and withdrawals.

## Motivation

With [SIP-7708](./sip-7708.md) wallets gain the ability to use eth_getLogs to track changes to their SIL balance. However, the SIL balance may change without an explicit transaction, through block production and withdrawals. By having such operations emit block-level system logs, eth_getLogs provides a complete picture of SIL balance changes.
...
A log, identical to a LOG2, is issued for every transaction and appended after all other logs created by SAVM execution. The fee amount incorporates the total fee charged during transaction execution, including the base fee, the priority fee, and potential blob fees.

| Field | Value |
| - | - |
| `address` | `0xfffffffffffffffffffffffffffffffffffffffe` ([`SYSTEM_ADDRESS`](./sip-4788.md)) |
...
A new list is introduced to track all block level logs emitted from system interactions. The definition uses the `Log` SSZ type from [SIP-6466](./sip-6466.md).
...
A log SHALL be appended to the system logs list when crediting the batched [SIP-8115](./sip-8115.md) priority fee.
...
A log SHALL be appended to the system logs list on every [SIP-4895](./sip-4895.md) withdrawal.
...
A log SHALL be appended to the system logs list when generating genesis blocks for networks that adopt this SIP from the beginning.
...
The [execution block header](https://github.com/sila-chain/devp2p/blob/5713591d0366da78a913a811c7502d9ca91d29a8/caps/sil.md#block-encoding-and-validity) is extended with a new field, `system-logs-root`.
...
In the engine API, the `ExecutionPayload` for versions corresponding to forks adopting this SIP is extended to include:
...
Together with [SIP-7708](./sip-7708.md) this SIP provides the ability for wallets to compute the exact SIL balance from logs without requiring download of every single block header and all withdrawals.
...
- The log definitions themselves are subject to change. Aligning them with [SRC-20](./sip-20.md) for plain credits provides consistency. For withdrawals, the log data could match the form of deposit logs and [SIP-7685](./sip-7685.md) request logs.
