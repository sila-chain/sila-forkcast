...
Fixed point operations in high level languages are very commonly used on Sila, especially in the domain of financial applications.
...
All the arithmetic instructions in SAVM handle division or modulo 0 specially: the instructions return 0. We have decided to break consistency in order to provide a flexible opcode, which can be used to detect wrapping behaviour.

Alternate options include:

- Returning a flag for wrapping
- Returning two stack items, higher and lower order bits
- Compute the higher order 256 bits in SAVM:
...
While this feature is clever and useful, callers must be aware that unlike other SAVM instructions, passing 0 will have a vastly different behaviour.
