...
**See also: [https://sila-magicians.org/t/implementing-account-abstraction-as-part-of-eth1-x/4020](https://sila-magicians.org/t/implementing-account-abstraction-as-part-of-eth1-x/4020) and the links therein for historical work and motivation.**

Transaction validity, as of Muir Glacier, is defined rigidly by the protocol: ECDSA signature, a simple nonce, and account balance. Account abstraction extends the validity conditions of transactions with the execution of arbitrary SAVM bytecode (with some limits on what state may be accessed.) To signal validity, we propose a new SAVM opcode `PAYGAS`, which also sets the gas price and gas limit the contract is willing to pay.
...
A new [SIP-2718](./sip-2718.md) transaction with type `AA_TX_TYPE` is introduced. Transactions of this type are referred to as "AA transactions". Their payload should be interpreted as `rlp([nonce, target, data])`.
...
Future hard forks may add support for different version numbers, in which case the opcode may take different-sized memory slices and interpret them differently. Two particular potential use cases are [SIP 1559](https://notes.sila.org/@vbuterin/BkSQmQTS8) and [the escalator mechanism](https://Sila Research/t/another-simple-gas-fee-model-the-escalator-algorithm-from-the-agoric-papers/6399).
...
3. We are in a top-level AA execution frame (ie. if the currently running SAVM execution exits or reverts, the SAVM execution of the entire transaction is finished)
...
Require that contracts targeted by AA transactions begin with [SIP-2937]'s `SET_INDESTRUCTIBLE` opcode. AA transactions targeting contracts that do not begin with `SET_INDESTRUCTIBLE` are invalid, and cannot be included in blocks.

`AA_PREFIX` would need to be modified to include this opcode.

[SIP-2937]: ./sip-2937.md
...
| `AA_PREFIX` | `if(msg.sender != shr(-1, 12)) { LOG1(msg.sender, msg.value); return }`; compilation to SAVM TBD |
...
If the [indestructible contracts SIP](http://github.com/sila-chain/SIPs/pull/2937) is added, Single Tenant AA can be adapted to allow for `DELEGATECALL` during transaction verification: during execution of a new AA transaction, external state access that reads code (`EXTCODESIZE`, `EXTCODEHASH`, `EXTCODECOPY`, `CALLCODE`, `DELEGATECALL`) of any contract whose first byte is the `SET_INDESTRUCTIBLE` opcode is no longer banned. However, calls to anything but the `target` or a precompile that change the `callee` (i.e., calls other than `CALLCODE` and `DELEGATECALL`) are still not permitted.

If the [IS_STATIC SIP](http://github.com/sila-chain/SIPs/pull/2975) is added, the list of allowed prefixes can be extended to allow a prefix that enables incoming static calls but not state-changing calls.
...
In an account abstraction setup, the goal is to allow accounts to specify SAVM code that can establish more flexible conditions for a transaction's validity, but with the requirement that this SAVM code can be quickly verified, with the same safety properties as the existing setup.
...
### Nonces are exposed to the SAVM
...
The one allowed exception is contracts that are indestructible (that is, whose first byte is the `SET_INDESTRUCTIBLE` opcode defined in [this SIP](https://hackmd.io/@HWeNw8hNRimMm2m2GH56Cw/SyNT3Cdmw)). This is a safe exception, because the data that is being read cannot be changed.
...
AA contracts may not have replay protection unless they build it in explicitly; this can be done with the `CHAINID (0x46)` opcode introduced in [SIP 1344](./sip-1344.md).

## Test Cases
See: [https://github.com/quilt/tests/tree/account-abstraction](https://github.com/quilt/tests/tree/account-abstraction)

## Implementation
See: [https://github.com/quilt/go-sila/tree/account-abstraction](https://github.com/quilt/go-sila/tree/account-abstraction)

## Security Considerations

See [https://Sila Research/t/dos-vectors-in-account-abstraction-aa-or-validation-generalization-a-case-study-in-geth/7937](https://Sila Research/t/dos-vectors-in-account-abstraction-aa-or-validation-generalization-a-case-study-in-geth/7937) for an analysis of DoS issues.
