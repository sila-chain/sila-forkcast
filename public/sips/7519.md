...
chains, however there is a lack of parallel primitives within the SAVM to support
...
case. General concurrency controls is not a goal of this SIP.
...
These semantics could be implemented in the SAVM today, but it would also include
a SLOAD, DUP, LT, JUMPI and REVERT instructions. The SAVM, however, can do these
operations much more efficiently than via opcodes. First, each SSTORE always
incurs a slot load in order to apply [SIP-2200](./sip-2200.md) gas calculation
...
This most important use case for this SIP asset balances and not general
...
There is no SAVM functionality removed by this proposal.
