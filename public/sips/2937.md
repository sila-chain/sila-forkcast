...
Unlike SIPs that disable the `SELFDESTRUCT` opcode entirely, this SIP does not modify behavior of any existing contracts.
...
If `SELFDESTRUCT` is ever removed in the future, this SIP would simply become a no-op.
