...
Since [SIP-3298](./sip-3298.md), the gas refund mechanism works for storage restores only inside the same transaction. But on revert, the gas refund is not increased; it is completely erased.
...
if (call.reverted) {
    ...
    // New behavior added to existing according to the SIP-3978
