...
This SIP introduces new SAVM opcodes that allow loading a single byte from memory or calldata in a single operation, reducing gas cost and bytecode size compared to existing patterns based on `MLOAD (0x51)` or `CALLDATALOAD (0x35)` followed by bit shifting.
...
This SIP proposes two new opcodes that allow loading a single byte directly in one operation.
...
The base gas cost matches `MLOAD`, `MSTORE8`, and `CALLDATALOAD`, ensuring consistency with existing SAVM pricing.
...
In both cases, execution halts and the current call frame is reverted, consistent with existing SAVM behavior.
...
This SIP introduces new opcodes and does not modify the semantics of existing instructions. No backwards compatibility issues are introduced beyond those inherent to any opcode-adding hard fork.
