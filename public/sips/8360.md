...
This SIP introduces a new EVM opcode, `TCREATE`, where `T` stands for transient, providing an official, gas-efficient, and state-aware mechanism for deploying temporary contracts.

## Motivation

Many Sila applications do not require deployed code to persist beyond a single transaction. Examples include state-channel contracts, single-use accounts, and other ephemeral execution environments.

Today, such patterns rely on a combination of `CREATE`/`CREATE2` and `SELFDESTRUCT`. However, `SELFDESTRUCT` has no longer provided gas refunds since [SIP-3529](./sip-3529.md), can only delete contracts created within the same transaction since the SilaCancun upgrade, and is expected to undergo further semantic restrictions in future protocol upgrades. As a result, the deploy-and-self-destruct pattern is no longer economically attractive, nor can its long-term behavior be relied upon.
...
| --- | --- | --- | --- |
| `TCREATE` | New opcode | TBD | This SIP |
| `BASE_OPCODE_COST` | Base opcode cost | 100 | This SIP |
| `CPSB` | State gas price per byte | 1530 | [SIP-8037](./sip-8037.md) |
| `STATE_BYTES_PER_NEW_ACCOUNT`| State bytes created per account| 120 | [SIP-8037](./sip-8037.md) |
| `WARM_ACCESS` | Touch of an already warm account or storage slot| 100 | [SIP-8038](./sip-8038.md) |
| `COLD_ACCOUNT_ACCESS` | Cold touch of an account | 3000 | [SIP-8038](./sip-8038.md) |
| `ACCOUNT_WRITE` | Surcharge applied when an account leaf is modified for the first time | 9000 | [SIP-8038](./sip-8038.md) |

### Semantics

The behavior of `CREATE2` is defined in [SIP-1014](./sip-1014.md).
...
- At the end of the transaction, all code, nonce, and storage created during transaction execution are removed from the target account. The account balance is preserved. An account that becomes empty is removed from the state trie at the end of the transaction in accordance with [SIP-161](./sip-161.md). The semantics are clarified further [here](#state-transition-handling).
- Code deployment rules and collision semantics are identical to `CREATE2`. A target account is considered non-empty during the transaction, and address collisions therefore revert execution in accordance with [SIP-7610](./sip-7610.md). The semantics are clarified further [here](#state-transition-handling).
...
The separation between this ephemeral storage and the transient storage defined by [SIP-1153](./sip-1153.md) is an implementation detail. Clients must ensure that writes performed through `SSTORE` are isolated from persistent storage while preserving the observable semantics of the `SLOAD` and `SSTORE` opcodes.
...
Account access costs continue to follow [SIP-8038](./sip-8038.md). In particular, accesses to cold accounts are charged `COLD_ACCOUNT_ACCESS` as appropriate.

State-gas accounting is defined separately by this SIP. Rather than charging for account creation itself, state-gas is charged only for state changes that cannot be reasonably reclaimed by the end of the transaction, and refilled when those state changes are reverted within the same transaction.
...
Execution-gas accounting for account updates (following the principles introduced in [SIP-8038](./sip-8038.md)) is defined as follows.
...
The `0xfe` prefix is chosen to ensure that the address derivation scheme used by `TCREATE` cannot collide with that of `CREATE2`, which uses the `0xff` prefix as specified in [SIP-1014](./sip-1014.md).
...
For EOAs delegated through [SIP-7702](./sip-7702.md), this property also improves transaction simulation and mempool analysis, since the sender's post-transaction nonce depends only on permanent account-creation operations and remains independent of transient deployments.
...
The distinction between ephemeral storage used by `SLOAD` and `SSTORE` and the transient storage defined by [SIP-1153](./sip-1153.md) is intentionally invisible to contracts. It exists solely to simplify client implementations while preserving the expected semantics of both storage models.

### State-Gas Accounting for Balance Transfers

Removing balance accounting entirely would simplify gas accounting but would reintroduce a special case that [SIP-8246](./sip-8246.md) sought to eliminate.
...
Because target addresses are derived using the same rules as `CREATE2`, while all account state except balance is removed after transaction completion, such accounts become valid deployment targets again under [SIP-7610](./sip-7610.md).
