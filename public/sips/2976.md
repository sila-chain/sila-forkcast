## Abstract
[Typed Transactions](./sip-2718.md) can be sent over devp2p as `TransactionType || TransactionPayload`.
The exact contents of the `TransactionPayload` are defined by the `TransactionType` in future SIPs, and clients may start supporting their gossip without incrementing the devp2p version.
...
## Motivation
[SIP-2718](./sip-2718.md) introduced new transaction types for blocks (which presents itself in the makeup of a block header's transaction root and receipts root).
...
*Note: See [SIP-2718](./sip-2718.md) for additional motivations of Typed Transactions.*
