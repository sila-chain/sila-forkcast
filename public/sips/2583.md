...
This SIP introduces a gas penalty for opcodes which access the account for trie non-existent accounts.

## Abstract

This SIP adds a gas penalty for accesses to the account trie, where the address being looked up does not exist. Non-existing accounts can be used in 
...
As the sila trie becomes more and more saturated, the number of disk lookups that a node is required to do in order to access a piece of state increases too. This means that checking e.g. `EXTCODEHASH` of an account at block `5` was _inherently_ a cheaper operation that it is at, say `8.5M`. 
...
So far, the attempts to handle this problem has been in raising the gas cost, e.g. [SIP-150](./sip-150.md), [SIP-1884](./sip-1884.md). 
...
This SIP proposes a mechanism to alleviate the situation. 
...
This SIP adds a second rule in the following way:
...
Whether the rules of this SIP is to be applied for regular sila-sends in `transactions` is TBD. See the 'Backwards Compatibility'-section for some more discussion on that topic. 
...
- The `base` costs of any opcodes are not modified by the SIP.
- The opcode `SELFBALANCE` is not modified by this SIP, regardless of whether the `self` address exists or not. 
...
This SIP does not modify anything regarding storage trie accesses, which might be relevant for a future SIP. However, there are a few crucial differences. 
...
This SIP requires a hard-fork. 

### Sila transfers

A regular `transaction` from one EOA to another, with value, is not affected. 

A `transaction` with `0` value, to a destination which does not exist, would be. This scenario is highly unlikely to matter, since such a `transaction` is useless -- even during success, all it would accomplish would be to spend some `gas`. With this SIP, it would potentially spend some more gas.

### Layer 2

Regarding layer-2 backward compatibility, this SIP is a lot less disruptive than SIPs which modify the `base` cost of an opcode. For state accesses, there are 
...
With this SIP in place, the 'happy-path' would work as previously, and the 'notorious'-path where `recipient` does not exist would cost an extra `penalty` gas, but the actual execution-flow would be unchanged.  

#### ERC223

[ERC223 Token Standard](https://github.com/sila-chain/SIPs/issues/223) is, at the time of writing, marked as 'Draft', but is deployed and in use on sila-mainnet today.

The SRC specifies that when a token `transfer(_to,...)` method is invoked, then:
...
2. The `_to` exists, but is no contract. The flow exits here, and is not affected by this SIP 
...
The contract [`Dentacoin`](https://silascan.io/address/0x08d32b0da63e2c3bcf8019c9c5d849d7a9d791e6#code) would be affected. 
        ...
        if (balances[msg.sender] >= _value && balances[_to] + _value > balances[_to]) {               // Check if sender has enough and for overflows
            ...
            if (msg.sender.balance >= minBalanceForAccounts && _to.balance >= minBalanceForAccounts) {    // Check if sender can pay gas and if recipient could
                ...
                if(msg.sender.balance < minBalanceForAccounts) {
                    if(!msg.sender.send(gasForDCN)) throw;                  // Send sil to sender
                  }
                if(_to.balance < minBalanceForAccounts) {
                    if(!_to.send(gasForDCN)) throw;                         // Send sil to recipient
                }
            }
        } else { throw; }
    }
...
The contract checks `_to.balance >= minBalanceForAccounts`, and if the `balance` is too low, some `DCN` is converted to `sila` and sent to the `_to`. This is a mechanism to ease on-boarding, whereby a new user who has received some `DCN` can immediately create a transaction.

Before this SIP:

- When sending `DCN` to a non-existing address, the additional `gas` expenditure would be:
  - `9000` for an sila-transfer
