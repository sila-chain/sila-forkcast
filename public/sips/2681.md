...
Account nonces are currently specified to be arbitrarily long unsigned integers. Dealing with arbitrary length data in the state witnesses is not optimal, therefore this SIP will allow proofs to represent the nonce in a more optimized way.
...
2. It must be noted that in the past, in the Morden testnet, each new account had a starting nonce of `2^20` in order to differentiate transactions from sila-mainnet transactions.
This mode of replay protection is out of fashion since [SIP-155](./sip-155.md) introduced a more elegant way using chain identifiers.

3. Most clients already consider the nonce field to be 64-bit, such as go-sila.
...
2. go-sila already has this restriction partially in place (`state.Account.Nonce` and `types.txdata.AccountNonce` it as a 64-bit number).
