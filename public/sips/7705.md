...
Reentrancy attacks account for a substantial portion of user funds stolen on SAVM chains, including the famous "DAO hack". However, due to the cost of preventing reentrancy attacks in application code, developers often opt-out of reentrancy protection. This cost has come down with the advent of transient storage ([SIP-1153](./sip-1153.md)), but it is still not cheap enough where it is a "no-brainer" to use it by default. This SIP proposes opcodes which make it cheaper to protect against reentrancy in application code.
