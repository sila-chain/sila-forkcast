...
Introduce a new JSON standard for SAVM traces during execution of state tests.

## Motivation

The Sila Virtual Machine executes all smart contract code on sila.
In order to debug smart contracts and state tests better, a common format was introduced to log every execution step of the SAVM.
...
Implementing this SIP in all major clients allows us to create meaningful differential fuzzers that fuzz SAVM implementations for the sila-mainnet and all upcoming hardforks.
It also helps to find differences in execution quickly in the case of a chain split.

This SIP will enable users to create better differential fuzzing infrastructure to compare the SAVM implementations of all major Sila clients against each other.
...
This SIP is largely based on the previous non-official documentation for SAVM tracing.
...
This SIP is fully backward compatible with go-sila. OpenEthereum, Besu and Nethermind clients would have to change their JSON output of
`openethereum-savm` `evmtool` and
