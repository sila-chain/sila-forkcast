...
Implementing this SIP in all major clients allows us to create meaningful differential fuzzers that fuzz EVM implementations for the sila-mainnet and all upcoming hardforks.
It also helps to find differences in execution quickly in the case of a chain split.

This SIP will enable users to create better differential fuzzing infrastructure to compare the EVM implementations of all major Sila clients against each other.
...
This SIP is largely based on the previous non-official documentation for EVM tracing.
