...
This SIP describes the format of Whisper messages within the ÐΞVp2p Wire Protocol.
This SIP should substitute the [existing specification](https://github.com/sila-chain/wiki/wiki/Whisper-Wire-Protocol).
More detailed documentation on Whisper could be found [here](https://github.com/sila-chain/go-sila/wiki/Whisper).
...
This section outlines the optional description of Data Field to set up an example. Later it may be moved to a separate SIP.
...
This SIP is compatible with Whisper version 6. Any client which does not implement certain packet codes should gracefully ignore the packets with those codes. This will ensure the forward compatibility. 
