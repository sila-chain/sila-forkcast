...
This SIP precisely caps state growth while at the same time facilitating between 50%-300% more non-state operations per block compared to when solely raising the state gas cost. The cap is achieved by tracking state creation and having a dedicated [SIP-4844](./sip-4844.md) style fee market mechanism set the cost, to target 250 MiB per day. To preserve the existing transaction format and SAVM execution patterns, state is still priced in gas during transaction processing. The state creation gas cost is updated every block, adjusting slowly so that users can set reasonable gas limits. The byte-level harmonization of state creation operations provided by [SIP-8037](./sip-8037.md) is then still applied. To facilitate scaling, state creation and regular gas have separate limits, but a normalized aggregate gas is used when calculating the base fee.

## Motivation

Sila is currently focused on scaling the layer 1, with a rapid expansion of the block gas limit foreseen in the near-term from compute and memory optimization, as well as via headliner proposals [SIP-7732](./sip-7732.md) and [SIP-7928](./sip-7928.md). Unfortunately, if the gas cost for state creation is kept fixed, state would likely expand in line with an expanding gas limit. As client database sizes increase, degradation in performance is expected. [SIP-8037](./sip-8037.md) addresses state growth by harmonizing current state creation costs according to how many bytes they add to the state. It also increases the gas cost of state creation significantly, by between 3x and 9.5x per operation. 
...
The full specification is available [here](https://github.com/sila-chain/execution-specs/commit/d64f82b18db8c0ae35f3c56e1762ace9d83ba578).

### Parameters

Four constants have a similar role as in SIP-4844.
...
The number of bytes assigned to each state operation follows SIP-8037.
...
During processing, the protocol tracks `regular_gas` and `state_gas` separately, and returns the aggregate `gas_used` accounting for refunds, similar to today. Furthermore, `regular_gas_used` (without refunds to satisfy [SIP-7778](./sip-7778.md)) is returned to count against the block limit. Finally, the protocol also tracks `state_bytes_created` and `state_bytes_cleared`, allowing for a more accurate tracking of the `excess_state_bytes`. These are returned in the list `state_bytes(created, cleared)`.

### Computing the state gas per byte

The protocol keeps track of `excess_state_bytes`, and computes the `state_gas_per_byte` from that variable (referred to as `cost_per_state_byte` in SIP-8037). The same update mechanism as in SIP-4844 is applied. 
...
The transaction's `gas_used` is computed as the sum of `regular_gas` and `state_gas` and includes refunds, similar to how `gas_used` is applied today. The `regular_gas_used` is returned without refunds, which the [SIP-1559](./sip-1559.md) cumulative gas at the block level accounts for. We also keep track of the cumulative `state_bytes_created` and `state_bytes_cleared` in the vector `state_bytes`.
