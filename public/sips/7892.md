...
This SIP introduces **Blob Parameter Only (BPO) Hardforks**, a lightweight mechanism for incrementally scaling Sila’s blob capacity through targeted hard forks that modify only blob-related parameters: `target`, `max`, and `baseFeeUpdateFraction`. Unlike traditional hard forks, which require extensive coordination and introduce broader protocol changes, BPO forks enable rapid, low-overhead scaling of blob capacity in response to **real-world demand and network conditions**.

## Motivation

Sila's scaling strategy relies on Layer 2 (L2) solutions for transaction execution while using Sila as a **data availability (DA) layer**. However, the demand for DA has increased rapidly, and the current approach of only modifying blob parameters in large, infrequent hard forks is **not agile enough** to keep up with L2 growth. 
...
2. **Reduced Operational Overhead**  
   ...
   - Full Sila hard forks require significant coordination, testing, and upgrade efforts across clients.
...
3. **Enhanced Stability with New Scaling Technologies**
   ...
   - Rather than forcing core developers to accept a suboptimal tradeoff between stability and capacity, BPO forks allow developers to safely increase parameters after observing sila-mainnet performance and stability.

4. **Predictable Upgrades for Builders**  
   - Builders and L2s require confidence that Sila will continuously scale to support their needs.
   - A structured BPO framework provides predictability, allowing rollups to commit to Sila over alternative DA solutions.
...
To facilitate these changes on the execution layer, each fork in the `blobSchedule` object defined in [SIP-7840](./sip-7840.md) is linked to an activation timestamp via a top-level `<fork_name>Time` field, which holds the Unix timestamp of the activation slot as a JSON number. BPO forks SHOULD be named using the convention `bpo<index>`, where `<index>` starts at `1`. Left padding is unnecessary since these labels are not subject to lexicographic sorting. Activation timestamps are required only for forks that occur **after** SilaPrague.
...
We modify the functions `get_base_fee_per_blob_gas` and `calc_excess_blob_gas` defined in [SIP-4844](./sip-4844.md) to explicitly use the blob schedule.
In line with how updating `BLOB_BASE_FEE_UPDATE_FRACTION` was handled in [SIP-7691](./sip-7691.md), the functions use the *current* block's blob schedule.
...
- Sila's recent gas limit increase to 36M took nearly a year to coordinate
