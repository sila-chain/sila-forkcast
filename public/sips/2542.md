...
As concepts of relaying, meta-transactions, gas fees, and account abstraction gain popularity, it becomes critical for some contracts to be able to track gas expenditure with absolute precision. Without access to this data on an SAVM level, such contracts resort to approximation, mimicking SAVM logic on-chain, and some use-cases even become infeasible.
...
Pushes the gas limit of the current execution frame onto the stack. This is the 'callGas' value that was obtained after the application of the SIP-150 "all but one 64th" rule.  
Gas costs: 2 (same as `GASLIMIT`)
...
Consider a solidity smart contract that wants to know how much gas the entire transaction or a part of it had consumed. It is not entirely possible with the current SAVM. With proposed changes, using a pseudo-Solidity syntax, this information would be easily available:
...
A major consideration for this proposal is its alignment with one or many possible future modifications to the SAVM:

1. SIP-2489 Deprecate the GAS opcode (a.k.a. 39-UNGAS proposal)  
 ...
 While criticizing said misconception is beyond the scope of this SIP, in case there is a need to make a breaking change to the behavior of the existing `0x5a GAS` opcode, the same considerations will apply to the proposed opcodes. This means this SIP does not add any new restraints on EMV evolution.
 
2. Stateless Sila  
 The UNGAS proposal is said to be related to the ongoing project of Stateless Sila. It's not strictly necessary for stateless Sila, but it is an idea for how to make future breaking changes to gas schedules easier.  
 As long as the concept of 'gas limit' is part of the SAVM, the author sees no reason proposed opcodes would conflict with Stateless Sila. Gas schedules are not exposed by this proposal.
...
3. Comparison with other controversial opcodes   
 ...
 This SIP neither encourages nor enables any bad security practices, and does not introduce any concepts that are new for SAVM either.  
