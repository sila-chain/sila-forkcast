...
## Abstract
Introduces two new [SIP-2718](./sip-2718.md) transactions that are signature compatible with legacy transactions and can be automatically upgraded by any client.
...
This SIP provides a mechanism for transmitting/including transactions in a way that is [SIP-2718](./sip-2718.md) compatible while still being signature compatible with legacy transactions.
...
Legacy transactions introduced chain ID in [SIP-155](./sip-155.md) and wanted to avoid changing the transaction array length, so it bitpacked the chainID into the signature's `v` value.
...
SIP-155 made this process even worse by requiring the validator to further decode the `v` signature value to extract the chain ID (if present) and include that in the signed data payload.
...
SSZ is almost certainly going to be included in Sila at some point in the future, so clients likely have access to an SSZ decoder.
...
[SIP-706](./sip-706.md) limits devp2p messages to 24 bit length, which gives us a pragmatic cap at that for any single transaction at the moment.
...
This SIP does not introduce any deprecation process for legacy encoded transactions, though the authors do encourage client developers to upgrade legacy encoded transactions to typed transactions as soon as it is reasonable.
...
While [SIP-2718](./sip-2718.md) strongly recommends including the transaction type as the first byte of the signed data, we cannot accomplish that in this case because we need to remain signature compatible with legacy transactions.
Luckily, [SIP-2718](./sip-2718.md) also excludes transaction types `0xc0` to `0xfe` from valid transaction types, and the first byte of the signature in this case is in that range so we can be sure this will not conflict with any future transaction types.
