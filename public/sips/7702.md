...
Add a new [SIP-2718](./sip-2718.md) transaction type that allows Externally
...
back broad adoption of UX improvements across applications. This SIP therefore
...
particular features this SIP is designed around are:

* **Batching**: allowing multiple operations from the same user in one atomic
transaction. One common example is an [SRC-20](./sip-20.md) approval followed by
...
Account X could be paid in some other SRC-20 for this service, or it could be an
...
a permission to spend SRC-20 tokens but not SIL, or to spend up to 1% of the
...
A new [SIP-2718](./sip-2718.md) transaction known as the "set code transaction"
...
transaction follow the same semantics as [SIP-4844](./sip-4844.md). *Note, this
...
3. Let `authority = ecrecover(msg, y_parity, r, s)`.
    ...
    * Verify `s` is less than or equal to `secp256k1n/2`, as specified in
    [SIP-2](./sip-2.md).
4. Add `authority` to `accessed_addresses`, as defined in [SIP-2929](./sip-2929.md).
...
[SIP-3541](./sip-3541.md), to indicate that the code must be handled differently
...
[SIP-2930](./sip-2930.md), specifically `21000 + 16 * non-zero calldata bytes +
...
delegated code, add an additional [SIP-2929](./sip-2929.md)
...
Modify the restriction put in place by [SIP-3607](./sip-3607.md) to allow EOAs
...
Below is the rationale for both general design directions of the SIP, as well as
...
calldata cost, plus the [SIP-3860](./sip-3860.md) cost of `2 * 15 = 30`, plus
...
While this SIP provides a lot of flexibility to applications and EOAs, there are
...
This SIP is designed to be forward-compatible with endgame account abstraction,
without over-enshrining any fine-grained details of [SRC-4337](./sip-4337.md) or
RIP-7560.

To start, the `address` that users sign could directly point to existing
SRC-4337 wallet code. This essentially requires the "code pathways" that are
...
relatively a small subset. The SIP does not require adding any opcodes, that
would become dangling and useless in a post-EOA world, and it allows EOAs to
masquerade as contracts to be included in SRC-4337 bundles, in a way that's
...
is called self-sponsoring. It allows users to take advantage of SIP-7702 without
relying on any third party infrastructure.

However, that means the SIP breaks the invariant that `msg.sender == tx.origin`
...
2. Protecting against atomic sandwich attacks like flash loans, which rely on
   ...
   by this SIP. However, relying on `tx.origin` in this way is considered bad
   practice, and can already be circumvented by miners conditionally including
   transactions in a block.
3. Preventing reentrancy.

Examples of (1) and (2) can be found in contracts deployed on Sila sila-mainnet,
...
SIP did not find any examples of this form of reentrancy protection, though the
search was non-exhaustive.

This distribution of occurrences—many (1), some (2), and no (3)—is exactly what
the authors of this SIP expect because:
...
special cases an SIP has. In early iterations, this SIP resisted a special case
...
Consistency is a valuable property in the SAVM, both from an implementation
...
Creation instructions were considered for a ban as well on other similar SIPs,
however because this SIP allows EOAs to spend value intra-transaction, the
...
presence of `SELFDESTRUCT` (which, as of SilaCancun, only applies in the same
...
one-type-does-all solution. SIP-4844 is treated differently at the p2p level due
to burden blobs place on a node's bandwidth. SIP-7702 has different implications
...
SAVM occurs and verify the change works as expected in that context.

For these reasons, the authors have chosen to keep the scope of the SIP focused
on improving UX.
...
[SIP-1559](./sip-1559.md) transactions.

## Backwards Compatibility

This SIP breaks a few invariants:
...
This SIP does not provide developers the opportunity to run initcode and set
...
[SRC-7201](./sip-7201.md) a contract may root its storage layout at a slot
...
Allowing the sender of an SIP-7702 to also set code has the possibility to:
...
The authors of this SIP believe the risks of allowing this are acceptable for
...
With this SIP, it becomes possible to cause transactions from other accounts to
...
An alternative would be to expand the SIP-7702 transaction with a list of
accounts the caller wishes to "hydrate" during the transaction. Those accounts
behave as the delegated code *only* for SIP-7702 transactions which include them
