...
This SIP makes it possible to call functions that return strings and other dynamically-sized arrays.
Currently, when another contract / function is called from inside the Sila Virtual Machine,
...
The solution proposed in this SIP is to charge gas only for memory that is actually written to at
...
This way of dealing with the problem requires a minimal change to the Sila Virtual Machine.
...
gas is still available. Typical uses of the SIP include "reserving" `2**256-1` bytes of memory for the output.
