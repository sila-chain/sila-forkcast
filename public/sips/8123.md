...
This SIP specifies a new Sila JSON-RPC method, `eth_txGasLimitCap`, which returns the maximum transaction gas limit
...
[SIP-7825](./sip-7825.md) introduces a protocol-level per-transaction gas limit cap (e.g. `2^24 = 16,777,216` on Sila) to bound
...
This SIP adds a new JSON-RPC method and does not modify existing methods. Existing clients and applications remain
compatible.
...
This SIP only exposes information that is already public or otherwise observable by probing. It does not expose secrets
or user data.
