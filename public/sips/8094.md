...
This is the response to GetPooledBlobs, returning the requested blobs. The items in the list are blobs in the format described in the main Sila specification.
...
SIP-4844 introduced the following:
...
- Extend announcements with nonce (see SIP-8077)
...
### Relation to other SIPs in draft state

- SIP-8077 (announce source and nonce): the changes can be simply combined.
- SIP-8070 (Sparse blobpool): both SIPs change how blob transactions are propagated over the network. The goal of the two SIPs are different. SIP-8070 is about a proportional bandwidth reduction in the normal case without dealing with specifics of RBF. This SIP is about enabling RBF without using extra bandwidth. The two SIPs can be combined, but the combination depends on the order of introduction, hence we leave this for later <-- TODO -->.

## Backwards Compatibility

This SIP changes the sil protocol and requires rolling out a new version. Supporting multiple versions of a wire protocol is routine practice. Rolling out this new version does not break older clients, since they can keep using the previous protocol version.
