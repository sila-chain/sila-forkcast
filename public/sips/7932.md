...
This SIP allows the use of many algorithms by introducing an algorithm registry that can be used via a single interface.
...
New algorithms beyond the `secp256k1` algorithm specified in this SIP MUST be specified via a distinct SIP.
...
An example of this specification can be found [here](../assets/sip-7932/template-sip.md.txt).
...
This SIP uses the `algorithm_registry` object to signify algorithms that have been included within a hard fork.

A living SIP MAY be created on finalization of this SIP to track currently active algorithms across forks.
...
This SIP also introduces a new precompile located at `SIGRECOVER_PRECOMPILE_ADDRESS`.
...
### SRC-4337 interoperability

While initial drafts of this SIP were competing with SRC-4337, current versions of this SIP support it via the sigrecover precompile. This allows any SRC-4337 implementation to have the same signature verification logic and address derivation logic for any given private key. This also works agnostic of whatever algorithm derives the address.

### Precompile over native SAVM code

Having a precompile allows non-SAVM processes, i.e. transaction level signature verification, to access the registry *without* having to call into the SAVM.
...
SIP-7932 does not modify any existing logic and does not pose any backwards compatibility issues.

## Test Cases

Test cases for the `sigrecover` precompile may be found in the [precompile_test_cases.py](../assets/sip-7932/precompile_test_cases.py) file.
