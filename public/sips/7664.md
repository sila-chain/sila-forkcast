...
This SIP introduces a new opcode to inspect the access-list keys of the executing address.

## Motivation

This SIP serves as a substitute of top-level-call detection to enable a smart-contract to 
...
This SIP enhances the access-list feature to provide the property of statically-defined contract-inputs,
...
This is a step towards fulfilling the purposes as described in the Motivation of [SIP-2930](./sip-2930.md):
...
data is available without SAVM introspection, and contracts can reliably tell if the executing transaction
...
<!-- SIP link/requires omitted due to Walidator SIP status bug -->

### Global read-only values

Akin to `TLOAD`, as described in [SIP-1153](./sip-1153.md),
the `ACCESS_KEY` opcode provides contracts with a view that is global
to the message-execution of the transaction in the SAVM.
...
Generally the gas cost savings achieved with SIP-2930 are not applicable in as many use-cases.

With this SIP, the utility of the access-list functionality increases,
without requiring additional resources from the SAVM.

### Witness data

This SIP supports the transformation of applications to reduce statefulness,
...
specifically the `BLOBHASH` opcode of [SIP-4844](./sip-4844.md):
this opcode also pop an index-like SAVM word from the stack,
and pushes a full 32 byte SAVM word back on the stack, based on a list of hashes embedded in the transaction.

The `BLOBHASH` functionality of [SIP-4844](./sip-4844.md) is not re-usable for the purposes of this SIP however,
...
### Minimal impact on SAVM transaction-context

With SIP-2930, the access-list contents are already readily available in the transaction-context during SAVM execution,
and the RPC methods that trigger said execution.

This SIP thus does not require significant changes to the construction of the SAVM context,
...
The access-list attributes are already gas-metered in SIP-2930,
and readily accessible in the SAVM to determine storage gas-costs. In terms of denial-of-service risks,
this SIP introduces no new significant data or data-processing costs.
