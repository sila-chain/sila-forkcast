...
## Abstract
An SIP-2718 transaction with the type number `2` is a new type of transaction that includes support for:
...
With the advent of tokens and especially stable coins, it has become common for users to not hold SIL in an account while they may have other assets of value in that account.  Some users don't want to be exposed to the perceived volatility of SIL and instead would prefer to transact using other assets.  Unfortunately, since gas **MUST** be paid for with SIL, this prevents the user from transacting with their assets without first acquiring some SIL using some other means, and then using that SIL to pay fees.

This SIP proposes a mechanism by which we can allow people to transact without ever having to own any SIL by allowing someone else to cover gas costs.  The arrangements that enable the covering of gas costs is out of scope for this SIP but it could be an extra-protocol monthly subscription, payment could occur as part of the transaction being submitted, the recipient may be willing to cover gas costs, or it could be a free service offered as a value-add by a company that you are working with.
...
* If any form of dust-account clearing is introduced, e.g. (https://github.com/sila-chain/SIPs/issues/168), it will be necessary to introduce a replay protection, such as https://github.com/sila-chain/SIPs/issues/169 . Having temporal replay protection removes the need to change nonce-behaviour in the state, since transactions would not be replayable at a later date than explicitly set by the user.
...
## Specification
### Definitions
**`TransactionType`** 2.  See [SIP-2718](./sip-2718.md)
...
As of `FORK_BLOCK_NUMBER` an [SIP-2718](./sip-2718.md) transaction with a `TransactionType` of `2` will have its `Payload` interpreted as an RLP encoded tuple of:
```
...
4. ...whose SIL balance is deducted if any value is attached to the transaction
5. ...whose SIL balance is deducted to pay for gas if `GasPayerSignature` is not present

If `GasPayerSignature` is present, then the address recovered from it is the address...
1. ...whose SIL balance is deducted to pay for gas
...
As of `FORK_BLOCK_NUMBER` an [SIP-2718](./sip-2718.md) transaction receipt with a `TransactionType` of `2` will have its `Payload` interpreted as a `rlp([status, cumulativeGasUsed, logsBloom, logs][])` where each item of the array corresponds to the child-transaction at matching offset in the transaction type 2 `Payload`.

## Rationale
### One Monolithic SIP
This SIP could be split up into multiple SIPs, one for each of the subtypes and one for the meta-type.  Alternatively, each of the subtypes could be a unique TransactionType.  The reason we chose to go with a single SIP with subtypes is because these 4 transactions all have a *lot* in common and each separate SIP would be almost identical to the previous.  We felt that in this case, splitting into multiple SIPs wasn't worth the duplication of SIP content.
### ChainID not encoded with `v`
While we could save one byte in the common case by bundling the y-parity bit of the signature with the Chain ID like in SIP-155, this adds complexity to signing tools that the authors deem not worth it given the size of the transaction overall.
