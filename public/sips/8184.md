...
This is especially concerning because it pushes some of Sila’s most important activity away from the public, composable domain. Offchain market structures may provide reasonable local arrangements, but they are unlikely to constitute a sufficient long-run equilibrium for Sila on their own. The promise of DeFi is not merely that transactions settle onchain; it is that trading, competition, and execution themselves can occur permissionlessly onchain. 

Another core property is censorship resistance (CR), but because [SIP-7805](./sip-7805.md) (FOCIL) relies on open propagation, it cannot provide CR for transactions subject to MEV extraction. A public encrypted mempool addresses this gap. If transactions can be propagated in encrypted form and committed to before their contents are revealed, users no longer need to choose between MEV protection and trustless access to blockspace. This broadens the practical censorship resistance of inclusion lists while reducing the importance of privileged private order flow in builder competition.

At the same time, Sila cannot presently enshrine a single encryption scheme for this purpose. A viable in-protocol scheme requires a silent setup with small public keys, non-interactive decryption, no trusted setup, practical ciphertext sizes, CCA2 security, and a credible path to quantum safety. No known construction fully satisfies these requirements at Sila’s scale, and waiting for cryptographic breakthroughs would impose an unacceptable delay.

LUCID extends the inclusion pipeline to sealed transactions (STs) that can be propagated in public without revealing their contents. Builders commit to them before decryption, and key publishers release keys only after the scheduling decision has been fixed. In doing so, it preserves a public, permissionless path to inclusion for MEV-sensitive transactions without coupling Sila to a specific encryption scheme. Transactions may be self-decrypted in a trustless manner or encrypted in an open design according to a key publisher's instructions. This broadens the practical CR afforded by FOCIL while reducing Sila’s dependence on centralized order-flow relationships. In the longer term, it helps preserve an Sila in which trading and transaction competition can take place onchain, in public, and under permissionless market structure.
...
MAX_BYTES_PER_ST = 2**24 // 64 # Byte bound implied by SIP-7976 and SIP-7825
MAX_ST_COMMITS = MAX_ST_COMMITS_PER_IL * IL_COMMITTEE_SIZE # IL_COMMITTEE_SIZE defined in SIP-7805
...
![Figure 1](../assets/sip-8184/1.png)
...
It is encoded in RLP when propagating across the mempool, as a new [SIP-2718](./sip-2718.md) transaction.
...
An ST ticket is a new [SIP-2718](./sip-2718.md) typed transaction. It is charged and consumes the sender's execution nonce, but is not executed as a normal SAVM message call.
...
The `ciphertext_envelope` decrypts to `plaintext_tx`, the raw bytes of an [SIP-1559](./sip-1559.md) transaction to be executed onchain.
...
The `gas_limit` must be sufficient to cover the calldata cost of the byte size of the `SealedTransaction`. [SIP-7825](./sip-7825.md) thus bounds the byte size of an ST.
...
The `InclusionList` container of [SIP-7805](./sip-7805.md) is extended with STs and ST bundles:
...
* Update `/sil/v2/events` endpoint to notify wallets that their ST has been included, and it is safe to publish the key.
* Update `/sil/v2/events` endpoint to notify clients and committees that a key has been released.
* Add `/sil/v1/beacon/key` endpoint to relay the key to the libp2p topic.
...
LUCID can be integrated with [SIP-8141](./sip-8141.md) by allowing `signature_id = FRAME_TX_TYPE` to indicate that the ST ticket has no standalone signature and must instead appear inside an enclosing frame transaction. In this mode, the frame transaction has a fixed layout: the first frame is a `VERIFY` frame that authenticates the outer frame transaction, and the second frame is a new `ST` frame whose `data` is the full `SealedTransaction` bytes. P2P nodes extract the `SealedTransaction` from the `ST` frame and otherwise validate it as they do today. The EL extracts the ST ticket from the ST and derives `ticket_from` and `ticket_nonce` from `tx.sender` and `tx.nonce` of the enclosing frame transaction respectively.

Bundles can be supported similarly by introducing a reserved `ST_BUNDLE` frame whose `data` is the full `SealedBundle` bytes and adding `FRAME_TX_TYPE` as a valid `key_publisher_signature_id` in `SealedBundle`. Integration requires a future SIP making coordinated additions to both [SIP-8141](./sip-8141.md) and LUCID.
