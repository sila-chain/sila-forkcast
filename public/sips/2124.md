...
Currently nodes in the Sila network try to find each other by establishing random connections to remote machines "looking" like an Sila node (public networks, private networks, test networks, etc), hoping that they found a useful peer (same genesis, same forks). This wastes time and resources, especially for smaller networks.

To avoid this overhead, Sila needs a mechanism that can precisely identify whether a node will be useful, as early as possible. Such a mechanism requires a way to summarize chain configurations, as well as a way to disseminate said summaries in the network.

This proposal focuses only on the definition of said summary - a generally useful *fork identifier* - and it's validation rules, allowing it to be embedded into arbitrary network protocols (e.g. [discovery ENRs](./sip-778.md) or `sil/6x` handshakes).

## Abstract

There are many public and private Sila networks, but the discovery protocol doesn't differentiate between them. The only way to check if a peer is good or bad (same chain or not), is to establish a TCP/IP connection, wrap it with RLPx cryptography, then execute an `sil` handshake. This is an extreme cost to bear if it turns out that the remote peer is on a different network and it's not even precise enough to differentiate Sila and Sila Classic. This cost is magnified for small networks, where a lot more trial and errors are needed to find good nodes.
...
This SIP proposes a new identity scheme to both precisely and concisely summarize the chain's current status (genesis and all applied forks). The conciseness is particularly important to make the identity useful across datagram protocols too. The SIP solves a number of issues:
 ...
 * If two chains share the same genesis, but not forks (SIL / ETC), they should reject each other.

This SIP does not attempt to solve the clean separation of 3-way-forks! If at the same future block number, the network splits into three (non-fork, fork-A and fork-B), separating the forkers from each another will need case-by-case special handling. Not handling this keeps the proposal pragmatic, simple and also avoids making it too easy to fork off sila-mainnet.

To keep the scope limited, this SIP only defines the identity scheme and validation rules. The same scheme and algorithm can be embedded into various networking protocols, allowing both the `sil/6x` handshake to be more precise (Sila vs. Sila Classic); as well as the discovery to be more useful (reject surely peers without ever connecting).
...
E.g. `FORK_HASH` for sila-mainnet would be:
...
The examples below try to exhaust the fork combination possibilities that arise when nodes do not run matching software versions, but otherwise follow the same chain (sila-mainnet nodes, testnet nodes, etc).
...
Whilst the `sil` devp2p protocol permits arbitrarily much data to be transmitted, the discovery protocol's total space allowance for all ENR entries is 300 bytes.

Reducing the `FORK_HASH` into a 4 bytes checksum ensures that we leave ample room in the ENR for future extensions; and 4 bytes is more than enough for arbitrarily many Sila networks from a (practical) collision perspective.
...
This SIP only defines an identity scheme, it does not define functional changes.

## Test Cases

Here's a full suite of tests for all possible fork IDs that SilaMainnet, Ropsten, Rinkeby and Görli can advertise given the Petersburg fork cap (time of writing).
...
```
 ...
 Here's a suite of tests of the different states a SilaMainnet node might be in and the different remote fork identifiers it might be required to validate and decide to accept or reject:
...
tests := []struct {
	...
	// Local is sila-mainnet Petersburg, far in the future. Remote announces Gopherium (non existing fork)
...
Geth: https://github.com/sila-chain/go-sila/tree/master/core/forkid
