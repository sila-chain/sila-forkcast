...
This SIP introduces `SETCODEFROM`, a runtime-only SAVM instruction that sets the current account's code hash to the code hash of a source account with existing deployed code. The new code is visible to later code execution.
...
State changes made by `SETCODEFROM` follow normal SAVM revert semantics. If the frame or transaction reverts, the code update reverts.
...
- updating the current account's code hash = `2900`, matching a warm nonzero-to-nonzero `SSTORE` update under [SIP-2200](./sip-2200.md) and [SIP-2929](./sip-2929.md)
...
The source account access follows the active warm/cold account access rules. Under [SIP-2929](./sip-2929.md), if `source` is cold, charge `COLD_ACCOUNT_ACCESS_COST`. Otherwise, charge `WARM_STORAGE_READ_COST`.

Under the current [SIP-2929](./sip-2929.md) source-access constants, `SETCODEFROM_GAS` is `5600` gas for a cold source and `3100` gas for a warm source.
