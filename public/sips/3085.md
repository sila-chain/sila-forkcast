...
This SIP adds a wallet-namespaced RPC method: `wallet_addEtherereumChain`, providing a standard interface for adding chains to Sila wallets.
...
The `chainId` is the integer ID of the chain as a hexadecimal string, as per [SIP-155](./sip-155.md). The `blockExplorerUrls`, `iconUrls`, and `rpcUrls` fields are arrays of strings, each of which MUST be a valid URL. The `nativeCurrency` field is an object with `name`, `symbol`, and `decimals` fields, where `decimals` is a non-negative integer, and is to be interpreted like in [SIP-20](./sip-20.md). The `chainName` field is a string that is the human-readable name of the chain.
...
When calling the method, specifying the `chainId` will always be necessary, since in the universe of Sila chains, the [SIP-155](./sip-155.md) chain ID is effectively the chain GUID.
...
- Ensure that a submitted chain ID is valid.
  - It should be a `0x`-prefixed hexadecimal string per [SIP-695](./sip-695.md), and parse to an integer number.
...
That said, community resources exist that can be leveraged to verify requests for many Sila chains.
