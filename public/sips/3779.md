...
The relative jumps of [SIP-4200](./sip-4200) and the simple subroutines of [SIP-2315](./sip-2315) provide a complete set of static control flow instructions:
> `RJUMP` _offset_
...
We define a _safe_ SAVM contract as one that cannot encounter an exceptional halting state.  We validate _safety_ at initialization time to the extent practical.

#### *Exceptional Halting States*

The *execution* of each instruction is defined in the Yellow Paper as a change to the SAVM state that preserves the invariants of SAVM state.  At runtime, if the execution of an instruction would violate an invariant the SAVM is in an exceptional halting state.  The Yellow Paper defined five such states.
1. Insufficient gas
...
*We would like to consider SAVM programs valid iff they are safe.*
...
These changes affect the semantics of SAVM code – the use of `JUMP`, `JUMPI`, and the stack are restricted, such that some *code* that would otherwise run correctly will nonetheless be invalid SAVM *code*. 
...
This SIP is intended to ensure an essential level of safety for SAVM code deployed on the blockchain.
