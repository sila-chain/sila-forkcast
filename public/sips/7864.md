...
Introduce a new binary state tree, intended to replace the hexary patricia trees. Account headers, contract code and storage are stored in a single logical tree. Account data is broken into independent leaves which  are grouped by 256 in order to provide some locality. The tree is also grouped by storage type (headers vs code vs storage), to enable separate syncing and distinct treatment of different types of storage that in the future will have different properties.

Note: The hash function used in the current draft is not final. The current implementation uses BLAKE3 to reduce friction for EL clients experimenting with this SIP, but the final decision remains TBD. Other potential candidates include Keccak and Poseidon2. 

For Poseidon2, there's an ongoing Sila Foundation cryptography initiative assessing its security properties. If Poseidon2 is selected, additional specifications will be needed for field selection (BN254 scalar field, 31-bit field elements, etc.) and encoding 32-byte values into field elements.

The hash function can be switched with minimal impact on implementations as this SIP progresses toward acceptance. **Do not** assume BLAKE3 is a final decision.

## Motivation

Sila's long-term goal is to allow blocks to be proved with validity proof so that chain verification is as simple and fast as possible. One of the most challenging parts of achieving that goal is proving the state of the tree, which is required for SAVM execution.
...
- All state of the same type (eg. headers, code, storage) is stored together.

### Tree structure

The proposed Binary Tree stores key-value entries where the key is bytes (arbitrary length, but this SIP guarantees that it will be prefix-free) and the value is bytes32. The first byte of the tree denotes storage type, the next bytes of the key define the entry stem, and the last byte is the subindex in that stem. If two keys have the same stem, they live in the same "big branch" — this co-locates groups of 256 keys (i.e: keys with the same first 31-bytes).

![Image](../assets/sip-7864/diagram2.png)
...
The path to a `StemNode` is defined by the key's first N-8 bits (N-1 bytes) from MSB to LSB. From this node, a subtree of 256 values exists indexed by the key's last byte (8 bits). A newly created `StemNode` subtree (i.e: 256 values) has all leaves with `value: empty`.
...
def get_tree_key(storage_type: int, tree_position: Bytes, sub_index: int):
    return (
        bytes([storage_type]) +
        tree_position +
        bytes([sub_index])
...
def get_tree_key_for_code_chunk(address: Address32, chunk_id: int):
    if chunk_id < CODE_CHUNKS_IN_HEADER:
        return get_tree_key(HEADER_SUBTREE, hash(address), CODE_OFFSET + chunk_id)
    else:
        high = (chunk_id - CODE_CHUNKS_IN_HEADER) // STEM_SUBTREE_WIDTH
        low = (chunk_id - CODE_CHUNKS_IN_HEADER) % STEM_SUBTREE_WIDTH
        return get_tree_key(CODE_SUBTREE, hash(address + bytes([high])), low)
```
...
def get_tree_key_for_storage_slot(address: Address32, storage_key: int):
    if storage_key < STORAGE_CHUNKS_IN_HEADER:
        return get_tree_key(HEADER_SUBTREE, hash(address), STORAGE_OFFSET + chunk_id)
    else:
        high = (storage_key - STORAGE_CHUNKS_IN_HEADER) // STEM_SUBTREE_WIDTH
        low = (storage_key - STORAGE_CHUNKS_IN_HEADER) % STEM_SUBTREE_WIDTH
        return get_tree_key(STORAGE_SUBTREE, hash(address) + hash(address + int_to_bytes(high)), low)
```
...
Described in [SIP-7612](./sip-7612.md).

### Access events

Described in [SIP-4762](./sip-4762.md).

## Rationale

This SIP defines a new Binary Tree that starts empty. Only new state changes are stored in the tree. The MPT continues to exist but is frozen. This sets the stage for a future hard fork that migrates the MPT data to this Binary Tree ([SIP-7748](./sip-7748.md)).

### Single tree design

The proposal uses a single-layer tree structure with bytes as keys and 32 byte values for several reasons:
...
### Storage type as prefix

This proposal has the storage type (headers, code, storage) as the prefix of the tree key, for several reasons:

- **Separate syncability**: it's possible to sync one subtree without syncing others. This will be especially valuable in the near term for syncing VOPS
- **Extensibility**: it's easier to handle different types of storage differently. For example, nodes could simply not store some types of state, or store them in some compacted way that depends on its structure, and because each type is in its own subtree, the need to maintain Merkle sister nodes to update the state would not interfere with this.

### `hash(address) + hash(address + storage_key_prefix)` formula

This ensures that storage keys that get "grinded" to be close to each other cannot be reused between different contracts, mitigating a DoS vector.
...
2. **Keccak**:
   - Already used in Sila, reducing implementation complexity
...
State-expiry strategies such as [SIP-7736](./sip-7736.md) could still be applied, requiring a change in the design. One potential solution is adding a field to the `StemNode` with `epoch` as described in the mentioned SIP. Another alternative is to use 247-bits for the stem, and have two subtrees `StemValuesNode`, which would correspond to the current 256-values, and `StemMetaNode` which is also a 256-subtree that can be used to store arbitrary stem metadata.
...
- (2) Tree structure change makes in-SAVM proofs of historical state no longer work

(1) can be mitigated by increasing the gas limit while implementing this SIP, allowing the same economic viability for applications.
