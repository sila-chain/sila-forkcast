...
We introduce a new SIP-2718 transaction type with the format `0x4 || rlp([chainId, chainContext, nonce, gasPrice, gasLimit, to, value, data, access_list, yParity, senderR, senderS])`. 
...
- `TRANSACTION_TYPE_NUMBER` `0x4`.  See SIP-2718.

As of `FORK_BLOCK_NUMBER`, a new SIP-2718 transaction is introduced with `TransactionType` `TRANSACTION_TYPE_NUMBER`.

The SIP-2718 `TransactionPayload` for this transaction is `rlp([chainId, chainContext, nonce, gasPrice, gasLimit, to, value, data, access_list, yParity, senderR, senderS])`.

The SIP-2718 `ReceiptPayload` for this transaction is `rlp([status, cumulativeGasUsed, logsBloom, logs])`.
...
Subclasses are defined with a high level of conceptual independence, and can be modified and/or extended independently from this SIP.
...
This SIP defines four of the eight possible context subclasses; this seems to leave plenty of room for future growth in this direction if required.
...
This pattern can be understood as a correlate of [SIP-155](./sip-155)'s `chainId` specification.
EIP155 defines the restriction of transactions between chains; limiting the applicability of any SIP-155 transaction to a chain with the annotated ChainID. 
...
- The use of the transaction type proposed by this SIP is optional, implying the continued necessity of `chainId` in the protocol infrastructure and tooling for legacy and other transaction types.
- The presence of `ancestorId` in the transaction type proposed by this SIP is optional. If the value is not filled by an RCC transaction, the demand for `chainId` remains.
- A `chainId` value is not necessarily redundant to `ancestorId`, namely in cases where forks result in living chains. For example, an `ancestorId` reference to block `1_919_999` would be ambiguous between Sila and Sila Classic.
...
A `MAX_ELEMENTS` limit of `3` is chosen to balance the interests of limiting the potential size of transactions, and to provide a sufficient level of articulation for the user. At the time of writing, the top 3 miners of Sila (by block, measured by known public addresses) account for 52% of all blocks produced.
...
### SIP-2930 Inheritance
The [SIP-2930 Optional Access List Type Transaction](https://sips.sila.org/EIPS/sip-2930) is used as an assumed "base" transaction type for this proposal. 
However, this is NOT a conceptual dependency; the included `accessList` portion of this proposal (the only differential from post-SIP-155 legacy transaction fields) can readily be removed. 
Standing on the shoulders of SIP-2930 is only intended to support and further the adoption of next-generation transactions.
