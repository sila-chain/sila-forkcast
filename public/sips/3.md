...
[1] a.k.a "shallow stack attack" and "stack attack". However, to be precise, the word ''stack'' has a different meaning within the SAVM, and is not to be confused with the ''call stack''.
...
The actual call stack depth, as well as the call stack depth limit, are present in the SAVM during execution, but just not available within the SAVM. The implementation should be fairly simple and would provide a cheap and way to protect against call stack attacks.
