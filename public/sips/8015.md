...
This SIP removes the legacy `deposits` and `eth1_data` fields from the `BeaconBlockBody` structure after [SIP-6110](./sip-6110.md) has been fully finalized. These fields become obsolete once all validators have transitioned to the new in-protocol deposit processing mechanism introduced in SIP-6110.
...
Once SIP-6110 has been fully finalized and the transition period is complete (when `state.eth1_deposit_index == state.deposit_requests_start_index`), the legacy deposit fields become permanently unused and can be safely removed. This cleanup provides several benefits:
...
### Dependency on SIP-7732

This SIP requires SIP-7732 because:

- SIP-7732 already modified the `BeaconBlockBody` structure significantly
