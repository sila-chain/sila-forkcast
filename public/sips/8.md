...
This SIP introduces new forward-compatibility requirements for implementations of the
devp2p Wire Protocol, the RLPx Discovery Protocol and the RLPx TCP Transport Protocol.
Clients which implement SIP-8 behave according to Postel's Law:
...
encoding for the encrypted key establishment handshake packets. If an SIP-8 style RLPx
...
sha3(MESSAGE)
    is the Keccak256 hash function as used by Sila.
...
will ensure that all client software in use on the Sila network can cope with future
...
This SIP is backwards-compatible, all valid version 4 packets are still accepted.
...
(Auth₂) SIP-8 format with version 4 and no additional list elements (sent from A to B):
```text
...
(Auth₃) SIP-8 format with version 56 and 3 additional list elements (sent from A to B):
```text
...
(Ack₂) SIP-8 format with version 4 and no additional list elements (sent from B to A):
```text
...
(Ack₃) SIP-8 format with version 57 and 3 additional list elements (sent from B to A):
```text
