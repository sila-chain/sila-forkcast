...
**It is not slashable.** Altair defines no slashing condition for sync committee messages. A corrupted committee can sign a header for a chain that does not exist and lose nothing. [SIP-7657](./sip-7657.md) proposes adding a slashing condition, which is the clearest evidence that none exists today.

**It is a small sample.** The active set is 901,505 validators holding 42,328,615 SIL. The committee is 512 of them, one seat per 1,761 validators. A light client that follows it is backed by 0.06% of the validator set, rotated daily, with no penalty for lying.

**Issuance pays for it.** `SYNC_REWARD_WEIGHT` is 2 against a `WEIGHT_DENOMINATOR` of 64, so the committee takes `1/32` of consensus issuance. At the current stake that is about 33,800 SIL a year, out of about 1,082,000 SIL.
...
Names used below without definition are defined in the `sila/consensus-specs` repository. `EIP8390_FORK_EPOCH` is the activation epoch.
...
def process_block(state: BeaconState, block: BeaconBlock) -> None:
    ...
    # process_sync_aggregate(state, block.body.sync_aggregate)  # [Removed in SIP-8390]
```
...
**Beacon API.** These endpoints are removed: `POST /sil/v1/beacon/pool/sync_committees`, `GET /sil/v1/beacon/states/{state_id}/sync_committees`, `POST /sil/v1/validator/duties/sync/{epoch}`, `GET /sil/v1/validator/sync_committee_contribution`, `POST /sil/v1/validator/contribution_and_proofs`, `POST /sil/v1/validator/sync_committee_subscriptions`, and `/sil/v1/beacon/light_client/*`.
...
Removing the sync committee removes the only in-protocol way for a light client to follow the chain. Before this SIP activates, public proving infrastructure must exist that publishes finality proofs with characteristics its consumers can rely on. This SIP does not specify that infrastructure, and scheduling is decided by client teams rather than by this document.

### Proof production is not incentivised

This SIP adds no in-protocol incentive to produce finality proofs, and does not propose one.
