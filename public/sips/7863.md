...
1. The first access to a slot in a block incurs the cold access cost as of [SIP-2929](./sip-2929.md).
2. All subsequent accesses to the same slot within the same block incur only the warm access cost as of [SIP-2929](./sip-2929.md).
...
def process_message_call(message: Message, env: Environment) -> MessageCallOutput:
    return MessageCallOutput(
        # ... other fields ...
        accessed_addresses=savm.accessed_addresses,
        accessed_storage_keys=savm.accessed_storage_keys
    )
```
...
1. **Caching Efficiency**: Modern Sila clients already implement sophisticated caching mechanisms at the block level. Extending address and storage key warming to match this caching behavior better aligns gas costs with actual computational costs.
