...
**Type-3 transaction:** Refers to SIP-4844 blob-carrying transactions (transaction type 0x03). These transactions include blob versioned hashes that commit to blobs carrying user data.
...
These parameters are defined in SIP-4844 and related specs:

| Name | Value | Source |
|------|-------|--------|
| `FIELD_ELEMENTS_PER_BLOB` | `4096` | SIP-4844 |
| `BYTES_PER_FIELD_ELEMENT` | `32` | SIP-4844 |
| `GAS_PER_BLOB` | `2**17` | SIP-4844 |
| `MAX_BLOBS_PER_BLOCK` | Network-specific | SIP-7892 |

**Note on `MAX_BLOBS_PER_BLOCK`:** This constant represents the maximum number of blobs (both payload-blobs and type-3 transaction blobs) that can be included in a block. Per SIP-7892, the execution layer's `blobSchedule.max` MUST equal the consensus layer's `MAX_BLOBS_PER_BLOCK` configuration value at any given time. This value may change across forks (e.g., initially 6 in SIP-4844, potentially increased in subsequent blob throughput increase proposals).

#### Derived Constants

| Name | Value | Description |
|------|-------|-------------|
| `USABLE_BYTES_PER_FIELD_ELEMENT` | `BYTES_PER_FIELD_ELEMENT - 1` (31) | Usable bytes per field element |
| `USABLE_BYTES_PER_BLOB` | `FIELD_ELEMENTS_PER_BLOB * USABLE_BYTES_PER_FIELD_ELEMENT` | Total usable bytes per blob |

**Note on `USABLE_BYTES_PER_FIELD_ELEMENT`**: Field elements are interpreted big-endian (see [execution-specs](https://github.com/sila-chain/execution-specs/blob/6137bb32f9b8eda6194096992cb97b28be06957d/src/sila/crypto/kzg.py#L102)). This SIP sets the first byte (MSB) to zero to ensure that every 32-byte element is a canonical BLS field element (`value < BLS_MODULUS`).
...
Specifically, we use the following methods from [polynomial-commitments.md](https://github.com/sila-chain/consensus-specs/blob/46c1199d6b4584ba484dec807f03b8e6211dd725/specs/deneb/polynomial-commitments.md?#introduction):

- [verify_blob_kzg_proof_batch](https://github.com/sila-chain/consensus-specs/blob/46c1199d6b4584ba484dec807f03b8e6211dd725/specs/deneb/polynomial-commitments.md#L578)
- [blob_to_kzg_commitment](https://github.com/sila-chain/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/deneb/polynomial-commitments.md#blob_to_kzg_commitment)
- [compute_blob_kzg_proof](https://github.com/sila-chain/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/deneb/polynomial-commitments.md#compute_blob_kzg_proof)

And the following methods from [polynomial-commitments-sampling.md](https://github.com/sila-chain/consensus-specs/blob/2938e1ad74cea54f1a24508a85704d5bd87837ad/specs/fulu/polynomial-commitments-sampling.md):

- [compute_cells_and_kzg_proofs](https://github.com/sila-chain/consensus-specs/blob/2938e1ad74cea54f1a24508a85704d5bd87837ad/specs/fulu/polynomial-commitments-sampling.md#compute_cells_and_kzg_proofs)

And the following methods from [beacon-chain.md](https://github.com/sila-chain/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/deneb/beacon-chain.md#introduction):

- [kzg_commitment_to_versioned_hash](https://github.com/sila-chain/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/deneb/beacon-chain.md#kzg_commitment_to_versioned_hash)
...
def chunk_to_blob(data: bytes) -> Blob:
    ...
    Each 31-byte chunk is stored in bytes [1:32] of a field element,
    with byte [0] (MSB) set to zero to ensure value < BLS modulus.
    """
    ...
    for i in range(FIELD_ELEMENTS_PER_BLOB):
        ...
        # Store 31 data bytes in [1:32], the first byte [0] stays zero
        field_element_start = i * BYTES_PER_FIELD_ELEMENT
        blob[field_element_start + 1 : field_element_start + BYTES_PER_FIELD_ELEMENT] = chunk
...
def blob_to_chunk(blob: Blob) -> bytes:
    ...
    Validates that the first byte is zero for each field element.
    """
    result = bytearray()
    ...
    for i in range(FIELD_ELEMENTS_PER_BLOB):
        ...
        # Validate first byte is zero
        assert blob[field_element_start] == 0x00, "Invalid blob: first byte must be zero"

        # Extract usable data bytes
        result.extend(blob[field_element_start + 1 : field_element_start + BYTES_PER_FIELD_ELEMENT])
...
def execution_payload_data_to_blobs(data: ExecutionPayloadData) -> List[Blob]:
    ...
    Note: blockAccessList is already RLP-encoded per SIP-7928. Transactions are RLP-encoded as a list.
    """
...
def blobs_to_execution_payload_data(blobs: List[Blob]) -> ExecutionPayloadData:
    ...
    # Verify that declared length does not exceed data length
    ...
    # The rest must be padding, no trailing data allowed
    trailing = raw[8 + bal_length + txs_length:]
    assert all(b == 0 for b in trailing)
...
- `blockAccessList` (added in SIP-7928)
- `transactions`
...
class ExecutionPayloadData(Container):
    # Block Access List (BAL) from SIP-7928, RLP-encoded
...
This SIP adds a new field to the header:
...
For the zkEVM-optimized variant of `engine_getPayload`, this SIP extends `BlobsBundle` with an additional field:
...
This SIP adds a new field to the `ExecutionPayload`:
...
The consensus layer does not introduce new blob specific validation rules for payload-blobs beyond what we have for SIP-4844/SIP-7594.
...
This SIP does not mandate that payload-blobs pay a per-blob fee like transaction blobs.
...
Execution-payload data includes `blockAccessList` (from SIP-7928) and `transactions`.
...
**Endianness:** BLS field elements already use big-endian encoding. The lengths in the 8-byte header also use big-endian encoding for consistency.
...
This SIP specifies that the block builder choose `payload_blob_count`, subject to the constraint imposed by `MAX_BLOBS_PER_BLOCK`.
...
While a more zk-friendly serialization algorithm could be beneficial in the future, this SIP uses RLP for simplicity. Once SIP-7807 (SSZ execution blocks) is implemented, the encoding can be updated to SSZ-serialize the list of SSZ-encoded transaction bytes.
