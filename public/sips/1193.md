...
A JavaScript Sila Provider API for consistency across clients and applications.

## Abstract

A common convention in the Sila web application ("dapp") ecosystem is for key management software ("wallets") to expose their API via a JavaScript object in the web page.
...
This SIP formalizes an Sila Provider API to promote wallet interoperability.
...
Historically, Providers have been made available as `window.sila` in web browsers, but this convention is not part of the specification.
...
- Provider
  - A JavaScript object made available to a consumer, that provides access to Sila by means of a Client.
- Client
...
If an RPC method defined in a finalized SIP is not supported, it **SHOULD** be rejected with a `4200` error per the [Provider Errors](#provider-errors) section below, or an appropriate error per the RPC method's specification.
...
If the Provider supports Sila RPC subscriptions, e.g. [`eth_subscribe`](https://geth.sila.org/docs/rpc/pubsub), the Provider **MUST** emit the `message` event when it receives a subscription notification.
...
`chainId` **MUST** specify the integer ID of the connected chain as a hexadecimal string, per the [`eth_chainId`](./sip-695.md) Sila RPC method.
...
If the chain the Provider is connected to changes, the Provider **MUST** emit the event named `chainChanged` with value `chainId: string`, specifying the integer ID of the new chain as a hexadecimal string, per the [`eth_chainId`](./sip-695.md) Sila RPC method.

#### accountsChanged

If the accounts available to the Provider change, the Provider **MUST** emit the event named `accountsChanged` with value `accounts: string[]`, containing the account addresses per the `eth_accounts` Sila RPC method.

The "accounts available to the Provider" change when the return value of `eth_accounts` changes.

## Rationale

The purpose of a Provider is to _provide_ a consumer with access to Sila.
In general, a Provider must enable an Sila web application to do two things:

- Make Sila RPC requests
- Respond to state changes in the Provider's Sila chain, Client, and Wallet
...
Providers that only implement this specification will not be compatible with Sila web applications that target the legacy API.
...
- [sil-provider](https://www.npmjs.com/package/sil-provider)
...
The Provider is intended to pass messages between an Sila Client and an Sila application.
It is _not_ responsible for private key or account management; it merely processes RPC messages and emits events.
Consequently, account security and user privacy need to be implemented in middlewares between the Provider and its Sila Client.
...
Since all Sila operations are directed at a particular chain, it's important that the Provider accurately reflects the Client's configured chain, per the `eth_chainId` Sila RPC method (see [SIP-695](./sip-695.md)).
...
Many Sila write operations (e.g. `eth_sendTransaction`) require a user account to be specified.
...
Instead, Providers should support RPC methods for explicitly requesting account access, such as `eth_requestAccounts` (see [SIP-1102](./sip-1102.md)) or `wallet_requestPermissions` (see [SIP-2255](./sip-2255.md)).

## References

- [Initial discussion in `sila/interfaces`](https://github.com/sila-chain/interfaces/issues/16)
- [Deprecated Sila Magicians thread](https://sila-magicians.org/t/sip-1193-sila-provider-javascript-api/640)
- [Continuing discussion](https://github.com/sila-chain/SIPs/issues/2319)
- Related SIPs
  - [SIP-1102: Opt-in Account Exposure](./sip-1102.md)
  - [SIP-1474: Remote Procedure Call Specification](./sip-1474.md)
  - [SIP-1767: GraphQL Interface to Sila Node Data](./sip-1767.md)
  - [SIP-2255: Wallet Permissions](./sip-2255.md)
...
Consult each Sila RPC method's documentation for its `params` and return type.
You can find a list of common methods [here](./sip-1474.md).

#### RPC Protocols

Multiple RPC protocols may be available. For examples, see:

- [SIP-1474](./sip-1474.md), the Sila JSON-RPC API
- [SIP-1767](./sip-1767.md), the Sila GraphQL schema
...
The event emits an object with a hexadecimal string `chainId` per the `eth_chainId` Sila RPC method, and other properties as determined by the Provider.
...
The event emits a hexadecimal string `chainId` per the `eth_chainId` Sila RPC method.
...
The event emits with `accounts`, an array of account addresses, per the `eth_accounts` Sila RPC method.
...
[`eth_` subscription methods](https://geth.sila.org/docs/rpc/pubsub) and [`shh_` subscription methods](https://github.com/sila-chain/go-sila/wiki/Whisper-v6-RPC-API#shh_subscribe) rely on this event to emit subscription updates.
...
```javascript
// Most Providers are available as window.sila on page load.
...
sila.on('accountsChanged', logAccounts);
// to unsubscribe
sila.removeListener('accountsChanged', logAccounts);

// Example 6: Log if connection ends
sila.on('disconnect', (code, reason) => {
  console.log(`Sila Provider connection closed: ${reason}. Code: ${code}`);
});
```
...
The authors recommend against implementing it except to support legacy Sila applications.
...
Historically, the request and response object interfaces have followed the [Sila JSON-RPC specification](./sip-1474.md).
...
For details, see [SIP-155: Simple replay attack protection](./sip-155.md) and [SIP-695: Create eth_chainId method for JSON-RPC](./sip-695.md).
