...
This SIP describes a scheme to charge for data in state, and 'archive' data which is no longer being paid for. It also describes how resurrection of 'archived' data happens.

## Motivation

The Sila blockchain in its current form is not sustainable because it grows
indefinitely. This is true of any blockchain, but Sila grows faster than most chains.
...
protocol. This SIP attempts to correct this by adding new consensus rules that put an
upper bound on the size of the Sila state.

Adding these new rules changes fundamental guarantees of the system and requires a hard
fork. Users of Sila already pay for the creation and modification of accounts and
their storage entries. Under the rules introduced in this SIP, users must also pay to keep
accounts accessible. A similar rent scheme was proposed in [SIP-103] but rejected
even back then because the proposal would've upset peoples expectations. As implementers
of Sila, we still feel that state rent is the right path to long-term sustainability
of the Sila blockchain and that its undesirable implications can be overcome with
off-protocol tooling and careful design.

[SIP-103]: https://github.com/sila-chain/SIPs/issues/35

## Specification

The cost of storing an account over time is called `rent`. The amount of `rent` due depends
on the size of the account. The `sila` that is paid for `rent` is destroyed. The `rent` is deducted whenever an account is touched.

`rent` can be paid from the account's regular `balance` or from its 'rent balance'. Accounts
can be endowed with `rent balance` through a new SAVM opcode. When `rent` is charged, it is
...
The reason to separate `balance` and `rent balance` is that certain contracts do not accept `sila` sends, or always send the entire balance off to some other destination. For these cases, a separate`rent balance` is required. 
...
Before processing the block in which this SIP becomes active, clients iterate the whole
...
`PAYRENT` deducts the given amount of `sila` from the account executing the opcode and adds
...
One of the fundamental guarantees provided by Sila is that changes to contract storage
