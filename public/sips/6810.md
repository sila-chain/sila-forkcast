...
While Sila has the capability of reversible transactions through smart contracts, instant settlement is the default.
...
A new [SIP-2718](./sip-2718.md) transaction is introduced with `TransactionType` `0x5a`.
The [SIP-2718](./sip-2718.md) `TransactionPayload` for this transaction is `rlp([chainId, nonce, revertNonce, budget, signatureYParity, signatureR, signatureS])`.
...
The [SIP-2718](./sip-2718.md) `ReceiptPayload` for this transaction is `rlp([status, budgetUsed, removedLogsBloom, [newReceiptPayloads]])`, where `newReceiptPayloads` is a sequential array of the updated receipts of all reverted transactions.
...
A transaction fee budget is initialized to the value specified by `budget`, denominated in sila.
