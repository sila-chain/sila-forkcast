...
The growth of the Sila state has caused certain opcodes to be more resource-intensive at this point than 
...
The following graph shows a go-sila full sync, where each data point represents
...
* This SIP proposes to extend the current opcode set.
  ...
  * As for why it is priced at `5` (`GasFastStep`) instead of `2` (`GasQuickStep`), like other similar operations: the SAVM execution engine still needs a lookup into the (cached) trie, and `balance`, unlike `gasPrice` or `timeStamp`, is not constant during the execution, so it has a bit more inherent overhead. 


### `EXTCODEHASH`

`EXTCODEHASH` was introduced in Constantinople, with [SIP-1052](./sip-1052.md). It was priced at `400` with the reasoning:
...
- Contracts that assume a certain fixed gas cost for calls (or internal sections) may cease to function.
  - A fixed gas cost is specified in [SRC-165](./sip-165.md) and implementations of this interface do use the affected opcodes.
    - The SRC-165 method `supportsInterface` must return a `bool` and use at most `30,000` gas.
    - The two example implementations from the SIP were, at the time of writing 
      1. `586` gas for any input, and 
      2. `236` gas, but increases linearly with a higher number of supported interfaces
  - It is unlikely that any SRC-165 `supportsInterface` implementation will go above `30.000` gas. That would require that the second variant is used, and thirty:ish interfaces are supported.  
...
This is the implementation for the new opcode in go-sila:

```golang

func opSelfBalance(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	stack.push(interpreter.intPool.get().Set(interpreter.savm.StateDB.GetBalance(contract.Address())
	return nil, nil
}
...
- In many cases, a recipient of `sila` from a `CALL` will want to issue a `LOG`. The `LOG` operation costs `375` plus `375` per topic. If the `LOG` also wants to do an `SLOAD`, this change may make some such transfers fail. 
