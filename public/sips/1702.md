...
In Sila, a contract has a deployment method, either by a contract
...
account, and we already have some SIPs that define new additional
...
already useful by itself and can handle most SAVM improvements. Below
...
purpose. When "enabling SIP-1702", those extensions should not be
enabled unless the extension specification is also included.

* [44-VERTXN: Account Versioning Extension for Contract Creation
  Transaction](https://corepaper.org/sila/compatibility/versioning/#extensions-for-state-based-account-versioning)
* [45-VEROP: Account Versioning Extension for CREATE and
  CREATE2](https://corepaper.org/sila/compatibility/versioning/#create-and-create2-extension)

## Usage Template

This section defines how other SIPs might use this account
...
meta. SIPs in the hard fork are grouped by the virtual machine
type, for example, SAVM and eWASM. For each of them, we define:
...
If a meta SIP includes SIPs that provide additional account state RLP
fields, we also define:

* **Account fields**: all account fields up to the end of this meta
  SIP, excluding the basic 5 fields (`nonce`, `balance`,
  `storageRoot`, `codeHash` and `version`). If SIPs included that are
...
support multiple newest VMs (for example, SAVM and WebAssembly running
...
* **[26-VER](https://corepaper.org/sila/compatibility/versioning/#prefix-based-account-versioning)** and
  **[40-UNUSED](https://corepaper.org/sila/compatibility/forward/)**: This makes an
  ...
  incompatible.
* **SIP-1891**: Instead of writing version field into account RLP
  state, we write it in a separate contract. This can accomplish the
  same thing as this SIP and potentially reduces code complexity, but
...
virtual machine. In that case, WASM contracts and SAVM contracts can
...
[43-VER](https://corepaper.org/sila/compatibility/versioning/#state-based-account-versioning).
