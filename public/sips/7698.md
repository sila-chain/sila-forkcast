...
Creation transaction is one of the three ways alongside creation instructions provided by legacy SAVM to deploy new code. Given that legacy creation instructions (`CREATE` and `CREATE2`) are not allowed to deploy EOF code, supporting EOF in creation transactions is the only way to get the first EOF on-chain.
...
- SAVM execution frame created for the initcode - memory, account context etc.
...
| Constant | Value |
| - | - |
| `EOF_MAGIC` | Defined as `0xEF00` in [SIP-3540](./sip-3540.md) |
...
1. Intrinsic gas cost rules and limits defined in [SIP-3860](./sip-3860.md) for creation transactions apply. The entire `data` of the transaction is used for these calculations.
...
6. Execute the container and deduct gas for execution.
    ...
    2. A successful execution ends with initcode executing `RETURNCODE{deploy_container_index}(aux_data_offset, aux_data_size)` instruction. After that:
        ...
        - set `state[new_address].code` to the updated deploy container (rules of [SIP-3541](./sip-3541.md), prohibiting deployment of `code` starting with `EF` from creation transactions, do not apply in this case).
