...
[SIP-8141](./sip-8141.md) frame transactions can verify recent roots through a canonical `VERIFY` frame. The frame carries one to sixteen `(source_id, slot, root)` tuples and calls a recent root contract that checks each tuple against its storage. Root sources write to the same contract. Each root is keyed by `(source_id, slot)`, where `source_id` is derived from the writer address and a salt.
...
Public mempool validation under [SIP-8141](./sip-8141.md) cannot read arbitrary storage controlled by another account or application. Some validation rules still need recent application state, such as privacy tree roots, wallet authorization roots, or account validation roots.
...
This specification extends [SIP-8141](./sip-8141.md). Terms not defined here, including `FrameTx`, `VERIFY`, `expiry_verify`, the validation prefix, `MAX_VERIFY_GAS`, `FRAMEPARAM`, `FRAMEDATALOAD`, and `FRAMEDATACOPY`, have the meanings defined in SIP-8141.
...
Execution clients MUST obtain `current_slot` from the [SIP-7843](./sip-7843.md) `slotNumber` field. Clients MUST NOT derive it from `block.timestamp` using a fixed slot duration.
...
Only execution whose storage context is `RECENT_ROOT_ADDRESS` writes recent root storage. `DELEGATECALL` and `CALLCODE` use the invoking account's storage context, while [SIP-7702](./sip-7702.md) delegated code uses the delegating account's storage context. None modifies recent root storage unless that context is `RECENT_ROOT_ADDRESS`.
...
The frame executes the validation operation through the `STATICCALL` semantics of SIP-8141. If the contract reverts or the frame halts exceptionally, SIP-8141 makes the transaction invalid.

The frame consumes ordinary EVM gas. Its target access and storage reads follow the normal warm and cold access rules, and its data follows the normal SIP-8141 transaction data pricing. This SIP adds no special intrinsic gas, warming rule, or exemption from block gas.

SIP-8141's canonical signature hash already covers the complete frame list, including each frame's data. This SIP does not change the transaction payload or signature hash.

### Application introspection

Applications read a recent root verifier frame through SIP-8141's existing `FRAMEPARAM`, `FRAMEDATALOAD`, and `FRAMEDATACOPY` instructions. This SIP adds no transaction parameter or opcode.
...
SIP-8141's public mempool policy is extended to permit the `current_slot` value defined above and the code and storage reads required by a `recent_root_verify` frame, subject to the rules below.
...
For the purpose of matching SIP-8141's four recognized validation prefix shapes, clients MUST skip both optional leading protocol verifier frames. The SIP-8141 rule that `deploy` is first applies after these frames are skipped.

Clients MUST check this frame shape and data encoding before reading sender state.

A recent root verifier frame is subject to SIP-8141's generic validation trace and opcode rules with exactly two additional permissions. These permissions apply only while this frame executes at the top level and the runtime code at `RECENT_ROOT_ADDRESS` exactly equals `RECENT_ROOT_CODE`:
...
Clients MUST count the recent root verifier frame's `limits.execution` toward SIP-8141's `MAX_VERIFY_GAS` limit.
...
[SIP-8141](./sip-8141.md) public mempool validation normally rejects reads from shared mutable storage. Recent root storage is a narrow exception because a valid entry cannot be overwritten during its usable window. A write in the current slot uses the only ring buffer index that could collide with a root exactly `RECENT_ROOT_LENGTH` slots old, and that older root is already expired.

### Canonical frame instead of an envelope field

The canonical frame reuses SIP-8141's transaction encoding, signature hash, execution semantics, gas accounting, and frame introspection. An envelope field would require a new transaction schema, native checks before frame execution, special gas and warming rules, and a new introspection opcode.

A contract and an informal transaction pool convention are not enough. SIP-8141 has a closed public mempool validation prefix grammar and normally bans both `SLOTNUM` and reads from storage outside `tx.sender`. The protocol must define the frame's shape, position, work bound, allowed storage reads, and revalidation rules.
...
The fixed position affects public mempool eligibility, not block validity. Privately submitted transactions remain subject to ordinary SIP-8141 execution, including its rule that a failed `VERIFY` frame invalidates the transaction.

### Public mempool validation budget

The frame's `limits.execution` counts toward `MAX_VERIFY_GAS`, together with the declared execution limits of the other frames in the validation prefix and the cost of checking the transaction signatures. This uses SIP-8141's existing validation budget. SIP-8141 can change that budget later if needed.
...
This SIP does not change the SIP-8141 transaction payload or signature hash and does not modify other transaction types.
...
Implementations should cover at least the following cases. Rejection from the public mempool does not by itself make a transaction invalid in a block. The contract and SIP-8141 columns state any separate block execution or static validity result.

| Case | Contract or SIP-8141 result | Public mempool result |
| --- | --- | --- |
| One valid tuple from the previous slot | success | accept if all other SIP-8141 rules pass |
| Sixteen valid tuples with distinct cold storage keys | success when `limits.execution` is sufficient | accept if all other SIP-8141 rules pass |
...
| Nonzero `VERIFY` value | statically invalid under SIP-8141 | reject |
| `flags == 1` | contract executes normally | reject; not `recent_root_verify` |
| `flags` contains `ATOMIC_BATCH_FLAG` | statically invalid under SIP-8141 | reject |
...
| Reorganization crosses back before activation | this SIP is inactive | evict |

The case with sixteen tuples must include the cold target account access charged at frame entry by SIP-8141, not only the contract's internal execution.

Each of SIP-8141's four recognized validation prefix shapes must be tested with no protocol verifier, with `recent_root_verify`, with `expiry_verify`, and with `expiry_verify` followed by `recent_root_verify`. Reversed or duplicate protocol verifiers must be rejected from the public mempool.
...
SIP-8141's canonical signature hash covers the frame data, but SIP-8141 also permits validation based on an explicit 32-byte signature message. Validation logic that does not authorize the canonical signature hash must bind the recent root verifier frame and every other relevant frame itself.
...
This SIP does not guarantee inclusion of root writes. Applications that rely on timely publication need their own publication path, redundant root sources, or an inclusion policy for write transactions.
