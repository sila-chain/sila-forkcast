...
An RPC method for switching the wallet's active Sila chain.

## Abstract

The `wallet_switchEthereumChain` RPC method allows Sila applications ("dapps") to request that the wallet switches its active Sila chain, if the wallet has a concept thereof.
...
All dapps require the user to interact with one or more Sila chains in order to function.
...
- `chainId`
  - **MUST** specify the integer ID of the chain as a hexadecimal string, per the [`eth_chainId`](./sip-695.md) Sila RPC method.
...
For related work, see [SIP-3085: `wallet_addEthereumChain`](./sip-3085.md) and [SIP-2015: `wallet_updateEthereumChain`](./sip-2015.md).
`wallet_switchEthereumChain` intentionally forgoes the chain metadata parameters included in those SIPs, since it is purely concerned with switching the active chain, regardless of RPC endpoints or any other metadata associated therewith.
...
- Display a confirmation whenever a `wallet_switchEthereumChain` is received, clearly identifying the requester and the chain that will be switched to.
  - The confirmation used in [SIP-1102](./sip-1102.md) may serve as a point of reference.
