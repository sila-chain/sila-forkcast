...
An alternative discovery mechanism to `window.sila` for [SIP-1193](./sip-1193.md) providers which supports discovering multiple injected Wallet Providers in a web page using Javascript's `window` events.

## Motivation

Currently, Wallet Provider that offer browser extensions must inject their Sila providers ([SIP-1193](./sip-1193.md)) into the same window object `window.sila`; however, this creates conflicts for users that may install more than one browser extension.

Browser extensions are loaded in the web page in an unpredictable and unstable order, resulting in a race condition where the user does not have control over which Wallet Provider is selected to expose the Sila interface under the `window.sila` object. Instead, the last wallet to load usually wins.
...
Some browser extensions attempt to counteract this problem by delaying their injection to overwrite the same `window.sila` object which creates an unfair competition for Wallet Providers and lack of interoperability.

In this proposal, we present a solution that focuses on optimizing the interoperability of multiple Wallet Providers. This solution aims to foster fairer competition by reducing the barriers to entry for new Wallet Providers, along with enhancing the user experience on Sila networks.

This is achieved by introducing a set of window events to provide a two-way communication protocol between Sila libraries and injected scripts provided by browser extensions thus enabling users to select their wallet of choice.
...
Wallet Provider: A user agent that manages keys and facilitates transactions with Sila.
...
- **`uuid`** - a globally unique identifier the Wallet Provider that MUST be ([UUIDv4][RFC-4122] compliant) to uniquely distinguish different [SIP-1193](./sip-1193.md) provider sessions that have matching properties defined below during the lifetime of the page. The cryptographic uniqueness provided by [UUIDv4][RFC-4122] guarantees that two independent `EIP6963ProviderInfo` objects can be separately identified.
...
Regarding the announced provider interface (`EIP6963ProviderDetail`), it was important to leave the [SIP-1193](./sip-1193.md) provider interface untouched for backwards compatibility; this allows conformant DApps to interface with wallets conforming to either, and for Wallets conformant to this spec to still inject [SIP-1193](./sip-1193.md) providers for legacy DApps. Note that a legacy dapp or a DApp conformant with this spec connecting to a legacy wallet cannot guarantee the correct wallet will be selected if multiple are present.

## Backwards Compatibility

This SIP doesn't require supplanting `window.sila`, so it doesn't directly break existing applications that cannot update to this method of Wallet discovery. However, it is RECOMMENDED DApps implement this SIP to ensure discovery of multiple Wallet Providers and SHOULD disable `window.sila` usage except as a fail-over when discovery fails. Similarly, Wallets SHOULD keep compatibility of `window.sila` to ensure backwards compatibility for DApps that have not implemented this SIP. In order to prevent the previous issues of namespace collisions, it's also RECOMMENDED that wallets inject their provider object under a wallet specific namespace then proxy the object into the `window.sila` namespace.
...
### SIP-1193 Security considerations

The security considerations of [SIP-1193](./sip-1193.md) apply to this SIP. Implementers are expected to consider and follow the guidance of the providers they're utilizing as well.
