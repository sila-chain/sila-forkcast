...
Introduce the ability to have several code sections in EOF-formatted ([SIP-3540](./sip-3540.md)) bytecode, each one representing a separate subroutine/function. Two new opcodes,`CALLF` and `RETF`, are introduced to call and return from such a function. Dynamic jump instructions are disallowed.

## Motivation

Currently, in the SAVM everything is a dynamic jump. Languages like Solidity generate most jumps in a static manner (i.e. the destination is pushed to the stack right before, `PUSHn .. JUMP`). Unfortunately however this cannot be used by most SAVM interpreters, because of added requirement of validation/analysis. This also restricts them from making optimisations and potentially reducing the cost of jumps.

[SIP-4200](./sip-4200.md) introduces static jump instructions, which remove the need for *most* dynamic jump use cases, but not everything can be solved with them.

This SIP aims to remove the need and disallow dynamic jumps as it offers the most important feature those are used for: calling into and returning from functions.
...
Additionally, SAVM keeps track of the index of currently executing section - `current_section_index`.
...
2. *Note:* EOF validation [SIP-5450](./sip-5450.md) guarantees that operand stack has enough items to use as callee's inputs.
...
7. Pushes to return stack an item:
   ...
   *Note:* EOF validation [SIP-5450](./sip-5450.md) guarantees there is always an instruction following `CALLF` (since terminating instruction or unconditional jump is required to be final one in the section), therefore `PC_post_instruction` always points to an instruction inside section bounds.
...
2. *Note:* EOF validation [SIP-5450](./sip-5450.md) guarantees that operand stack has exact number of items to use as outputs.
3. Charges `3` gas.
...
*Note:* EOF validation requirement for 0th code section to be non-returning (non-returning sections introduced in a separate SIP) guarantees that return stack cannot be empty before `RETF`.

### Code Validation

In addition to container format validation rules above, we extend code section validation rules (as defined in [SIP-3670](./sip-3670.md)).

1. Code validation rules of [SIP-3670](./sip-3670.md) are applied to every code section.
...
5. No unreachable code sections are allowed, i.e. every code section can be reached from the 0th code section with a series of `CALLF` / `JUMPF` (`JUMPF` introduced in a separate SIP) instructions (0th code section is always reachable).
...
3. Stack underflow check is not performed anymore. *Note:* EOF validation [SIP-5450](./sip-5450.md) guarantees that it cannot happen at run-time.
...
On the other hand, it provides close to zero benefits for the SAVM implementations.
