...
The ability of an average user or a Wallet application to find, collect, review, and analyze the SAVM code the transaction will execute is very limited.
...
For params `0x00`–`0x05`, the accessed slot or address is added to the respective access list after the call. Params `0x06`–`0x0A` do not interact with the [SIP-2929](./sip-2929.md) access lists.
...
Event non-indexed data is variable-length and cannot be returned as a single 32-byte stack word. A memory-copy opcode with the same semantics as `CALLDATACOPY` is the idiomatic SAVM approach for variable-length data access.
