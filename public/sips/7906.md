...
This proposal introduces a new opcode that allows contracts to inspect the transaction outcomes on-chain. This opcode will allow contract developers to define assertions for state changes that can be enforced on-chain. These can protect Sila users by restricting the behavior of the smart contracts they are interacting with.
...
This leaves the users with no mechanism to enforce any restrictions on what the transaction actually does once it is signed. This leads users to perform de-facto blind signing every time they interact with Sila, exposing themselves to significant risks.
...
This SIP requires [SIP-8141](./sip-8141.md) and amends its frame transaction specification by adding a new frame `mode` value, `POST_TX`, alongside the `DEFAULT`, `VERIFY`, and `SENDER` modes already defined there.

The following rules apply to SIP-8141 frame transactions wherever this SIP is active:
...
- A `POST_TX` frame is executed as a `STATICCALL`, disallowing all state manipulation. Unlike `VERIFY` frames, `POST_TX` frames are not granted the `APPROVE` exception defined in [SIP-8141](./sip-8141.md#approve-instruction-0xaa): calling `APPROVE` anywhere in a `POST_TX` frame's call subtree is an ordinary `STATICCALL` violation and causes an exceptional halt of that call frame, handled like any other `POST_TX` failure per the rule below.
...
- Under SIP-8141's [default code](./sip-8141.md#default-code), a `POST_TX` frame is handled the same way as `SENDER` or `DEFAULT`.
...
It accepts a `(param, index)` inputs similar to the `FRAMEPARAM` opcode from [SIP-8141](./sip-8141.md).
The available parameters are listed in the table below.

| `param` | `in2`                         | Return value                                                                  |
|---------|-------------------------------|-------------------------------------------------------------------------------|
| 0x00    | must be 0                     | `balances_changed` - the number of addresses with a net balance change        |
| 0x01    | must be 0                     | `slots_changed` - the number of `(address, slot)` pairs with a net storage change |
| 0x02    | must be 0                     | `contracts_deployed` - the number of addresses with newly deployed contract code |
...
`TXTRACE`, `EVENTDATACOPY`, and `TXDIFF` are valid only for execution inside a `POST_TX` mode frame, as defined above. Executing any of these opcodes in any other context — including legacy transactions, [SIP-1559](./sip-1559.md) transactions, or any other SIP-8141 frame mode — results in an exceptional halt.
...
The `gas_payer_address` is the target of whichever frame called `APPROVE(APPROVE_PAYMENT)` or `APPROVE(APPROVE_EXECUTION_AND_PAYMENT)`, i.e. the SIP-8141 `payer`, which may or may not be the transaction sender.
...
An address appears in `contracts_deployed` when its code hash changed during the transaction from the empty-code hash to a non-empty code hash that is not an [SIP-7702](./sip-7702.md) delegation designator. An account that a `CREATE`/`CREATE2` leaves with empty code is not enumerated, as it has no code change.
...
`TXDIFF` params that may fall back to reading live state use the [SIP-2929](./sip-2929.md) access lists to determine their cost:
...
For params `0x00`–`0x05`, the accessed slot or address is added to the [SIP-2929](./sip-2929.md) access list after the call, and — where [SIP-7928](./sip-7928.md) is active — recorded in the block-level access list, like any other state-reading opcode. `TXTRACE` and `EVENTDATACOPY` read only already-recorded diff and log data and therefore add no new accesses. Params `0x06`–`0x0A` do not interact with the [SIP-2929](./sip-2929.md) access lists.
...
The `TXTRACE` opcode follows the same `(param, index)` two-argument pattern used by `FRAMEPARAM` in [SIP-8141](./sip-8141.md). This keeps the interface consistent and avoids introducing a separate opcode for every piece of trace information.
...
`TXTRACE`'s enumeration model has no way to directly check one specific value, such as "the value of `usdc.balances[vitalik.sil]` before this transaction".
...
The gas pre-charge (`gas_limit × gas_price`) is deducted at transaction start and appears in the gas payer's `balance_after`, making it hard to isolate actual SIL transfers. The pre-charge is also provisional: a refund for unused gas is issued after execution, so the bundled figure is not the final cost.

Exposing `gas_pre_charge` directly lets callers subtract it with a single opcode call. It covers all gas-related deductions including the blob fee for [SIP-4844](./sip-4844.md) transactions, so the same subtraction isolates pure SIL transfers. `gas_payer_address` completes the picture: the [SIP-8141](./sip-8141.md) gas payer may be a separate paymaster rather than the sender, and no existing opcode exposes that address. Together the two parameters let assertion contracts identify the right `balances_changed` entry and apply the subtraction correctly.
...
This proposal has a hard dependency on [SIP-8141](./sip-8141.md) (`requires: 8141`): `TXTRACE`, `EVENTDATACOPY`, and `TXDIFF` can only execute inside an SIP-8141 `POST_TX` frame, as defined under [The `POST_TX` Frame Mode](#the-post_tx-frame-mode). Legacy transactions, [SIP-1559](./sip-1559.md) transactions, and any other SIP-8141 frame mode cannot use any of these opcodes.
...
As stated previously, a transaction can produce up to ~42,600 events in a transaction in the current Sila configuration.
...
The `deploy` frame's restriction to installing `tx.sender`'s code is a mempool-propagation rule ([Structural Rules](./sip-8141.md#structural-rules)), not a protocol invariant — nothing prevents it from producing other side effects, which `TXTRACE` will surface but which `POST_TX` cannot undo. A wallet must never use the `deploy` frame for anything beyond deploying `tx.sender`; they cannot rely on `POST_TX` to catch or reverse it.
