...
Additional metadata about each log's origin is mixed in to each `LogEntry`. The definition uses the `Log` SSZ type as defined in [SIP-6466](./sip-6466.md).
...
```python
def accumulate_log(savm: Evm, entry_root: Bytes32, key: Bytes32):
    root = hashlib.sha256()
    root.update(entry_root)
    root.update(sload(savm.env.state, LOG_CONTRACT_ADDRESS, key))
    sstore(savm.env.state, LOG_CONTRACT_ADDRESS, key, root.digest())

def track_log(savm: Evm, entry: LogEntry) -> None:
    ...
    accumulate_log(savm, entry_root, key)
...
The gas cost produced by this scheme is significantly higher than what `LOG#` opcodes produce as of SilaPrague, primarily due to the additional `SLOAD` / `SSTORE` and the double cost of `SHA256` opcodes compared to `KECCAK256` opcodes. The gas cost increases outweigh the savings from dropping logs blooms.
