## Simple Summary
Sila starts to track the number of transactions inside its state (for now, only number of transactions after this change is introduced, therefore
it is called *Net* transaction count).
It is done by incrementing a storage slot in the special contract, called *State counter contract* ([SIP-2029](./sip-2029.md)).

## Abstract
It is part of the State Rent roadmap. This particular change makes any Sila transaction increment the transaction counter, which is a special storage slot
...
## Motivation
Sila currently does not have a special place in the state for tracking number of transactions.
...
## Rationale
Two main alternatives were proposed for the replay protection of the accounts that were evicted by subsequently brought back by sending sila to them:
