...
Introduce a new [SIP-2718 transaction type](./sip-2718.md) to support validator withdrawals that are "pushed" from the beacon chain to the SAVM.
...
This SIP provides a way for validator withdrawals made on the beacon chain to enter into the SAVM.
...
This approach is more involved than "pull"-based alternatives (e.g. [SIP-4788](./sip-4788.md) + user-space withdrawal contract) with respect to the core protocol (by providing a new transaction type with special semantics) but does provide tighter integration of a critical feature into the protocol itself.
...
Define a new [SIP-2718](./sip-2718.md) transaction type with `TransactionType` `WITHDRAWAL_TX_TYPE`.

The `TransactionPayload` is an RLP-encoded list `RLP([index, address, amount])` where the `index` is a 64-bit value uniquely labeling a specific withdrawal, the `address` refers to an execution layer account and the `amount` refers to an sila value given in units of wei.
...
This push approach gives validators a small subsidy with respect to processing, in lieu of needing to buy gas via normal SAVM processing that would be required for a pull-based approach.
...
This SIP suggests a new transaction type as it has special semantics different from other existing types of SAVM transactions.

An entirely new transaction type firewalls off generic SAVM execution from this type of processing to simplify testing and security review of withdrawals.
...
### Why only balance updates? No general SAVM execution?

More general processing introduces the risk of failures, which complicates accounting on the beacon chain.

This SIP suggests a route for withdrawals that provides most of the benefits for a minimum of the (complexity) cost.
...
Consensus-layer validation of withdrawal transactions is critical to ensure that the proper amount of SIL is withdrawn back into the execution layer.
This consensus-layer to execution-layer SIL transfer does not have a current analog in the SAVM and thus deserves very high security scrutiny.
