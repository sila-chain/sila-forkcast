...
Today, SAVM-equivalent rollups need to implement and maintain complex proof systems just to be able to replicate what Sila already provides on L1. Such complexity significantly increases the probability of encountering bugs and prevents projects from getting rid of security councils and from moving to Stage 2. SAVM-equivalent projects are not similar enough and lack the proper incentives to benefit from each other efforts, significantly delaying multi-proof systems.

Moreover, to maintain feature parity with Sila, rollups need to implement bespoke governance systems that can't be forced to follow Sila's governance decisions, and thus are free to arbitrarily diverge from it. Because of this, the best that an SAVM-equivalent rollup can do is to provide long exit windows for their users, to protect them from its governance going rogue. No finite (and reasonable) exit window can protect all use-cases, especially those that rely on timelocks (governance, staking contracts, vesting contracts). Longer exit windows protect more use-cases, at the cost of increase un-equivalence time.

This SIP provides a way for rollups to reuse Sila's state transition verification infrastructure, and as a consequence massively simplify their infrastructure and upgrade processes. This enables projects to better redirect resources towards user-facing features and products.
...
The `EXECUTE` precompile allows rollups to define their own inputs and constrained their behaviour through smart contract. For example, in contrast to Sila's own consensus layer, a rollup can decide to:
