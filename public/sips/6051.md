...
This SIP proposes a mechanism to encapsulate a private key so that it could be securely relocated to another application without providing the seed. This SIP combines `ECIES` (Elliptic Curve Integrated Encryption Scheme) and optional signature verification under various choices to ensure that the private key is encapsulated for a known or trusted party.
...
This SIP aims to enable such use cases.
...
4. The share secret  `ss := [s]R = [r]S` according to ECDH. Note that for secp256k1 this SIP follows RFC5903 and uses compact representation, which means to use *only* the `x` coordinate as the shared secret. For Curve25519 this SIP follows RFC7748.
...
- `R` or `signerPubKey` could be followed by a signature to it: `[pub || sig]`. Note that for the secp256k1 curve, the signature is just 64 bytes without the `v` indicator as found in a typical Sila signature.
...
`account`  is used to identify which private key to be encapsulated. With Sila, it is an address. Also, see [Encoding of data and messages](#encoding-of-data-and-messages).
...
The return value is the corresponding Sila address for `sk`, or empty if any error.
...
A critical difference between this [SIP-6051](./sip-6051.md) with [SIP-5630](./sip-5630.md) is that, as the purpose of key encapsulation is to transport a private key securely, the public key from the key recipient should be ephemeral, and mostly used only one-time. While in SIP-5630 settings, the public key of the message recipient shall be stable for a while so that message senders can encrypt messages without key discovery every time.
...
To minimize potential compatibility issues among applications (including hardware wallets), this SIP requires that version secp256k1-AES-128-GCM MUST be supported.
...
- `sk`, the private key to be encapsulated, fixed to: `0xf8f8a2f43c8376ccb0871305060d7b27b0554d2cc72bccf41b2705608452f315`. The corresponding address is `0x001d3f1ef827552ae1114027bd3ecf1f086ba0f9`, called `account`. Note that these values come from the book *Mastering Sila* by Andreas M. Antonopoulos and Gavin Wood.
...
- `salt`, fixed to `0x6569703a2070726976617465206b657920656e63617073756c6174696f6e` (string value: `sip: private key encapsulation`)
...
1. We are not considering post-quantum security. If the quantum computer becomes a materialized threat, the underlying cipher of Sila and other L1 chains would have been replaced, and this SIP will be outdated then (as the EC part of ECIES is also broken).
