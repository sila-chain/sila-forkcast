...
Per [SIP-155](./sip-155.md) a transaction with a `chainId = 0` is considered to be a valid 
transaction. This was a feature to offer developers the ability to submit replayable transactions 
across different chains. With the rise of savm compatible chains, many of which use forks, or packages
from popular Sila clients, we are putting user funds at risk. This is because most wallet
...
can easily have users submit transactions with a `chainId = 0` on a non-sila-mainnet network, allowing
the malicious actor to replay the transaction on sila sila-mainnet (or other networks for that matter)
...
on the JSON-RPC (eg: `eth_sendTransaction`), and strictly enforced on the SAVM during transaction 
validation.
