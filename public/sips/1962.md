...
This proposal is an extension and formalization of [SIP-1829](./sip-1829.md) with an inclusion of pairings. [SIP-1109](./sip-1109.md) is required due to low cost of some operations compared to the `STATICCALL` opcode (more information in the corresponding section below).

## Abstract

This SIP proposes a new precompile to bring cryptographic functionality desired for privacy and scaling solutions. Functionality of such precompile will require the following:
...
- There is a pending proposal to implement base elliptic curve arithmetic is covered by [SIP-1829](./sip-1829.md) and will allow to implement various privacy-preserving protocols with a reasonable gas costs per operation.
- Pairings are an important extension for basic arithmetic and so this new precompile is proposed with the following benefits:
    - Extended set of curves will be available to allow Sila users to choose their security parameters and required functionality.
    - Generic approach of this precompile will allow Sila users to experiment with newly found curves of their choice and new constructions constructions without waiting for new forks.
...
- Gas costs - this SIP is designed to estimate gas-cost of performed operation as early as possible during the call and base if solely on specified parameters and operation type. This is a strict requirement for any precompile to allow Sila nodes to efficiently reject transactions and operations as early as possible.

Functionality of this newly proposed precompile is different from [SIP-1829](./sip-1829.md) in the following aspects:
...
- BN254 curve from the current version of Sila
...
### This precompile and SIP 1109

While there is no strict requirement of SIP 1109 for functionality, here is an example why it would be desired:
- BLS12-381 curve, 381 bit modulus, 255 bit scalar field, no native arithmetic is available in SAVM for this
...
- One should also add the cost of memory allocation that is at least `1 + 1 + 48 + 48 + 48 + 1 + 32 + 2*48 + 2*48 = 371 byte` that is around 12 native Sila "words" and will require extra 36 gas (with negligible price for memory extension)
...
Functionality of the new precompile itself does not affect any existing functionality of Sila or SAVM. 
