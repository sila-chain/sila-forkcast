...
* “Shutter Network Introduces Plan for First Encrypted Mempool on Sila,” GlobeNewswire, 13 Feb 2025.
...
We define our randomness `R` as the `randao_reveal` of the each slot, after [SIP-7998](./sip-7998.md), since this SIP provides slot-by-slot randomness for `randao_reveal` (only predictable for the builder). This provides enough randomness for our purpose of reducing MEV attacks by MEV bots.
...
* **Hashing / sort key** – Treat the bundle as a *virtual transaction* with key `H(concat(child_tx_rlps))`, where:
   ...
   * `child_tx_rlps[i]` **MUST** be the exact bytes that will later appear in the block body for that transaction, i.e. the *canonical RLP of the fully‑signed transaction* per [SIP‑2718](./sip-2718) / [SIP‑155](./sip-155) rules (for typed transactions the leading *type byte* and length prefix are included).
...
for item in candidates:
    if item.type == "single_tx":
        ...
        baseHash      ← HASH(item.RLP)          // canonical signed RLP (SIP-2718)
...
1. Consensus‑layer upgrade —  Beacon‑chain fork at `ORDERING_FORK_EPOCH` activates **`EL‑VRF‑exposure`** (companion SIP) and begins populating the `vrf_output_proposer` field.  Execution clients receiving the `ExecutePayload` after this epoch expect a non‑zero `randomness` field.
...
* *Single‑validator bias* – A block proposer can not change its `randao_reveal` output after SIP-7998, only predictable to him.
...
* *Builder discretion* – Builders may censor or selectively include transactions while forming the candidate set; exactly like the current status of Sila.
