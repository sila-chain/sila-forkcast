...
- Is **compatible with FOCIL** ([SIP-7805](./sip-7805.md)), ensuring that the aggregation scheme works with fork-choice enforced inclusion lists.
...
- `verification_key_hash`: 32 bytes. For `LEANSPHINCS_SCHEME`, this is the public key hash (`pubkey_hash`). For `LEANSTARK_SCHEME`, this is a hash of the STARK verification key (`vk_hash`).
...
A frame transaction may contain frames with mode `DEP_VERIFY_FRAME_MODE`. These frames declare **dependencies** that must be satisfied by the block's recursive STARK.
...
During EVM execution, the transaction's dependencies are made available through the existing [SIP-8141](./sip-8141.md) `FRAMEPARAM`, `FRAMEDATASIZE`, and `FRAMEDATACOPY` instructions.
...
The data format for each dependency frame is a concatenation of 96-byte triples: `scheme || data_hash || verification_key_hash`.

This allows smart contracts to introspect the declared dependencies during execution using the existing SIP-8141 frame introspection mechanism.
...
The recursive STARK is generated using Lean Sila tooling. It proves the following statement:
...
- All leanSPHINCS signatures using the Lean Sila leanSPHINCS verifier
- All leanSTARKs using the Lean Sila leanSTARK verifier
- Recursive STARKs using the Lean Sila leanSTARK verifier.
...
3. All transactions satisfy the normal (SIP-8141 or other) validity rules.
...
transactions = list of SIP-8141 frame transactions (or their hashes if already broadcast)
...
- `deps` = sorted, deduplicated union of all `(scheme, data_hash, verification_key_hash)` triples from all transactions in the wrapper
...
1. The `deps` list is the sorted, deduplicated union of all dependencies from each transaction in `transactions`
...
This SIP introduces **recursive aggregation** at the mempool layer. This allows mempool nodes to combine proofs and reduce bandwidth overhead to builders.
...
3. Other nodes' aggregates, in descending order of how many new transactions they will add to the aggregate
...
To prevent DoS attacks, two mempool-level limits apply to recursive STARK aggregation: `MAX_LEANSIG_DEPS_PER_WRAPPER` and `MAX_LEANSTARK_DEPS_PER_WRAPPER`, defined in [Constants](#constants).
...
These limits in all cases apply to the length of the sorted, deduplicated dependency list. The list in a wrapper is required to be deduplicated in any case. For transactions, however, it does mean that it is legal to have a transaction with eg. `> MAX_SIGS_PER_TX` leanSPHINCS dependency _declarations_, if some of them are pointing to the same object so the deduplicated list is within limits. This behavior is wasteful but harmless; it is charged for and limited through the gas mechanism.

### FOCIL Compatibility

This SIP is designed to be compatible with [SIP-7805 (FOCIL)](./sip-7805.md), which implements fork-choice enforced inclusion lists. The compatibility considerations are as follows.
...
The recursive STARK scheme is compatible with the consensus layer's use of Lean Sila tooling. The same STARK backend and verification infrastructure can be used for both consensus layer signature aggregation and execution layer dependency verification. Inclusion lists (ILs) are just a different kind of package of transactions (like blocks) and use the same signature schemes as other components; no new signature scheme is required specifically for ILs.

### Lean Sila Integration

This SIP assumes the use of Lean Sila tooling for generating and verifying the recursive STARKs. This tooling can be found at the leanEthereum GitHub organization.

The recursive STARK is generated by:

1. Defining a Lean function that takes a flat list of `(scheme, data_hash, verification_key_hash)` triples and their corresponding witnesses.
2. Iterating over the list, and for each triple with `scheme == LEANSPHINCS_SCHEME`, calling `leanSPHINCS.verify(data_hash, verification_key_hash, witness)`.
3. For each triple with `scheme == LEANSTARK_SCHEME`, calling `leanSTARK.verify(data_hash, verification_key_hash, witness)`.
4. Proving that all verifications return `true`.
5. Generating a STARK proof of this execution using the Lean Sila STARK backend.
...
The verification key `AGGREGATED_VK` used to verify the recursive STARK is a fixed protocol-level value derived from the Lean Sila STARK circuit definition.
...
The Lean Sila tooling has already been extensively developed, has full formal verification in progress, and is the expected signature and recursive proof type for the future Sila consensus chain.

**This SIP's use case — processing many thousands of signatures and STARKs per slot without extreme data overhead — is almost identical to the consensus layer's use case.** The main differences are:

1. Hashes vs bitfields as identifiers of what objects are being proven
2. Many-time stateless signatures (leanSPHINCS), instead of the counter-based stateful signatures (leanXMSS) that the consensus layer can use because it needs to enforce one-signature-per-slot anyway.
...
As written, the SIP enforces fixed gas costs for both signatures and STARKs. This is assuming one parametrization of leanSPHINCS is used, and one parametrization of STARKs (or at least a small ratio between the average case and the upper bound).
...
This SIP introduces new transaction types and frame modes. It does not modify the behavior of existing transactions. Transactions without signature or STARK verification frames are unaffected.

Nodes that do not implement this SIP will:
...
The security of this SIP relies on:

1. **Soundness of Lean Sila circuits**: The STARK circuits must correctly implement the verification logic.
2. **Soundness of the hash function used for `deps_hash` and `block_deps_hash`**: the current leading choice is BLAKE3.
3. **Soundness of recursive STARKs**: a STARK of a STARK of ... of a STARK must be sound. This is widely believed to be true and many constructions running live onchain already depend on this. However, this adds difficulty to end-to-end formally proving the scheme and its applications. Note also that the STARK verification keys that users put in to this system are up to users, and may potentially have unlimited recursion depth.
...
- `MAX_FRAMES` (from SIP-8141) limits the number of frames per transaction.
...
These limits are inspired by the mempool policy framework of [SRC-7562](./sip-7562.md) (as used in SIP-8141).
