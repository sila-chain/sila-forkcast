...
This SIP proposes adding a 'temporal' replay protection to transactions, in the form of a `valid-until` timestamp. 
This SIP is very similar to https://github.com/sila-chain/SIPs/pull/599 by Nick Johnson and Konrad Feldmeier, the main difference
being that this SIP is based on clock-time / walltime instead of block numbers. 
...
- If any form of dust-account clearing is introduced, e.g. (https://github.com/sila-chain/SIPs/issues/168), it will be necessary
to introduce a replay protection, such as https://github.com/sila-chain/SIPs/issues/169 . Having temporal replay protection removes the need
...
Why use walltime instead of block numbers, as proposed in https://github.com/sila-chain/SIPs/pull/599 ? 
...
This SIP means that all software/hardware that creates transactions need to add timestamps to the transactions, or will otherwise be incapable of signing transactions after block `Y`. Note: this SIP does not introduce any maximum `valid-until` date, so it would still be possible to create
...
The most notable security impact is that pre-signed transactions stored on paper backups, will become invalid as of block `Y`. There are a couple of cases where this might be used
   - Pregenerated onetime 'bootstrap' transactions, e.g. to onboard a user into Sila. Instead of giving a user a giftcard with actual sila on it, someone may instead give the person a one-time pregenerated transaction that will only send those sila to the card once the 
user actively wants to start using it.
   - If a user has an offline paper-wallet, he may have pregenerated transactions to send value to e.g. an exchange. This is sometimes done to be able to send sila to an exchange without having to go through all the hoops of bringing the paper wallet back to 'life'. 
