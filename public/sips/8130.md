...
Note that the accounts are portable to any SAVM and the system is able to work on all chains. If a chain does not support 8130, an alternative AA mechanism such as SRC-4337 can be used with the same account, provided the account code allows support. 
...
| **Existing Smart Contracts** | Already-deployed accounts (e.g., SRC-4337 wallets) register actors via `importAccount()` on the Account Configuration Contract | Wallet-defined |
...
Chains choose how authenticator execution is priced. A chain MAY meter authenticator execution as ordinary SAVM execution (see [Mempool Acceptance](#mempool-acceptance) for rules), or it MAY enshrine canonical authenticators and charge a fixed, standard gas cost per enshrined authenticator instead of metering. When an authenticator is enshrined, its execution MUST produce identical results to the corresponding authenticator contract.
...
Any contract implementing `IAuthenticator` can be permissionlessly deployed and registered as an actor's authenticator. However, registration does not make an authenticator usable on the 8130 path: only canonical authenticators in the node allowlist are accepted for block-level AA authentication (see [Canonical Authenticator Set](#canonical-authenticator-set)). Non-canonical authenticators remain fully usable within SAVM execution; for example, an account can authenticate an actor against an arbitrary `IAuthenticator` via a config change call, enabling use cases such as wallet-defined recovery methods. Such actors simply cannot authenticate transactions directly over the 8130 path and they operate through ordinary SAVM execution.
...
The canonical authenticator set and corresponding contract addresses are maintained in a companion SRC (number TBD) and deployed at deterministic CREATE2 addresses across chains. The canonical set is expected to grow as new algorithms are adopted (e.g., post-quantum) through the companion SRC process.
...
Actors are identified by their `actorId`, a 32-byte identifier derived by the authenticator from public key material. Each authenticator defines its own actorId derivation algorithm (see [Canonical Authenticator Set](#canonical-authenticator-set)). Actors can be modified via calls within SAVM execution by calling the authenticated config change functions.
...
|-----|-------|------|---------|
| 0 | `0x01` | SIGNATURE | SRC-1271 via `verifySignature()` |
...
Nonce state is managed by a precompile at `NONCE_MANAGER_ADDRESS`. The protocol reads and increments nonce slots directly during AA transaction processing; the precompile exposes a read-only `getNonce()` interface to the SAVM.
...
Nonce-free deduplication and replay protection MUST key on a **signature-invariant** identifier rather than the [SIP-2718](./sip-2718.md) transaction hash:
...
| `default_eoa_policy_type` | Inline self-actor `policyType` (`uint8`; `0x00` = none) |
...
When `locked` is set, all config changes are rejected: both config change entries in `account_changes` and `applySignedActorChanges()` via SAVM. The lock cannot be removed without a timelock delay.
...
The `signature` is validated against the account via [SRC-1271](./sip-1271.md) `isValidSignature(digest, signature)`, binding the initial actor set to the account's existing authorization logic. `digest` is a typed `ActorInitialization` struct hash:
...
This digest is a typed (SIP-712-style) struct hash that intentionally omits the SIP-712 domain separator, which prevents phishing via standard wallet signing flows. The `salt` field is bound to the account address. This typed style is distinct from the packed style used for [Address Derivation](#address-derivation); that split is intentional.
...
`DEFAULT_ACCOUNT_ADDRESS` SHOULD implement [SRC-1271](./sip-1271.md) by delegating to the Account Configuration Contract's `verifySignature()`, and SHOULD implement token receiver hooks (SRC-721, SRC-1155) to safely receive assets.
...
The specific per-component gas *values* in this section are a **recommended schedule** that reflects the SAVM access and data-availability costs ([SIP-2929](./sip-2929.md) / [SIP-2028](./sip-2028.md)) at the time of writing. They are a reference, not protocol constants: just as a chain may choose how it prices authenticator execution (see [Authenticators](#authenticators)), a chain MAY adopt a different intrinsic-gas schedule, for example to track a future SAVM repricing or a local cost model. The formula's structure (its components and what each one accounts for) is the normative part; the absolute numbers below are the recommended values for a chain that mirrors current SAVM costs.
...
| Component | Value |
|-----------|-------|
| `tx_payload_cost` | Standard per-byte cost over the entire RLP-serialized transaction: 16 gas per non-zero byte, 4 gas per zero byte, consistent with [SIP-2028](./sip-2028.md). Ensures all transaction fields (`account_changes`, `sender_auth`, `calls`, `metadata`, etc.) are charged for data availability |
...
3. Reject if `code` is empty or `len(code) > MAX_CODE_SIZE` ([SIP-170](./sip-170.md): 24576 bytes), to keep the placed code within the SAVM contract size limit that CREATE/CREATE2 would otherwise enforce
...
|-----------|-------------|-----------------|
| **Account Configuration Contract** | Protocol reads storage directly for validation; SAVM interface available | Standard contract (SRC-4337 compatible factory) |
...
7. Verify nonce, payer SIL balance, and expiry:
...
2. SIL gas deduction from payer (`sender` for self-pay). Transaction is invalid if payer has insufficient balance.
...
The `DEPLOYMENT_HEADER(n)` is a 14-byte SAVM loader that copies trailing code into memory and returns it. The header encodes code length `n` into its `PUSH2` instructions:
...
interface IAccountConfiguration {
    ...
    // Import existing account (SRC-1271 verification for initial actor registration)
    ...
    // verifySignature: SRC-1271-style boolean check; returns false on any failure.
    ...
    // Resolves the policy sub-type, gate target, and signed commitment:
    //   0x00 -> (0x00, address(0), bytes32(0)); non-zero -> (policyType, manager, commitment)
    function getPolicy(address account, bytes32 actorId)
        external view returns (uint8 policyType, address target, bytes32 commitment);
...
**Account Creation Security**: `initial_actors` (the `actorId` and `authenticator`; scope, expiry, and policy fields are zero for the unrestricted owner keys created here, per [Address Derivation](#address-derivation)) are salt-committed, preventing front-running of actor assignment. Wallet bytecode should be inert when uninitialized as it can be permissionlessly deployed. The create entry applies only to addresses that satisfy CREATE2 freshness. Without the nonce check, a create entry could be replayed against an EOA that has transaction history at the counterfactual address. Direct code placement also bypasses CREATE/CREATE2's [SIP-170](./sip-170.md) `MAX_CODE_SIZE` check, so the protocol enforces `len(code) <= MAX_CODE_SIZE` explicitly to keep the placed code within the SAVM contract size limit.
