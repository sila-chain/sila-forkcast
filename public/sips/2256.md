...
This is a proposal for a new JSON-RPC call for retrieving from a wallet a selection of owned assets by an Sila address, with the user's permission.
...
**Arguments:**
- type `address`, Sila address that owns the assets
- options object, optional:
  - `chainId` - type `uint`, chain id respecting [SIP-155](./sip-155.md); optional
...
- array with asset records:
  - `address` - type `address`, Sila checksummed address
  - `chainId` - type `uint`, identifier for the chain on which the assets are deployed
  - `type` - type `string`, asset interface SRC identifier; e.g. `ERC20`; optional - [SIP-1820](./sip-1820.md) could be used
