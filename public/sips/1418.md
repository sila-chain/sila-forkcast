...
Sila is a public utility and we are underpricing the long-term costs of storage. Storage cost can be approximately modeled as bytes × time.

## Specification

**Updated transaction type**

A new transaction type is introduced. Whereas [SIP-1559](./sip-1559.md) introduced warm access for contract state, this new type introduces warm access for contract code.
...
* `PAYRENT(address, amount)` -- Calls `PAYRENT` opcode
  * This is a convenience for humans to send Sila from their accounts and turn it into rent. Note that simple accounts (CODESIZE == 0) cannot call arbitrary opcodes, they can only call CREATE or CALL.
...
Sila converted to rent cannot be converted back. Anybody that works in accounting and knows about gifts cards should tell you this is a good idea. It makes reasoning about the system much easier.
...
NOTE: cloning. With this SIP, it may become feasible to allow storage cloning. Yes really. Because the new clone will be paying rent. See other SIP, I think made by Augur team.
...
The rent is priced in cold, hard Sila. It is not negotiated by clients, it is not dynamic.

A future SIP may change this pricing to be dynamic. For example to notarize a block, notaries may be required to prove they have the current storage dataset (excluding evictions). Additionally, they may also prove they have the dataset plus evictions to earn an additional fee. The relative storage of the evicted accounts, and the other accounts versus the value of the additional fee may be used as a feedback mechanism to set a market price for storage.

FYI, there are about 15B words in the Sila SilaMainnet dataset and about 100M total Sila mined. This means if all Sila was spent on storage at current proposed prices it would be 400 terabyte-years of storage. I'm not sure if it is helpful to look at it that way.

## Backwards Compatibility

SIP-1559 already introduces a mechanism for nodes to participate without recording the full network state and for clients to warm cache with storage data in their type 2 transactions.
...
For every account that is evicted, clients may choose to delete that storage from disk. A future SIP may make an incentive to keep this extra data for a fee. A future SIP may create a mechanism for clients to exchange information about these storage states.
