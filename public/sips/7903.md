...
This SIP proposes the removal of the `initcode` size limit of 49152 bytes introduced in [SIP-3860](./sip-3860). The restriction complicates deploying multiple contracts addressing the [SIP-170](./sip-170) limit (24576 bytes) within a single transaction, while the existing gas metering for `initcode`, already ensures fair `initcode` costing, including for `JUMPDEST` analysis.

## Motivation

The SIP-3860 `initcode` size limit imposes an unnecessary constraint on deployment patterns, particularly for creation transactions creating large _logical_ contracts composed of multiple _physical sub-contracts_ in a single transaction.
A key argument for retaining SIP-170's 24KB runtime code limit is that high-level languages (HLLs) should abstract it away.
However, the SIP-3860 limit prevents HLLs from cleanly abstracting this, as deploying large logical contracts exceeding 49152 bytes of `initcode` requires splitting into multiple transactions, undermining the abstraction.
...
Revert the `initcode` size limit introduced in SIP-3860. Specifically:

- Remove the 49152-byte cap on `initcode` size during contract creation.
- Retain existing gas costs for `initcode` execution, including the 2 gas per byte for `JUMPDEST` analysis, as defined in SIP-3860.

No changes to deployed contract size limits (SIP-170) or gas schedules beyond removing the size restriction are proposed.
