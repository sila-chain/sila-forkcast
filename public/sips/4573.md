...
Five SAVM instructions are introduced to define, call, and return from named SAVM _procedures_ and access their _call frames_ in memory - `ENTERPROC`, `LEAVEPROC`, `CALLPROC`, `RETURNPROC`, and `FRAMEADDRESS`.

## Motivation

Currently, Sila bytecode has no syntactic structure, and _subroutines_ have no defined interfaces.
...
Also, the SAVM currently has no automatic management of memory for _procedures_.  So we also propose to automatically reserve call frames on an in-memory stack.

Constraints on the use of _procedures_ must be validated at contract initialization time to maintain the safety properties of [SIP-3779](./sip-3779.md): Valid programs will not halt with an exception unless they run out of gas or recursively overflow stack.

### Prior Art

The terminology is not well-defined, but we will follow Intel in calling the low-level concept _subroutines_ and the higher level concept _procedures_.  The distinction is that _subroutines_ are little more than a jump that knows where it came from, whereas procedures have a defined interface and manage memory as a stack. [SIP-2315](./sip-2315.md) introduces _subroutines_, and this SIP introduces _procedures_.
...
Equivalently, in the SAVM's twos-complement arithmetic, `FP` moves from the highest address down, as is common in many calling conventions.
...
There is actually not much new here.  It amounts to [SIP-615](./sip-615.md), refined and refactored into bite-sized pieces, along lines common to other machines.

This proposal uses the [SIP-2315](./sip-2315.md) return stack to manage calls and returns, and steals ideas from [SIP-615](./sip-615.md), [SIP-3336](./sip-3336.md), and  [SIP-4200](./sip-4200.md). `ENTERPROC` corresponds to `BEGINSUB` from SIP-615.  Like SIP-615 it uses a frame stack to track call-frame addresses with `FP` as _procedures_ are entered and left, but like SIP-3336 and SIP-3337 it moves call frames from the data stack to memory. 
...
* `JUMPSUB` and `RETURNSUB` (from [SIP-2315](./sip-2315.md) -- like `CALL` and `RET` -- jump to and return from _subroutines_.
...
This proposal adds new SAVM opcodes.  It doesn't remove or change the semantics of any existing opcodes, so there should be no backwards compatibility issues.

## Security

Safe use of these constructs must be checked completely at validation time -- per SIP-3779 -- so there should be no security issues at runtime.

`ENTERPROC` and `LEAVEPROC` must follow the same safety rules as for `JUMPSUB` and `RETURNSUB` in SIP-2315. In addition, the following constraints must be validated:
