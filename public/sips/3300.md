...
## Abstract
This SIP would define a block when the `SSTORE` and `SELFDESTRUCT` refunds would begin to diminish.
...
## Specification
Parameters:
* `FORK_BLOCK_NUM`: SIP-3300 activation block
...
On the block this SIP activates, and again every `REFUND_DECAY_FREQUENCY` blocks, all gas refunds, including `SELFDESTRUCT` and `SSTORE` would diminish by `REFUND_DECAY_STEP`, until 0.
...
### SIP-2929
The refunds as of SIP-2929 are as follows:
