...
The aim of this SIP is to provide some level of regularity and predictability to
the Sila network upgrade/hard fork process. This will allow service
...
deployment are out of scope of this SIP.
...
nine, or more month cadence is out of scope of this SIP.
...
This SIP provides neither guidance nor restrictions to the development and
...
of a particular network in the Sila Ecosystem. Networks that serve as a
...
To date all SilaMainnet activation blocks have ended in three or more zeros,
including Critical Network Upgrades. Ropsten and SilaKovan initially started with
...
To continue this pattern network upgrade activation block numbers for sila-mainnet
...
The rationale for defining launch windows is to give business running Sila
...
Dragon were critical upgrades in response to the SilaShanghai Spam Attacks.
Constantinople Fix (as it is termed in the reference tests) was in response to
the SIP-1283 security issues.
...
testnet consensus failures. The SIP-1283 issues likely would have resulted in an
out of window upgrade because of the impact of the difficulty bomb.

<!-- ## Test Cases -->
<!-- no test cases are relevant for this SIP -->

## Implementation

The windows in this SIP are expected to start after the Istanbul Network
upgrade, which is the next planned Roadmap upgrade. Istanbul is currently slated
for sila-mainnet launch on 2019-10-16, which is compatible with the schedule in this
SIP.
