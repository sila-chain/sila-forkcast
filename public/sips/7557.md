...
[SIP-2929: Gas cost increases for state access opcodes](./sip-2929) introduced a new gas cost model that differentiates
...
[SIP-2930: Optional access lists](./sip-2930) made it possible for transactions to pre-specify and pre-pay for the
...
With the [SIP-6800: Sila state using a unified verkle tree](./sip-6800) on the roadmap for inclusion,
the cost of reading from the Sila state and especially the contract code is expected to increase.
...
The validators, however, only have to perform the actual reading from the Sila state once per block,
and all subsequent reads of the values that were already referenced are significantly more efficient.

If witnesses are introduced to Sila blocks, the same witness can be reused by multiple transactions.
Requiring each transaction to pay regardless of the contents of the block is unfair and inefficient.

Another change that may be on the roadmap for Sila is Account Abstraction.
...
A major distinction between this proposal and alternatives, such as [SIP-7863](./sip-7863), lies in performing a fair
distribution of gas savings between all participants - all transactions senders and the block builder.

## Specification

The [SIP-2930: Optional access lists](./sip-2930) already introduced the first part of the solution.
...
and all rules from SIP-2929 and SIP-2930 continue to apply.
...
2. The rest of the Sila that was charged by the validator as a  `priorityFee` is redistributed back to all the senders
...
The contents of the `accessList` parameter are part of the Sila history and the potential cost of keeping this
...
export function calculatePriorityFeeReimbursements (sortedAccesses: AccessDetails[], accessGasCost: string) {
  ...
  // Calculate the total amount of sila to be reimbursed for this access
...
are necessary in light of [SIP-1559: Fee market change for SIL 1.0 chain](./sip-1559) in order to differentiate
between the Sila reimbursement that is originating from a reduced block gas burn,
and from the reduced block proposer priority fee per gas reward.

### Future SIP-6800 gas reform support

Once [SIP-6800](./sip-6800) is active, the cost of accessing a contract code for a cold address is expected to change.
...
For a maximum contract size of `24576 bytes` defined by [SIP-170](./sip-170) the cost of accessing this contract
...
The [SIP-4895: Beacon chain push withdrawals as operations](sip-4895) sets a precedent by introducing a concept of a `system-level withdrawal operation`.
...
There exists a list of SAVM instructions that trigger both a gas charge and a gas refund.
A notable example of such operations is the `0x55 SSTORE` opcode as defined in [SIP-1283: Net gas metering for SSTORE without dirty maps](./sip-1283).
