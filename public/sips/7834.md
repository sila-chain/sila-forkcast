...
Introduce a new separate metadata section to the Sila Object Format (EOF) that is unreachable by the code, and any changes to which does not affect the code.
...
Extending the format introduced in [SIP-3540](./sip-3540.md), this SIP proposes to add a new OPTIONAL section in the body called `metadata_section` before the `data_section`, and to add two new OPTIONAL fields `kind_metadata` (value: `0x05`) and `metadata_size` to the header before the `kind_data` and `data_size` fields.
...
The structure and the encoding of the `metadata_section` is not defined by this SIP. It is left to the compilers, tooling, or the contract developers to define the encoding and the content. The current practice by the Solidity and Vyper compilers is to use CBOR encoding.
...
No backward compatibility issues are expected since [SIP-3540](./sip-3540.md) is not implemented yet.
