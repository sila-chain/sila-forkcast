...
| - | - | - |
| `HISTORY_PRUNE_EPOCHS` | 33024 | The consensus-layer block retention window in epochs |
...
This SIP impacts the way clients bootstrap and sync. Clients will not be able to full sync using devp2p since historical data will no longer be served.
...
This proposal sets `HISTORY_PRUNE_EPOCHS` to 33,024 epochs, matching the consensus layer's block retention window. Some consensus clients rely on their paired execution client to store execution payloads. This ensures those clients can serve complete beacon blocks throughout the period in which the consensus-layer p2p network requires them to be available.

The consensus-layer retention window is derived from the maximum weak subjectivity period. It is therefore long enough to support checkpoint sync while bounding execution clients' disk usage.

## Backwards Compatibility

### Preserving historical data

This proposal impacts nodes that make use of historical data (e.g. web3 applications that display history of blocks, transactions or accounts). Preserving the history of Sila is fundamental and we believe there are various out-of-band ways to achieve this.
...
We suggest that a specialized "full sync" client is built. The client is a shim that pieces together different releases of execution engines and can import historical blocks to validate the entire Sila chain from genesis and generate all other historical data.
...
If the consensus layer changes its block retention window, `HISTORY_PRUNE_EPOCHS` must be updated to match. Otherwise, clients with a valid weak subjectivity checkpoint may be unable to "checkpoint sync" because the execution-layer p2p network might not provide all required execution payloads.
...
It's important that Sila historical data are preserved and seeded by independent organizations, and their availability should be checked frequently. We consider these mechanisms as out-of-scope for this proposal.
...
Because there are a number of alternative proposals for reducing the execution client's footprint on disk, we've decided to enforce a specific pronunciation of the SIP. When pronouncing the SIP number, it **MUST** be pronounced SIP "four fours". All other pronunciation are incorrect.
