...
This proposal defines a standard set of remote procedure call methods that an Sila node should implement.

## Abstract

Nodes created by the current generation of Sila clients expose inconsistent and incompatible remote procedure call (RPC) methods because no formal Sila RPC specification exists. This proposal standardizes such a specification to provide developers with a predictable Sila RPC interface regardless of underlying node implementation.
...
Communication with Sila nodes is accomplished using [JSON-RPC](https://www.jsonrpc.org/specification), a stateless, lightweight [remote procedure call](https://en.wikipedia.org/wiki/Remote_procedure_call) protocol that uses [JSON](http://www.json.org/) as its data format. Sila RPC methods **MUST** be called using [JSON-RPC request objects](https://www.jsonrpc.org/specification#request_object) and **MUST** respond with [JSON-RPC response objects](https://www.jsonrpc.org/specification#response_object).

#### Error codes

If an Sila RPC method encounters an error, the `error` member included on the response object **MUST** be an object containing a `code` member and descriptive `message` member. The following list contains all possible error codes and associated messages:
...
Specific types of values passed to and returned from Sila RPC methods require special encoding:
...
Since there is no way to clearly distinguish between a `Data` parameter and a `Quantity` parameter, [SIP-1898](./sip-1898.md) provides a format to specify a block either using the block hash or block number. The block identifier is a JSON `object` with the following fields:
...
New Sila RPC methods and changes to existing methods **MUST** be proposed via the traditional SIP process. This allows for community consensus around new method implementations and proposed method modifications. RPC method proposals **MUST** reach "draft" status before being added to this proposal and the official Sila RPC specification defined herein.
...
- `"3"` - Ropsten testnet
- `"4"` - Rinkeby testnet
- `"42"` - SilaKovan testnet

**Note:** See SIP-155 for a [complete list](./sip-155.md#list-of-chain-ids) of possible chain IDs.
...
**Note:** The resulting gas estimation may be significantly more than the amount of gas actually used by the transaction. This is due to a variety of reasons including SAVM mechanics and node performance.
...
Returns the current Sila protocol version
...
{`string`} - current Sila protocol version
...
Calculates an Sila-specific signature in the form of `keccak256("\x19Ethereum Signed Message:\n" + len(message) + message))`
...
**Note:** Client developers should refer to SIP-712 for complete semantics around [encoding and signing data](./sip-712.md#specification). Dapp developers should refer to SIP-712 for the expected structure of [RPC method input parameters](./sip-712.md#parameters).
...
Much of Sila's effectiveness as an enterprise-grade application platform depends on its ability to provide a reliable and predictable developer experience. Nodes created by the current generation of Sila clients expose RPC endpoints with differing method signatures; this forces applications to work around method inconsistencies to maintain compatibility with various Sila RPC implementations.

Both Sila client developers and downstream dapp developers lack a formal Sila RPC specification. This proposal standardizes such a specification in a way that's versionable and modifiable through the traditional SIP process.

## Backwards compatibility

This proposal impacts Sila client developers by requiring that any exposed RPC interface adheres to this specification. This proposal impacts dapp developers by requiring that any RPC calls currently used in applications are made according to this specification.

## Implementation

The current generation of Sila clients includes several implementations that attempt to expose this RPC specification:

|Client Name|Language|Homepage|
|-|-|-|
|Geth|Go|[geth.sila.org](https://geth.sila.org)|
|Parity|Rust|[parity.io/sila](https://parity.io/sila)|
|Aleth|C++|[cpp-sila.org](https://cpp-sila.org)|
