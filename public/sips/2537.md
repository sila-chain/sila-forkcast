...
Certain operations have variable length input, such as MSMs (takes a list of pairs `(point, scalar)`), or pairing (takes a list of `(G1, G2)` points). While their behavior is well-defined (from an arithmetic perspective) on empty inputs, this SIP discourages such use cases and variable input length operations must return an error if the input is empty.
...
Following the current state of all other precompiles, if a call to one of the precompiles in this SIP results in an error then all the gas supplied along with a `CALL` or `STATICCALL` is burned.

### DDoS protection

A sane implementation of this SIP *should not* contain potential infinite loops (it is possible and not even hard to implement all the functionality without `while` loops) and the gas schedule accurately reflects the time spent on computations of the corresponding function (precompiles pricing reflects an amount of gas consumed in the worst case where such a case exists).
...
Implementations SHOULD use the optimized subgroup check method detailed in a dedicated [document](../assets/sip-2537/fast_subgroup_checks.md).
...
The algorithms and set of parameters for SWU mapping method are provided by a separate [document](../assets/sip-2537/field_to_curve.md)
...
Test vectors can be found [in the test vectors files](../assets/sip-2537/test-vectors.md).

### Benchmarking test cases

A set of test vectors for quick benchmarking on new implementations is located in a separate [file](../assets/sip-2537/bench_vectors.md)

## Reference Implementation

There are two fully spec compatible implementations on the day of writing:

- One in Rust language that is based on the [SIP-196](./sip-196.md) code and integrated with OpenEthereum for this library
