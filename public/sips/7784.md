...
Today, existing contract discovery relies on addresses, which are non-deterministic and can be obfuscated through proxies. Indexing by bytecode hash provides a deterministic and tamper-proof way to identify and verify contract code, enhancing security and trust in the Sila ecosystem.
...
This also allows to build new core standards more conveniently, for example, [SIP-7702](./sip-7702) can use it as dependency, to allow users who want to set up one-time code on their EOAs by referring to `GETCONTRACT` instead of specifying potentially mutable address based functionality.
...
Every contract stored in SAVM MUST be added to the state trie with the key being the keccak256 hash of the contract's bytecode, provided it is:
...
**[SIP-7702](./sip-7702) delegation:** Temporary code delegation is disallowed to prevent case of registering a code hash that becomes unavailable. Standard explicitly disallows contracts with bytecode delegated via [SIP-7702](./sip-7702) from being indexed.
