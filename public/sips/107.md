...
This draft SIP describes the details of an authorization method that if provided by rpc enabled sila nodes would allow regular websites to send transactions (via ```eth_sendTransaction```) without the need to enable CORS. Instead, user would be asked to confirm the transaction via an html popup.
...
- if the transaction is a plain sila transfer, the user is asked to enter it in a dedicated trusted wallet like "Mist"
...
![](../assets/sip-107/authorization.png)
...
![](../assets/sip-107/authorization-locked.png)
...
![](../assets/sip-107/authorization-password.png)
...
<html>
  <head>
    <title>Sila Authorization</title>
  </head>
  <script>
  ...
  <body>
    ...
    function askAuthorization(transactionInfo, data, requireUnlock, sourceWindow){
      ...
      textSpan.innerHTML = etherValue.toFormat() + " sila <br/>  + gas cost (" + gasEtherValue.toFormat() + " sila )"
