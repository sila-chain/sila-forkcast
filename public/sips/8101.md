...
This SIP introduces Payload Chunking, a protocol upgrade that restructures Sila block propagation into self-contained execution chunks with separated Chunk Access Lists (CALs). The proposer propagates beacon and execution payload headers, followed by CALs and chunks as independent network messages. CALs contain state diffs from chunk execution, enabling parallel chunk validation: chunk N can be validated once CALs 0..N are available. After all chunks validate successfully, the consensus layer finalizes the block. This architecture transforms block validation from a monolithic operation into a streaming pipeline while preserving block atomicity. Chunks exist only as a propagation and validation construct; the canonical chain stores complete execution payloads.

## Motivation

As Sila's gas limit increases, block sizes and execution complexity grow correspondingly. The current monolithic block structure requires validators to download and decompress the entire execution payload before beginning execution, creating a bottleneck in the consensus timeline. This sequential dependency—download, decompress, then execute—becomes increasingly problematic as blocks grow larger.
...
|------|-------|-------------|
| `CHUNK_GAS_LIMIT` | `2**24` (16,777,216) | Maximum gas per chunk, from [SIP-7825](./sip-7825.md) |
...
class ExecutionChunkHeader(Container):
    ...
    chunk_access_list_hash: Root     # keccak256(RLP(ChunkAccessList)) per SIP-7928
...
CALs are RLP-encoded structures following the Block Access List format from [SIP-7928](./sip-7928.md), scoped to individual chunks. Each CAL records state diffs produced by executing its corresponding chunk.

**Type Definitions:**

```python
Address = Bytes20              # Sila address
...
- **First CAL (chunk 0)**: Includes pre-execution system writes ([SIP-2935](./sip-2935.md) block hash history, [SIP-4788](./sip-4788.md) beacon root)
- **Last CAL (chunk N-1)**: Includes post-execution system operations ([SIP-4895](./sip-4895.md) withdrawals, [SIP-7002](./sip-7002.md) and [SIP-7251](./sip-7251.md) validator operations)
...
Note: If [SIP-7732](./sip-7732.md) (ePBS) is enabled, then `execution_payload` field of `ExecutionPayloadEnvelope` is replaced by `execution_payload_header` in the same way.
...
Note: The EL uses `keccak256(RLP(CAL))` in chunk headers per [SIP-7928](./sip-7928.md). The CL uses SSZ `hash_tree_root` for inclusion proofs. Both reference the same RLP-encoded CAL data.
...
**ePBS Integration ([SIP-7732](./sip-7732.md))**: PTC (Payload Timeliness Committee) members attest to chunk and CAL data availability. PTC attestations are independent of execution validity—they confirm only that all messages were received within the timeliness window.

### Execution Layer Changes

#### Chunk Execution

The execution layer validates chunks independently. For chunk `N`, the EL applies CALs `0..N-1` to the parent block state to reconstruct the chunk's pre-state, then executes the chunk transactions. It uses CAL `N` to execute transactions in parallel (the same as in [SIP-7928](./sip-7928.md)).
...
def el_execute_chunk(
    ...
    # Verify CAL hash matches chunk header commitment (SIP-7928 format)
...
| `MAX_TRANSACTIONS_PER_CHUNK` | `2^16` (65,536) | Accommodates chunks filled with minimal-gas transactions (SIL transfer - 21,000 gas) with margin for future gas cost reductions. |

## Backwards Compatibility

This SIP introduces breaking changes requiring a coordinated hard fork:
