...
|Chain Type| 2 |  Reserve 0x00 as undefined chaintype. 0x01 as sila-mainnet type. 0x1[0-A]: testnet, 0x2[0-A]: private development network|
...
Crosschainid can be backward compatible with SIP-155.  The crosschain id contains an 8 byte segment to record the `Native Chain ID`.
For Sila chains, that can be used for a value intended to be used with SIP-155.
